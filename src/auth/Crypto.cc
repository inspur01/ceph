...
#include <cryptopp/hex.h>  //add by zhanghao
			...
			std::string &error) const
{
...
#ifdef USE_CRYPTOPP
  {
	...
	CryptoPP::StreamTransformationFilter stfEncryptor(cbcEncryption, new CryptoPP::HexEncoder(new CryptoPP::StringSink(ciphertext)));//add by zhanghao
			...
			bufferlist& out, std::string &error) const
{
  ...
  CryptoPP::HexDecoder stfDecryptor(new CryptoPP::StreamTransformationFilter(cbcDecryption, sink));// add by zhanghao
  for (std::list<bufferptr>::const_iterator it = in.buffers().begin(); 
       it != in.buffers().end(); ++it) {
      const unsigned char *in_buf = (const unsigned char *)it->c_str();
