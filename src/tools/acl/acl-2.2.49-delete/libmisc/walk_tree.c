...
static sqlite3 *sqlite3_db_global;
...
close_another_dir:
		...
		while ((entry = readdir(dir.stream)) != NULL) {
			...
			*path_end++ = '/';
			...
			/*ÐÞ¸Äsqlite3_ACLÂ·¾¶ by Lvy*/
			
			if(  sqlite3_change_acl_path(latest_rowid, path, sqlite3_db_global) != 0 ) {
				
				fprintf(stderr, "sqlite3 change acl_path failed: %s",path);
			}
			err += walk_tree_rec(path, walk_flags, func, arg,
					     depth + 1, latest_rowid);
			*--path_end = 0;
			if (!dir.stream) {
...
int walk_tree(const char *path, int walk_flags, unsigned int num,
	      ...
	      void *arg, sqlite3_int64 latest_rowid)
{
	...
	DB_OPEN(sqlite3_db_global);
	ret = walk_tree_rec(path_copy, walk_flags, func, arg, 0, latest_rowid);
	DB_CLOSE(sqlite3_db_global);
	return ret;
}
