...
void usage()
{
    ...
    printf("\n"
           ...
           "icfs set quota!\n"
           "\n"
           "Usage: icfs-setquota [/setquota_directory_path] [size_value] \n"
           "\n"
           "Example :\n  icfs-setquota /mnt/icfs/test1/ 1000\n"
           "-------------------------------------------------------\n");
    exit(0);
}
...
int write_xattr_value(const char *opath,const char* xattr_name,const char* xattr_value){
    ...
    if(ret<0){
       ...
       //sprintf(log_buf,"write_xattr_value error [ipath:%s][name:%s][value:%s][ret:%d][errno:%d][strerror:%s]",opath,xattr_name,xattr_value,ret,errno_t,strerror(errno_t));
		   cout<<"icfs-setquota: "<< opath <<": "<< strerror(errno_t) << std::endl;
...
int read_xattr_value(const char *opath, const char* xattr_name, char* xattr_value, unsigned int xattr_value_size){
    ...
    if(ret<0 && errno_t != ENODATA){
        //cout << "icfs-setquota["<<LOG_ERROR<<"]: read_xattr_value error [opath:"<<opath<<"][name:"<<xattr_name<<"][ret:"<<ret<<"][errno:"<<errno_t<<"][strerror:"<<strerror(errno_t)<<std::endl;
        cout << "icfs-getquota: "<<opath<<": "<<strerror(errno_t)<<std::endl;
        return XATTR_ERROR;
    }else if(ret<0 && errno_t==ENODATA){
        //cout <<"icfs-setquota["<<LOG_DEBUG<<"]: read_xattr_value ENODATA [opath:"<<opath<<"][name:"<<xattr_name<<std::endl;
...
int main(int argc, char *argv[])
{
    ...
    FILE *fp;
    char find_str[]="client_dir_quota";
    char find_thin[]="client_fs_provisioning";
    int line=0;
    int thinnum=0;
    int quotanum=0;
    ...
    if(res<0)
    {
       // printf("ERROR 7: set path: [%s]  quota: [%s] value: [%s] failed!\n", path_name, xattr_name, quota_value);
        return res;
    }
    ...
    printf("set quota : %s %s\n", path_name, xattr_value);
    printf("set quota value %s SUCCESS!\n", quota_value);
	}else if(!squota){
       //fclose(fp);
       cout<<"icfs-setquota: "<<path_name<<": Operation not supported"<<std::endl;
       return -1;
   }else {
       cout<<"client_fs_provisioning can't be set to true "<<std::endl;
       return -1;
   }
	strcpy(string, "touch ");	
