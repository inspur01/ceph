...
class LRUObject {
 ...
 public:
  ...
  bool is_inlru(){return lru != 0;}
  void move_to(LRU *newlru);
  LRU * get_lru(){return lru;}
  bool have_relation(LRUObject *other){return (lru!=0 && lru==other->get_lru());}
  void remove_self();
  void touch_self();
  friend class LRU;
...
class LRU {
  ...
  */
  ...
  LRUObject *lru_pop_next(){
    LRUObject *p;

    if(lru_bot.get_length()) {
      p = lru_bot.get_tail();
      return lru_remove(p);
    }

    if(lru_top.get_length()) {
      p = lru_top.get_tail();
      return lru_remove(p);
    }

    if(lru_pintail.get_length()) {
      p = lru_pintail.get_tail();
      return lru_remove(p);
    }
...
inline void LRUObject::remove_self()
{
	if(lru){
		lru->lru_remove(this);
	}
}
inline void LRUObject::touch_self()
{
	if(lru){
		lru->lru_touch(this);
	}
}
inline void LRUObject::move_to(LRU *newlru)
{
  if(lru){
		lru->lru_remove(this);
	}
  newlru->lru_insert_top(this);
}
#endif
