...
//WRITE_RAW_ENCODER(UserQuotaData)
...
struct prettyunsignedbyte_t {
  int64_t v;
  prettyunsignedbyte_t(int64_t _v) : v(_v) {}
};

inline ostream& operator<<(ostream& out, const prettyunsignedbyte_t& b)
{
  uint64_t bump_after = 100;
  uint64_t uValue = 0;
  if(b.v >= 0)
  ...
  if (uValue > bump_after << 60)
    return out << (uValue >> 60) << " EB";
  if (uValue > bump_after << 50)
    return out << (uValue >> 50) << " PB";
  if (uValue > bump_after << 40)
    return out << (uValue >> 40) << " TB";
  if (uValue > bump_after << 30)
    return out << (uValue >> 30) << " GB";
  if (uValue > bump_after << 20)
    return out << (uValue >> 20) << " MB";
  if (uValue > bump_after << 10)
    return out << (uValue >> 10) << " kB";
  return out << uValue << " bytes";
}
