...
int main(int argc, const char **argv, const char *envp[]) {
  ...
  if (childpid == 0) {
    ...
    if(client->cct->_conf->client_file_quota) {
      r = client->file_quota_init();
      if (r != 0) {
        cerr << "ceph-fuse[" << getpid() << "]: fuse failed to file init" << std::endl;
        goto out_client_unmount;
      }      
    }
    if(client->cct->_conf->client_user_quota) { 
      r = client->user_quota_init();
      if (r != 0) {
        cerr << "ceph-fuse[" << getpid() << "]: fuse failed to user quota init" << std::endl;
        goto out_client_unmount;
      }     
    }
    if(client->cct->_conf->client_dir_quota || client->cct->_conf->client_fs_provisioning) {
      r = client->dir_quota_init();
      if (r != 0) {
        cerr << "ceph-fuse[" << getpid() << "]: fuse failed to dir quota init" << std::endl;
        goto out_client_unmount;
      }     
    }
    if(client->cct->_conf->client_dir_quota) {
      client->get_sys_maxbytes();
    }
    r = cfuse->loop();
