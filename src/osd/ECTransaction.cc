...
struct TransDumper: public boost::static_visitor<void> {
  typedef void result_type;
  ceph::Formatter& f;

  TransDumper(ceph::Formatter& _f): f(_f) {}
  
  void operator()(const ECTransaction::AppendOp &op) {
    f.open_object_section("op");
    f.dump_string("op_name", "append");
    f.dump_unsigned("offset", op.off);
  ...
  void operator()(const ECTransaction::RenameOp &op) {
    f.open_object_section("op");
    f.dump_string("op_name", "rename");
    f.close_section();
  }
  
  void operator()(const ECTransaction::StashOp &op) {
    f.open_object_section("op");
    f.dump_string("op_name", "stash");
    f.dump_unsigned("version", op.version);
    f.close_section();
  }
  ...
  void operator()(const ECTransaction::SetAttrsOp &op) {
    ...
    for (map<string, bufferlist>::const_iterator itr = op.attrs.begin();
         itr != op.attrs.end(); ++itr) {
      f.dump_string("name", itr->first);
      f.dump_unsigned("value_len", itr->second.length());
    }
    f.close_section();
    f.close_section();
  }
  ...
  {
    f.open_object_section("op");
    f.dump_string("op_name", "allochint");
    f.close_section();
  }
...
void ECTransaction::dump(ceph::Formatter& f)
{
  f.open_object_section("ECTransaction");
  f.dump_unsigned("num of ops", ops.size());
  f.dump_unsigned("written", written);
  TransDumper dumper(f);
  visit(dumper);
  f.close_section();
}
