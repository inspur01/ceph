...
void pg_pool_t::dump(Formatter *f) const
{
  ...
  f->dump_unsigned("tier_min_object_size", tier_min_object_size);
}
...
void pg_pool_t::encode(bufferlist& bl, uint64_t features) const
{
  ...
  ::encode(tier_min_object_size, bl);
  ENCODE_FINISH(bl);
}
...
void pg_pool_t::decode(bufferlist::iterator& bl)
{
  ...
  if (struct_v >= 18) {     
    ::decode(tier_min_object_size, bl);  
  } else {    
    tier_min_object_size = 0;  
  }
  DECODE_FINISH(bl);
  calc_pg_masks();
}
...
void pg_pool_t::generate_test_instances(list<pg_pool_t*>& o)
{
  ...
  a.tier_min_object_size = 1000000;
