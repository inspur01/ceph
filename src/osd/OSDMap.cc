...
int OSDMap::Incremental::get_net_marked_out(const OSDMap *previous) const
{
  int n = 0;
  int osd = 0;
  for (map<int64_t,uint32_t>::const_iterator p = new_weight.begin();
       p != new_weight.end();
       ++p) {
	if (p->first >> 32 != 0xFFFFFFFF)
	{
	    continue;
	}
	osd = p->first & 0xFFFFFFFF;
    if (p->second == CEPH_OSD_OUT && !previous->is_out(osd))
      n++;  // marked out
    if (p->second != CEPH_OSD_OUT && previous->is_out(osd))
      n--;  // marked in
  }
...
void OSDMap::Incremental::dump(Formatter *f) const
{
  ...
  //modify by wangshuguang for capacity balance
  for (map<int64_t,uint32_t>::const_iterator p = new_weight.begin(); p != new_weight.end(); ++p) {
    f->open_object_section("osd");
	f->dump_int("pool", p->first >> 32);
    f->dump_int("osd", p->first & 0x00000000FFFFFFFF);
...
{
  int old_size = max_osd;
  int i = old_size;
  int o = max_osd;
  max_osd = m;
  osd_state.resize(m);
  osd_weight.resize(m);
  for (map<__u32, vector<__u32> >::iterator p = pool_osd_weight->begin();
       p != pool_osd_weight->end(); ++p) {
      p->second.resize(m);
	  for(; i < max_osd; i++)
	  {
          p->second[i] = CEPH_OSD_OUT;
	  }
	  i = old_size;
  }
  for (; o<max_osd; o++) {
...
void OSDMap::adjust_osd_weights(const map<int,double>& weights, Incremental& inc) const
{
  ...
  for (map<int,double>::const_iterator p = weights.begin();
       p != weights.end(); ++p) {
	osd = p->first | 0xFFFFFFFF00000000;
    inc.new_weight[osd] = (unsigned)((p->second / max) * CEPH_OSD_IN);
  }
}
...
uint64_t OSDMap::get_features(int entity_type, uint64_t *pmask) const
{
  ...
  //add by wangshuguang for add chooseleaf_stable patch
...
int OSDMap::apply_incremental(const Incremental &inc)
{
  ...
  for (map<int64_t,pg_pool_t>::const_iterator p = inc.new_pools.begin();
	...
	(*pool_osd_weight)[p->first] = osd_weight;
  }
  ...
  for (map<int64_t,uint32_t>::const_iterator i = inc.new_weight.begin();
       i != inc.new_weight.end();
       ++i) {
	pool_id = i->first >> 32;
	osd_id = i->first & 0x00000000FFFFFFFF;
	if ((__u32)pool_id == 0xFFFFFFFF)
	{
	    set_weight(osd_id, i->second);
		set_all_pool_osd_weight(osd_id, i->second);

	    // if we are marking in, clear the AUTOOUT and NEW bits, and clear
	    // xinfo old_weight.
	    if (i->second) {
	      osd_state[osd_id] &= ~(CEPH_OSD_AUTOOUT | CEPH_OSD_NEW);
	      osd_xinfo[osd_id].old_weight = 0;
	    }
	}
	else
	{
	    set_pool_osd_weight(pool_id, osd_id, i->second);
	}    
  }
			...
			ps_t *ppps) const
{
  ...
  unsigned pool_id = pg.pool();
  map<__u32, vector<__u32> >::const_iterator iter = pool_osd_weight->find(pool_id);
  vector<__u32> weight(iter->second);
  for (unsigned int i = 0; i < weight.size(); i++)
  {
      if (osd_weight[i] == CEPH_OSD_OUT)
      {
  ...
  if (ruleno >= 0)
    crush->do_rule(ruleno, pps, *osds, size, weight);
...
void OSDMap::encode_client_old(bufferlist& bl) const
{
  ...
  //modify by wangshuguang for capacity balance
  //for ::encode(pool_osd_weight, bl)
  n = pool_osd_weight->size();
  ::encode(n, bl);
  for (map<__u32, vector<__u32> >::const_iterator p = pool_osd_weight->begin(); p != pool_osd_weight->end(); p++)
  {
      ::encode(p->first, bl);
	  ::encode(p->second, bl);
  }
...
void OSDMap::print_osd_pool_line(int cur, ostream *out, Formatter *f,int64_t poolid) const
{	
  if (f) {
    f->dump_unsigned("id", cur);
    f->dump_stream("name") << "osd." << cur;
  ...
  if (!exists(cur)) {
	...
	*out << "down\t";
	...
	unsigned temp = get_pool_osd_weight(poolid, cur);
	float reweight = (float)temp/65536;
	  
    if (out) {
      std::streamsize p = out->precision();
	  *out << std::setprecision(4)
	   << (exists(cur) ? reweight : 0)
	   << std::setprecision(p)
	   << "\t";
    }
    if (f) {
...
void OSDMap::print_osd_pool_tree(ostream *out, Formatter *f,int64_t poolid) const
{
	if (out)
    *out << "# id\tweight\ttype name\tup/down\treweight\n";
  if (f)
  ...
  for (set<int>::iterator p = roots.begin(); p != roots.end(); ++p) {
	...
	*out << std::setprecision(4) << weight << std::setprecision(oldprecision) << "\t";
	...
	f->open_object_section("item");
      ...
      if (cur >= 0) {
	print_osd_pool_line(cur, out, f,poolid);
	if (out)
	  *out << "\n";
	if (f) {
	  f->dump_float("crush_weight", weight);
  ...
  if (!stray.empty()) {
    ...
    for (set<int>::iterator p = stray.begin(); p != stray.end(); ++p) {
      if (out)
	*out << *p << "\t0\t";
      print_osd_pool_line(*p, out, f,poolid);
      if (out)
	*out << "\n";
    }
    if (f)
			  ...
			  int nosd, int pg_bits, int pgp_bits)
{
  ...
  //pool_names.push_back("rbd"); // alter by kongjun for bug 4015
