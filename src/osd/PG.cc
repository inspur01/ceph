...
bool PG::proc_replica_info(pg_shard_t from, const pg_info_t &oinfo)
{
  ...
  if (info.history.merge(oinfo.history))
    dirty_info = true;
  reg_next_scrub();
  reg_next_flush();//add by yutao at 20141210 for cachepool periodical flush
...
//add begin by yutao at 20141209 for cachepool periodical flush
void PG::reg_next_flush()
{
  utime_t now = ceph_clock_now(cct);
  flusher.flush_reg_stamp = now;
  if (is_primary())
    osd->reg_last_pg_flush(info.pgid, flusher.flush_reg_stamp);
}

void PG::unreg_next_flush()
{
  if (is_primary())
    osd->unreg_last_pg_flush(info.pgid, flusher.flush_reg_stamp);
}
//add end by yutao at 20141209 for cachepool periodical flush
...
void PG::scrub(ThreadPool::TPHandle &handle)
{
  ...
  scrubber.last_write_update = eversion_t();
  chunky_scrub(handle);
  ...
  while (!done) {
	...
	if (scrubber.reserved) {
	  ...
	  scrubber.end = candidate_end;
        ...
        scrubber.state = PG::Scrubber::ASK_REPLICAS;
        break;

      case PG::Scrubber::ASK_REPLICAS:
	...
	for (set<pg_shard_t>::iterator i = actingbackfill.begin();
      ...
      case PG::Scrubber::WAIT_REPLICAS:
        if (scrubber.waiting_on > 0) {
          ...
          scrubber.state = PG::Scrubber::CHECK_CHUNK_WRITE;
        }
        break;

      case PG::Scrubber::CHECK_CHUNK_WRITE:
        if (scrubber.last_write_update > scrubber.subset_last_update) {
          scrubber.subset_last_update = scrubber.last_write_update;
          scrubber.state = PG::Scrubber::ASK_REPLICAS;
        } else {
...
boost::statechart::result PG::RecoveryState::Stray::react(const MLogRec& logevt)
{
  ...
  if (msg->info.last_backfill == hobject_t()) {
    // restart backfill
    pg->unreg_next_scrub();
    pg->unreg_next_flush();//add by yutao at 20141210 for cachepool periodical flush
    pg->info = msg->info;
    pg->reg_next_scrub();
