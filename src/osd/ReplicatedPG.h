...
public:
  ...
  struct OpContext {
    ...
    // wumq added for ErasureCodePG begin
    list<OpRequestRef> waiting_for_ack;
    list<OpRequestRef> waiting_for_ondisk;
    
	// added for DirectECProcess
    struct OpsInfo {
      bool writefull;
      bool rmobj;
      uint64_t beginpos;
      ...
      OpsInfo():writefull(false), rmobj(false), 
        beginpos(-1ULL), endpos(-1ULL) {}

      // CEPH_OSD_OP_WRITE/APPEND/WRITEFULL/ZERO/TRIMTRUNC/TRUNCATE
      void update(uint64_t begin, uint64_t end) {
        if (beginpos == -1ULL || beginpos > begin)
          beginpos = begin;
        if (endpos == -1ULL || endpos < end)
          endpos = end;
      }

      // CEPH_OSD_OP_WRITEFULL/DELETE
      void invalid() {
        beginpos = -1ULL;
        endpos = -1ULL;
      }
    } ops_info;
    // wumq added for ErasureCodePG end
...
protected:
  ...
  virtual void eval_repop(RepGather*); // wumq modified for ErasureCodePG
  void issue_repop(RepGather *repop, utime_t now);
  virtual RepGather *new_repop(OpContext *ctx, ObjectContextRef obc, ceph_tid_t rep_tid); // wumq modified for ErasureCodePG
  ...
  bool agent_choose_mode_tick(PG *pg);//add begin by yutao at 20141205 for cachepool periodical flush
...
protected:
  ...
  void _make_clone(
    ...
    object_info_t *poi);
  virtual void execute_ctx(OpContext *ctx); // wumq modified for ErasureCodePG
  ...
  virtual void apply_and_flush_repops(bool requeue); // wumq modified for ErasureCodePG
...
public:
  ...
  void snap_trimmer();
  virtual int do_osd_ops(OpContext *ctx, vector<OSDOp>& ops); // wumq modified for ErasureCodePG
...
// wumingqiao added for ErasureCodePG end
};

// wumingqiao moved from ReplicatedPG.cc for ErasureCodePG end
