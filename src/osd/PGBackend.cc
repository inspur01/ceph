  ...
  ostream &errorstream)
{
  ...
  for (set<hobject_t>::const_iterator k = master_set.begin();
	...
	enum scrub_error_type error = be_compare_scrub_objects(auth->second->objects[*k],
	  ...
	  dout(0)<<"wym, form inconsistent pg"<<" shard "<<j->first
                <<": soid "<<*k<<" "<<ss.str()<<dendl;
	...
	dout(0)<<"wym, cur_missing add" << " j->first is: "<<j->first<<dendl;
	++shallow_errors;
	errorstream << pgid << " shard " << j->first
		    << " missing " << *k << std::endl;
      }
    ...
    if (!cur_missing.empty()) {
      ...
      dout(0)<<"wym, missing pg"<<dendl;
    }
    if (!cur_inconsistent.empty()) {
      inconsistent[*k] = cur_inconsistent;
      dout(0)<<"wym, inconsistent pg"<<dendl;
    }
    if (!cur_inconsistent.empty() || !cur_missing.empty()) {
      authoritative[*k] = auth->first;
      dout(0)<<"wym, authoritative add"<<dendl;
    }
  }
}
