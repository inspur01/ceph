...
#include "osd/ErasureCodePG.h" // wumq added for directec
...
OSDService::OSDService(OSD *osd) :
  ...
  scrubs_active(0),
  sched_flush_lock("OSDService::sched_flush_lock"),
  ...
  spg_t pgid)
{
  ...
  // wumq added for directec begin
  int type = createmap->get_pg_type(pgid.pgid);
  if (type == pg_pool_t::TYPE_ERASURE && cct->_conf->osd_pool_direct_erasure_code)
    pg = new ErasureCodePG(&service, createmap, pool, pgid, logoid, infooid);
  else if (type == pg_pool_t::TYPE_REPLICATED || type == pg_pool_t::TYPE_ERASURE)
  // wumq added for directec end
...
void OSD::add_newly_split_pg(PG *pg, PG::RecoveryCtx *rctx)
{
  ...
  pg->set_role(role);
  pg->reg_next_scrub();
  pg->reg_next_flush();//add by yutao at 20141210 for cachepool periodical flush
...
/*
 * add by zhanghao
 * handle shutdown all osd
 */
 void OSD::handle_shutdown_all_osd(MShutDownAllOsd *m)
 {
	if(m->is_shutdown_all) {
		dout(0) << "** shutdown via the messenge for mon(because of unauth or expired) **" << dendl;
...
void OSD::_dispatch(Message *m)
{
  ...
  case CEPH_MSG_SHUT_DOWN_ALL_OSD:
	handle_shutdown_all_osd(static_cast<MShutDownAllOsd*>(m));
	break;
    // -- need OSDMap --
...
//add begin by yutao at 20141205 for cachepool periodical flush
bool OSD::flush_should_schedule()
{
  double loadavgs[1];
  if (getloadavg(loadavgs, 1) != 1) {
    dout(10) << "flush_should_schedule couldn't read loadavgs\n" << dendl;
    return false;
  }
  
  //dout(0) << "flush_should_schedule loadavgs = " << loadavgs[0] <<  dendl;

  if (loadavgs[0] >= cct->_conf->osd_flush_load_threshold) {
    dout(20) << "flush_should_schedule loadavg " << loadavgs[0]
	     << " >= max " << cct->_conf->osd_flush_load_threshold
	     << " = no, load too high" << dendl;
    return false;
  }
  ...
  dout(20) << "flush_should_schedule loadavg " << loadavgs[0]
	   << " < max " << cct->_conf->osd_flush_load_threshold
	   << " = yes" << dendl;
  return loadavgs[0] < cct->_conf->osd_flush_load_threshold;
}

void OSD::sched_flush()
{
  assert(osd_lock.is_locked());

  //NOFLUSH so skip periodical flush
  if (service.get_osdmap()->test_flag(CEPH_OSDMAP_NOFLUSH)){
    return;
  }

  bool load_is_low = flush_should_schedule();

  dout(20) << "sched_flush load_is_low=" << (int)load_is_low << dendl;
  if (!load_is_low){
    return;
  }

  pair<utime_t, spg_t> pos;
  if (service.first_flush_stamp(&pos)) {
    do {
      utime_t t = pos.first;
      spg_t pgid = pos.second;
      dout(30) << "sched_flush examine " << pgid << " at " << t << dendl;

      PG *pg = _lookup_lock_pg(pgid);
      if (pg && pg != NULL) {
	if (pg->is_active() && pg->is_primary()) {
          if (pg->agent_choose_mode_tick(pg)) {
	    pg->unlock();
	    break;
	  }
	}
    ...
    } while  (service.next_flush_stamp(pos, &pos));
  }
  dout(20) << "sched_flush done" << dendl;
}
//add end by yutao at 20141205 for cachepool periodical flush
