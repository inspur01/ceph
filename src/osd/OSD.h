...
public:
  ...
  bool next_scrub_stamp(pair<utime_t, spg_t> next,
    ...
    *out = *iter;
	...
	Mutex sched_flush_lock;
	set< pair<utime_t,spg_t> > last_flush_pg;

	void reg_last_pg_flush(spg_t pgid, utime_t t) {
	  Mutex::Locker l(sched_flush_lock);
	...
	void unreg_last_pg_flush(spg_t pgid, utime_t t) {
	  ...
	  assert(it != last_flush_pg.end());
	  last_flush_pg.erase(it);
	}
	bool first_flush_stamp(pair<utime_t, spg_t> *out) {
	  Mutex::Locker l(sched_flush_lock);
	  if (last_flush_pg.empty())
		return false;
	  set< pair<utime_t, spg_t> >::iterator iter = last_flush_pg.begin();
	  *out = *iter;
	  return true;
	}
	bool next_flush_stamp(pair<utime_t, spg_t> next,
			  pair<utime_t, spg_t> *out) {
	  Mutex::Locker l(sched_flush_lock);
	  if (last_flush_pg.empty())
		return false;
	  set< pair<utime_t, spg_t> >::iterator iter = last_flush_pg.lower_bound(next);
	  if (iter == last_flush_pg.end())
		return false;
	  ++iter;
	  if (iter == last_flush_pg.end())
		return false;
...
protected:
  ...
  // -- scrubbing --
  void sched_scrub();
  void sched_flush();//add by yutao at 20141205 for cachepool periodical flush
  bool scrub_random_backoff();
  bool scrub_should_schedule();
  bool flush_should_schedule();//add by yutao at 20141205 for cachepool periodical flush
