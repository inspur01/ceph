...
public:
  ...
  public:
    ...
    map<int64_t,uint32_t> new_weight;    //modify by wangshuguang for capacity balance
...
private:
  ...
  ceph::shared_ptr< map<__u32, vector<__u32> > > pool_osd_weight;  //add by wangshuguang for capacity balance
...
//add by wangshuguang for capacity balance
 public:
  OSDMap() : epoch(0), 
	     ...
	     pool_osd_weight(new map<__u32, vector<__u32> >),
...
//add by wangshuguang for capacity balance
  void set_all_pool_osd_weight(int osd_id, unsigned weight)
  {
      assert(osd_id < max_osd);
	  for (map<__u32, vector<__u32> >::iterator iter = pool_osd_weight->begin();
	       iter != pool_osd_weight->end(); iter++)
   	  {
       	  if (iter->second[osd_id] == CEPH_OSD_OUT)
       	  {
       	      iter->second[osd_id] = weight;
       	  }
      }
  }

  unsigned get_pool_osd_weight(int pool_id, int osd_id) const
  {
      assert(osd_id < max_osd);
	  map<__u32, vector<__u32> >::const_iterator iter = pool_osd_weight->find(pool_id);
	  assert(iter != pool_osd_weight->end());
	  return iter->second[osd_id];
  }

  void set_pool_osd_weight(int pool_id, int osd_id, unsigned weight)
  {
      assert(osd_id < max_osd);
	  map<__u32, vector<__u32> >::iterator iter = pool_osd_weight->find(pool_id);
	  assert(iter != pool_osd_weight->end());
	  iter->second[osd_id] = weight;
  }
...
public:
 ...
 /*add by yueyuanfang for capacity balance*/
  void print_osd_pool_line(int cur, ostream *out, Formatter *f,int64_t poolid) const;
  void print_osd_pool_tree(ostream *out, Formatter *f,int64_t poolid) const;
 /*add by yueyuanfang for capacity balance*/
