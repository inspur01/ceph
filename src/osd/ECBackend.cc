...
void ECBackend::dispatch_recovery_messages(RecoveryMessages &m, int priority)
{
  ...
  // kongjun@inspur.com start
  ObjectStore::Transaction::iterator i = m.t->begin();
  if(i.have_op() || replies.size()){
  ...
  OpRequestRef _op)
{
  ...
  case MSG_OSD_EC_READ: {
    ...
    if (g_conf->filestore_async_read) {
      op->set_priority(priority);
      handle_sub_read_async(op->op.from, op->op, reply);
    } else {
  ...
  case MSG_OSD_EC_READ_REPLY: {
    ...
    dispatch_recovery_messages(rm, priority); // kongjun@inspur.com 2015-10-16
    return true;
  }
...
struct ECAsyncReadCallback: public Context {
  PGBackend::Listener* parent;
  int32_t peer;
  MOSDECSubOpReadReply* msg;
  
  ECAsyncReadCallback(ECBackend *backend, pg_shard_t from, MOSDECSubOpReadReply* msg)
    :parent(backend->get_parent()), peer(from.osd), msg(msg)
  {}
  
  void finish(int r) {
    if (msg) {
      parent->send_message_osd_cluster(peer, msg, parent->get_epoch());
      msg = NULL;
    }
  }
...
void ECBackend::handle_sub_read_async(
  pg_shard_t from, 
  ECSubRead& op, 
  MOSDECSubOpReadReply* msg)
{
  ECSubReadReply* reply = &msg->op;
  ...
  ObjectStore::AsyncReadRequest* request = new ObjectStore::AsyncReadRequest;
  for (map<hobject_t, list<pair<uint64_t, uint64_t> > >::iterator i = op.to_read.begin();
       i != op.to_read.end(); ++i) {
    pair<coll_t, ghobject_t> key = make_pair(
      i->first.is_temp() ? temp_coll : coll,
      ghobject_t(i->first, ghobject_t::NO_GEN, shard));
    list<ObjectStore::AsyncReadParam>& value = (*request)[key];
    list<pair<uint64_t, bufferlist> >& buffers = reply->buffers_read[i->first];
    for (list<pair<uint64_t, uint64_t> >::iterator j = i->second.begin();
         j != i->second.end(); ++j) {
      bufferlist bl;
      buffers.push_back(make_pair(j->first, bl));
      value.push_back(make_pair(*j, make_pair(&(buffers.back().second), (Context *)NULL)));
    }
  }
  
  Context *cb = new ECAsyncReadCallback(this, from, msg);
  store->async_read(request, get_parent()->bless_context(cb));
}
