...
void ReplicatedBackend::objects_async_read(
    const hobject_t& hoid,
    const list<ObjectStore::AsyncReadParam>& to_read,
    Context *on_complete) {
  ObjectStore::AsyncReadRequest* request = new ObjectStore::AsyncReadRequest;
  request->insert(make_pair(make_pair(coll, ghobject_t(hoid)), to_read));
  store->async_read(request, on_complete);
}
