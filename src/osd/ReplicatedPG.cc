...
{
  inflightreads = 1;
  if (pg->pool.info.require_rollback()) {
    pg->pgbackend->objects_read_async(
      obc->obs.oi.soid,
      pending_async_reads,
      new OnReadComplete(pg, this));
  } else {
    pg->pgbackend->objects_async_read(
      obc->obs.oi.soid,
      pending_async_reads,
      pg->bless_context(
        new OnReadComplete(pg, this)));
...
int ReplicatedPG::do_osd_ops(OpContext *ctx, vector<OSDOp>& ops)
{
  ...
  for (vector<OSDOp>::iterator p = ops.begin(); p != ops.end(); ++p, ctx->current_osd_subop_num++) {
	...
	} else if (pool.info.require_rollback()/* || g_conf->filestore_async_read*/) {
   ...
   case CEPH_OSD_OP_NOTIFY:
	...
	if (pool.info.require_rollback()) {
	  ...
	  ctx->mod_desc.mark_unrollbackable();
	  if (obs.exists) {
	    t->truncate(soid, op.extent.offset + op.extent.length);
	  }
	  t->write(soid, op.extent.offset, op.extent.length, osd_op.indata);
	}
	if (!obs.exists) {
...
void ReplicatedPG::on_shutdown()
{
  ...
  unreg_next_scrub();
  unreg_next_flush();//add by yutao at 20141210 for cachepool periodical flush
...
bool ReplicatedPG::agent_work(int start_max)
{
  ...
  for (vector<hobject_t>::iterator p = ls.begin();
    ...
    //add begin by yutao at 20150514 for oHSM
    //增加校验，限制小于tier_min_object_size的对象的迁移
    if (obc->obs.oi.size !=0 && obc->obs.oi.size < pool.info.tier_min_object_size) {
      dout(20) << __func__ << " skip (tier_min_object_size) " << obc->obs.oi << dendl;
      osd->logger->inc(l_osd_agent_skip);
      continue;
    }
    //add end by yutao at 20150514 for oHSM
    if (scrubber.write_blocked_by_scrub(obc->obs.oi.soid)) {
...
//add begin by yutao at 20141205 for cachepool periodical flush
bool ReplicatedPG::agent_choose_mode_tick(PG *pg)
{
  if (!agent_state) {
    //assert(!agent_state);
    return false;
  ...
  num_user_bytes -= unflushable_bytes;
  */
  if (num_user_objects <= 0) {
    //dout(0) << __func__ << pg << " null" << dendl;
    return false;
  }
  ...
  agent_state->evict_effort = 0.1 * 1000000;
  
  osd->agent_enable_pg(pg, agent_state->evict_effort);
  unreg_next_flush();
  reg_next_flush();
	  
  return true;
}
//add end by yutao at 20141205 for cachepool periodical flush
...
void ReplicatedPG::_scrub(ScrubMap& scrubmap)
{
  ...
  for (map<hobject_t,ScrubMap::object>::reverse_iterator p = scrubmap.objects.rbegin(); 
	...
	dout(0)<< "wym, 1.shallow_errors +1, "<<mode << " " << info.pgid << " " << soid
                          << " no '" << SS_ATTR << "' attr"<<dendl;
	continue;
      }
      bufferlist bl;
	...
	osd->clog->error() << mode << " " << info.pgid << " " << head
			  << " missing clones";
        ++scrubber.shallow_errors;
	dout(0)<< "wym, 2.shallow_errors +1, "<< mode << " " << info.pgid << " " << head
                          << " missing clones"<<dendl;
      }
    ...
    if (p->second.attrs.count(OI_ATTR) == 0) {
      osd->clog->error() << mode << " " << info.pgid << " " << soid
			<< " no '" << OI_ATTR << "' attr";
      ++scrubber.shallow_errors;
      dout(0)<< "wym, 3.shallow_errors +1, "<< mode << " " << info.pgid << " " << soid
                        << " no '" << OI_ATTR << "' attr"<<dendl;
      continue;
    }
    bufferlist bv;
    ...
    if (pgbackend->be_get_ondisk_size(oi.size) != p->second.size) {
      osd->clog->error() << mode << " " << info.pgid << " " << soid
			...
			<< ")";
      ++scrubber.shallow_errors;
      dout(0)<< "wym, 4.shallow_errors +1, "<< mode << " " << info.pgid << " " << soid
                        << " on disk size (" << p->second.size
                        << ") does not match object info size ("
                        << oi.size << ") adjusted for ondisk to ("
                        << pgbackend->be_get_ondisk_size(oi.size)
    ...
    if (!next_clone.is_min() && next_clone != soid) {
      ...
      ++scrubber.shallow_errors;
      dout(0)<< "wym, 5.shallow_errors +1, "<< mode << " " << info.pgid << " " << soid
                        << " expected clone " << next_clone<<dendl;
    }
	...
	osd->clog->error() << mode << " " << info.pgid << " " << soid
        ...
        ++scrubber.shallow_errors;
	dout(0)<< "wym, 6.shallow_errors +1, "<< mode << " " << info.pgid << " " << soid
                          << " snapset.head_exists=false, but head exists"<<dendl;
      }
      if (soid.snap == CEPH_SNAPDIR && snapset.head_exists) {
	osd->clog->error() << mode << " " << info.pgid << " " << soid
			  << " snapset.head_exists=true, but snapdir exists";
        ++scrubber.shallow_errors;
	dout(0)<< "wym, 7.shallow_errors +1, "<< mode << " " << info.pgid << " " << soid
                          << " snapset.head_exists=true, but snapdir exists"<<dendl;
      }
      if (curclone == snapset.clones.rend()) {
	next_clone = hobject_t();
      } else {
	...
	++scrubber.shallow_errors;
	dout(0)<< "wym, 8.shallow_errors +1, " << mode << " " << info.pgid << " " << soid
                          << " found clone without head"<<dendl;
	continue;
      }
	...
	++scrubber.shallow_errors;
	dout(0)<< "wym, 9.shallow_errors +1, " << mode << " " << info.pgid << " " << soid
                          << " size " << oi.size << " != clone_size "
                          << snapset.clone_size[*curclone]<<dendl;
      }
  ...
  if (!next_clone.is_min() &&
    ...
    osd->clog->error() << mode << " " << info.pgid
		      << " expected clone " << next_clone;
    ++scrubber.shallow_errors;
    dout(0)<< "wym, 10.shallow_errors +1, " << mode << " " << info.pgid
                      << " expected clone " << next_clone<<dendl;
  }
...
void ReplicatedPG::_scrub_finish()
{
  ...
  if (scrub_cstat.sum.num_objects != info.stats.stats.sum.num_objects ||
    ...
    osd->clog->error() << info.pgid << " " << mode
		      ...
		      << scrub_cstat.sum.num_bytes_hit_set_archive << "/" << info.stats.stats.sum.num_bytes_hit_set_archive << " hit_set_archive bytes.\n";
    ++scrubber.shallow_errors;
    dout(0)<< "wym, ReplicatedPG::scrub_finish shallow_errors  +1, " << info.pgid << " " << mode
                      << " stat mismatch, got "
                      << scrub_cstat.sum.num_objects << "/" << info.stats.stats.sum.num_objects << " objects, "
                      << scrub_cstat.sum.num_object_clones << "/" << info.stats.stats.sum.num_object_clones << " clones, "
                      << scrub_cstat.sum.num_objects_dirty << "/" << info.stats.stats.sum.num_objects_dirty << " dirty, "
