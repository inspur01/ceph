...
public:
  ...
  int mount(const std::string &mount_root)
  {
    ...
    if(client->cct->_conf->client_file_quota) {
      ret = client->file_quota_init();
      if (ret)
        goto fail;      
    }
    if(client->cct->_conf->client_user_quota) {
      ret = client->user_quota_init();
      if (ret)
        goto fail;    
    }
    if(client->cct->_conf->client_dir_quota) {
      ret = client->dir_quota_init();
      if (ret)
        goto fail;    
    }		
...
extern "C" int ceph_get_dir_quota(struct ceph_mount_info *cmount, const char *path, struct statvfs *stbuf)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  //Î´¿ªÆô×Ô¾«¼ò¹¦ÄÜ£¬·µ»Ø-1
  if ( !(cmount->get_client()->cct->_conf->client_fs_provisioning) )
    return -1;
  else
	return cmount->get_client()->ceph_get_disk_quota(path, stbuf);
}
...
extern "C" int ceph_get_work_id(struct ceph_mount_info *cmount, int fd, int *work_id)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    
  return cmount->get_client()->get_work_id(fd, work_id);
}

extern "C" int ceph_set_work_id(struct ceph_mount_info *cmount, int fd, int work_id)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    
  return cmount->get_client()->set_work_id(fd, work_id);
}
...
extern "C" int ceph_get_user_quota(struct ceph_mount_info *cmount,uid_t uid, gid_t gid,
	        uint64_t *hard, uint64_t *soft, uint64_t *used)
{
  if (!cmount->is_mounted())
    return -ENOTCONN; 
  int res = cmount->get_client()->ceph_get_user_quota(uid,gid,hard,soft,used); 
