...
int FileStore::peek_journal_fsid(uuid_d *fsid)
{
  ...
  FileJournal j(this, *fsid, 0, 0, journalpath.c_str(), false, false);
                        ...
                        Index *index)
{
  ...
  if (create && (!exist)) {
	/*  
    r = (*index)->created(oid, (*path)->path()); // bug 3817
    if (r < 0) {
...
int FileStore::lfn_link(coll_t c, coll_t newcid, const ghobject_t& o, const ghobject_t& newoid)
{
  ...
  if (!index_same) {
    ...
    if (r < 0)
      return -errno;
    /*
    r = index_new->created(newoid, path_new->path()); // bug 3817
    if (r < 0) {
      assert(!m_filestore_fail_eio || r != -EIO);
      return r;
    }*/
...
FileStore::FileStore(const std::string &base, const std::string &jdev, osflagbits_t flags, const char *name, bool do_update) :
  ...
  blk_size(0),
  fsid_fd(-1), op_fd(-1),
  basedir_fd(-1), current_fd(-1), temp_fd(-1), // journal-tuning
  backend(NULL),
  ...
  // journal-tuning begin
  m_internal_osd_journal_optimize(g_conf->internal_osd_journal_optimize),
  op_tracer_map(this),
  append_desc_map(this),
  temp_fd_cache(g_ceph_context, 32),
  // journal-tuning end
  // async-read begin
  readq_lock("FileStore:readq_lock"),
  async_read_stop(true),
  async_read_ops(g_ceph_context, "filestore_async_read_ops", g_conf->filestore_async_read_ops),
  async_read_bytes(g_ceph_context, "filestore_async_read_bytes", g_conf->filestore_async_read_bytes),
  async_read_thread(this),
  m_temp_write_dio(g_conf->temp_write_dio),
  m_filestore_async_read_in_journal(false),
  m_filestore_async_write(false)
  // async_read end
{
  ...
  if (g_conf->temp_write_sync_by == "O_SYNC")
    m_temp_write_sync_by = TEMP_WRITE_OSYNC;
  else if (g_conf->temp_write_sync_by == "fsync")
    m_temp_write_sync_by = TEMP_WRITE_FSYNC;
  else
    m_temp_write_sync_by = TEMP_WRITE_SYNCFS;
  dout(2) << "temp write sync by " << m_temp_write_sync_by << dendl;
}
...
int FileStore::open_journal()
{
  if (journalpath.length()) {
    ...
    journal = new FileJournal(this, fsid, &finisher, &sync_cond, journalpath.c_str(),
...
int FileStore::dump_journal(ostream& out)
{
  ...
  FileJournal *journal = new FileJournal(this, fsid, &finisher, &sync_cond, journalpath.c_str(), m_journal_dio);
  r = journal->dump(out);
  delete journal;
  return r;
}
...
int FileStore::mkfs()
{
  ...
  // journal-tuning begin
  ret = ::mkdirat(basedir_fd, "temp", 0755);
  if (ret < 0) {
    ret = -errno;
    if (ret != -EEXIST) {
      derr << "mkfs failed to create temp/ :" << cpp_strerror(ret) << dendl;
      goto close_fsid_fd;
    }
  }
  
  ret = append_desc_map.create();
  if (ret < 0) {
    derr << "mkfs failed to create append_desc_map" << dendl;
    goto close_fsid_fd;
  }
  // journal-tuning end
...
// 创建stash_map，解析出transaction，并对每个transaction进行decode
// tls[in], 传入transaction链表
// seq[in], 日志开始的位置或者条数
void FileStore::update_stash_info(list<Transaction*>& tls, uint64_t seq)
{ 
  SequencerPosition spos(seq, 0, 0);
  list<Transaction*>::iterator itr;
  for (itr = tls.begin(); itr != tls.end(); ++itr, ++spos.trans)
  {
      ...
      case Transaction::OP_TOUCH:
        {
          ...
          map<ghobject_t, list<RemoveInfo> >::iterator it = remove_map.find(oid);
          if (it != remove_map.end()) {
            it->second.back().nextop_exist = true;
          }
          dout(20) << spos << " OP_TOUCH " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_WRITE:
        {
          ...
          dout(20) << spos << " OP_WRITE " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_ZERO:
        {
          ...
          dout(20) << spos << " OP_ZERO " << cid << "/" << oid << dendl;
        }
        break;
      ...
      case Transaction::OP_TRUNCATE:
        {
          ...
          dout(20) << spos << " OP_TRUNCATE " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_REMOVE:
        {
          ...
          map<ghobject_t, StashInfo& >::iterator it = dst_oid_stashinfo_map.find(oid);
          if (it != dst_oid_stashinfo_map.end()) {
	    if(0 == it->second.obj_use_count)
              it->second.will_remove = true;
          }

          map<ghobject_t, list<RemoveInfo> >::iterator it_remove = remove_map.find(oid);
          if (it_remove != remove_map.end()) {
            it_remove->second.back().nextop_exist = true;
            it_remove->second.push_back(RemoveInfo(spos));
          } else {
            remove_map[oid].push_back(RemoveInfo(spos));
          }
          dout(5) << spos << " OP_REMOVE " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_SETATTR:
        {
          ...
          dout(20) << spos << " OP_SETATTR " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_SETATTRS:
        {
          ...
          dout(20) << spos << " OP_SETATTRS " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_RMATTR:
        {
          ...
          dout(20) << spos << " OP_RMATTR " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_RMATTRS:
        {
          ...
          dout(20) << spos << " OP_RMATTRS " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_CLONE:
        {
          ...
          map<ghobject_t, list<RemoveInfo> >::iterator it = remove_map.find(noid);
          if ( it != remove_map.end()) {
            it->second.back().nextop_exist = true;
          }
          dout(20) << spos << " OP_CLONE to " << cid << "/" << noid << dendl;
        }
        break;
      case Transaction::OP_CLONERANGE: // changed to OP_CLONERANGE2
        {
          ...
          dout(20) << spos << " OP_CLONERANGE to " << cid << "/" << noid << dendl;
        }
        break;
      case Transaction::OP_CLONERANGE2:
        {
          ...
          i.decode_oid();
          ghobject_t noid = i.decode_oid();
          i.decode_length(); // src off
          i.decode_length(); // len
          i.decode_length(); // dst off

          map<ghobject_t, list<RemoveInfo> >::iterator it = remove_map.find(noid);
          if (it != remove_map.end()) {
            it->second.back().nextop_exist = true;
          }
          dout(20) << spos << " OP_CLONERANGE2 to " << cid << "/" << noid << dendl;          
        }
        break;
      ...
      case Transaction::OP_COLL_ADD:
        {
          ...
          dout(20) << spos << " OP_COLL_ADD " << ncid << "/" << oid << dendl;								
        }
        break;
      case Transaction::OP_COLL_REMOVE:
        {
          ...
          dout(20) << spos << " OP_COLL_REMOVE " << cid << "/" << oid << dendl;													
        }
        break;
      case Transaction::OP_COLL_MOVE:
        {
          ...
          dout(20) << spos << " OP_COLL_MOVE " << ncid << "/" << oid << dendl;													
        }
        break;
      case Transaction::OP_COLL_MOVE_RENAME:
        {
          ...
          bool is_exists = exists(ncid, noid);
          list<StashInfo>& si_list = stash_map[ooid];
          si_list.push_back(StashInfo(noid, spos, is_exists));
          dst_oid_stashinfo_map.insert(std::pair<ghobject_t, StashInfo&>(si_list.back().dst_oid, si_list.back()));

          map<ghobject_t, list<RemoveInfo> >::iterator it = remove_map.find(noid);
          if (it != remove_map.end()) {
            if (!it->second.back().nextop_exist) {
              it->second.back().nextop_exist = true;
              it->second.back().will_rollback = noid.is_no_gen();
            }
          }

	  map<ghobject_t, StashInfo& >::iterator iter = dst_oid_stashinfo_map.find(ooid);
          if (iter != dst_oid_stashinfo_map.end()) {
	    iter->second.obj_use_count++;
          }
	  
          dout(5) << spos << " OP_COLL_MOVE_RENAME " << ocid << "/" << ooid
               << " --> " << ncid << "/" << noid << dendl;
        }
        break;
      ...
      case Transaction::OP_OMAP_CLEAR:
        {
          ...
          dout(20) << spos << "OP_OMAP_CLEAR " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_OMAP_SETKEYS:
        {
          ...
          dout(20) << spos << " OP_OMAP_SETKEYS " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_OMAP_RMKEYS:
        {
          ...
          dout(20) << spos << " OP_OMAP_RMKEYS " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_OMAP_RMKEYRANGE:
        {
          ...
          dout(20) << spos << " OP_OMAP_RMKEYRANGE " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_OMAP_SETHEADER:
        {
          ...
          dout(20) << spos << " OP_OMAP_SETHEADER " << cid << "/" << oid << dendl;					
        }
        break;
      ...
      case Transaction::OP_SETALLOCHINT:
        {
          coll_t cid = i.decode_cid();
          ghobject_t oid = i.decode_oid();
          i.decode_length(); // expected object size
          i.decode_length(); // expected write size
          ...
          dout(15) << spos << " OP_SETALLOCHINT " << cid << "/" << oid << dendl;					
        }
        break;    
      // journal-tuning begin
      case Transaction::OP_OPTIMIZE_RENAME:
        {
          i.decode_u32(); // seq
          i.decode_length(); // len
          coll_t cid = i.decode_cid();
          ...
          dout(20) << spos << " OP_OPTIMIZE_RENAME " << cid << "/" << oid << dendl;
        }
        break;
      case Transaction::OP_OPTIMIZE_FINISH_APPEND:
        i.decode_length(); // seq
        i.decode_oid();
        break;
      // journal-tuning end
      default:
        derr << __func__ << " bad op " << op << dendl;
        assert(0);
      }
      ++spos.op;
    }
...
int FileStore::mount()
{
  ...
  if (journal) {
    ...
    if (m_filestore_journal_writeahead) {
      ...
      if (g_conf->filestore_async_read_in_journal) {
        m_filestore_async_read_in_journal = true;
        dout(0) << "mount: async read in journal" << dendl;
      } else if (g_conf->filestore_async_read) {
        m_filestore_async_write = true;
        dout(0) << "mount: do optimized ops in async read thread" << dendl;
      }
    }
  } else {
  ...
  // journal-tuning begin
  temp_fd = ::openat(basedir_fd, "temp", O_RDONLY | O_DIRECTORY);
  if (temp_fd < 0) {
    ret = -errno;
    derr << "FileStore::mount: error opening temp/ : " << cpp_strerror(ret) << dendl;
    goto close_current_fd;
  }

  ret = append_desc_map.read();
  if (ret < 0) {
    derr << "FileStore::mount: error reading append_desc_map" << dendl;
    goto close_temp_fd;
  }
  // journal-tuning end
  ...
  // journal-tuning begin
  _optimize_clear_temp_objects(); 
  ret = append_desc_map.rollback();
  if (ret < 0) {
    derr << "FileStore::mount: error rollback append_desc_map" << dendl;
    goto close_temp_fd;
  }
  // journal-tuning end
...
close_temp_fd:
  VOID_TEMP_FAILURE_RETRY(::close(temp_fd));
  temp_fd = -1;
close_current_fd:
  VOID_TEMP_FAILURE_RETRY(::close(current_fd));
  current_fd = -1;
...
void FileStore::_do_op(OpSequencer *osr, ThreadPool::TPHandle &handle)
{
  ...
  int r = _do_transactions(o->tls, o->op, &handle, o->tls_num);
...
{
  Context *onreadable, *ondisk, *onreadable_sync;
  ...
  // journal-tuning begin
  if (m_internal_osd_journal_optimize && m_filestore_journal_writeahead && 
      journal && journal->is_writeable()) {
    _optimize_queue_transactions(osr, tls, osd_op, handle);
    return 0;
  }
  // journal-tuning end
...
void FileStore::_optimize_queue_transactions(
  OpSequencer* osr,
  ...
  ThreadPool::TPHandle* handle)
{
  ...
  OptimizedOpList* op_list;
  
  try {
    op_list = _optimize_parse_transactions(tls, new_tls, tls_num, tracers);
  } catch (std::exception& e) {
    derr << __func__ << " catch " << e.what() << dendl;
    assert(false);
  }
  
  list<Transaction*>& rtls = op_list ? new_tls : tls;
  Context *onreadable, *ondisk, *onreadable_sync;
  Transaction::collect_contexts(rtls, &onreadable, &ondisk, &onreadable_sync);
  for (list<Transaction*>::iterator i = rtls.begin(); i != rtls.end(); ++i)
    (*i)->set_osr(osr);
  
  Op* o = build_op(rtls, onreadable, onreadable_sync, osd_op);
  o->tls_num = tls_num;
  o->tracers = tracers;
  op_queue_reserve_throttle(o, handle);
  journal->throttle();
  ...
  o->op = op_num;
  if (op_list)
    op_list->set_pos(op_num);
  dout(15) << __func__ << " op_num " << op_num << " tls_num " << tls_num << dendl;
  
  if (m_filestore_do_dump)
   dump_transactions(o->tls, o->op, osr);
  
  osr->queue_journal(o->op);
  if (m_filestore_async_write) {
    WriteItem* item = new WriteItem(o, osr, ondisk, osd_op, op_list);
    enqueue(item);
    dout(3) << "PUSH WRITE" << dendl;
  } else {
    _op_journal_transactions(o->tls, o->op, 
      new C_JournaledAhead(this, osr, o, ondisk), osd_op, op_list);
  }
  ...
  uint64_t tls_num)
{
  ...
  for (list<Transaction*>::iterator p = tls.begin();
       p != tls.end();
       ++p, trans_num++) {
    r = _do_transaction(**p, op_seq, trans_num, handle, tls_num);
    if (r < 0)
      break;
    if (handle)
      handle->reset_tp_timeout();
  ...
  uint64_t tls_num)
{
  ...
  TracedOp top(tls_num, trans_num);
  ...
  while (i.have_op()) {
	...
	if(check_stash_info(cid,oid, spos)){
	...
	if(check_stash_info(oldcid,oldoid, spos)){
	  if (_check_replay_guard(oldcid, oldoid, spos) > 0) {
	    r = _collection_move_rename(oldcid, oldoid, newcid, newoid, spos);
	    if (top.tls_num) {
	      OpTracerRef tracer = op_tracer_map.get(oldoid);
	      tracer->finish(top);
	      tracer = op_tracer_map.get(newoid);
	      tracer->finish(top);
	    }
	  }
	}else{
          if (_check_replay_guard(oldcid, oldoid, spos) > 0){
	    _remove(oldcid, oldoid, spos);
	    if (top.tls_num) {
	      OpTracerRef tracer = op_tracer_map.get(oldoid);
	      tracer->finish(top);
	    }
    ...
    case Transaction::OP_OPTIMIZE_RENAME:
      {
        uint32_t seq = i.decode_u32();
        i.decode_length(); // len is unused
        coll_t cid = i.decode_cid();
        ghobject_t oid = i.decode_oid();
        if (_check_replay_guard(cid, oid, spos) > 0)
          r = _optimize_rename(seq, cid, oid);
        if (top.tls_num) {
	  OpTracerRef tracer = op_tracer_map.get(oid);
	  tracer->finish(top);
	}
    ...
    case Transaction::OP_OPTIMIZE_FINISH_APPEND:
      {
        uint64_t seq = i.decode_length();
        ghobject_t oid = i.decode_oid();
        append_desc_map.erase(seq);
        if (top.tls_num) {
          OpTracerRef tracer = op_tracer_map.get(oid);
          tracer->finish(top);
        }
...
void FileStore::sync_entry()
{
  ...
  again:
	...
	assert(0);
      ...
      // journal-tuning begin
      int retval = append_desc_map.sync_remove();
      assert(retval >= 0);
      // journal-tuning end
	...
	while (snaps.size() > 2) {
      ...
      check_disk(); // wumq added for dropped-disk checking
      op_tp.unpause();
    }
				       ...
				       const SequencerPosition& spos)
{
...

  {
    ...
    if (r < 0) {
      ...
      // should be safe, so just return 0 and move on.
      if(!replaying){
        dout(0) << __func__ << "assert(false) " << c << "/" << o << " from "
	       << oldcid << "/" << oldoid << dendl;
      }
      assert(replaying);
...
// 检查remove_map,确认操作是否需要执行,以及应用到的对象
// 参数: oid [in, out] 对象id,该id有可能会被代替为stash到的新对象的id
//       spos [in]     当前操作的位置信息  
// 返回值:true:执行;false:忽略
bool FileStore::check_remove_info(ghobject_t& oid, const SequencerPosition& spos)
{
  dout(20) << "check_remove_info oid = " << oid << dendl;
  if (!replaying || backend->can_checkpoint()){
    return true;
  }
  ...
  bIsVersion = !oid.is_no_gen();//如果是带版本号的remove,则执行
  if(bIsVersion){
    return bResult;
  }
	
  map<ghobject_t, std::list<RemoveInfo> >::iterator iter;    
  iter = remove_map.find(oid);
  if(iter != remove_map.end()){
    std::list<RemoveInfo> & lstremove = iter->second;
    std::list<RemoveInfo>::iterator iterremoveinfo = lstremove.begin();
    for(;iterremoveinfo != lstremove.end();iterremoveinfo++){
      if(iterremoveinfo->spos == spos){				     
        if(!iterremoveinfo->nextop_exist){//不存在后续操作
          bResult = true;
	  dout(5) << spos << "FileStore::check_remove_info remove no next op  oid ="<< oid << dendl;
        }else{//存在后续操作
          if(iterremoveinfo->will_rollback){//后续操作为roll-back
            bResult = false;
	    dout(5) << spos << "FileStore::check_remove_info  roll-back  oid ="<< oid << dendl;
          }else{//后续操作不是roll-back
            bResult = true;
	    dout(5)<< spos << "FileStore::check_remove_info remove oid ="<< oid << dendl;
          }
        }				 
        break;
      }
...
// 检查stash_map,确认操作是否需要执行,以及应用到的对象
// 参数: oid [in, out] 对象id,该id有可能会被代替为stash到的新对象的id
//       cid[in]  对象集合id
//       spos [in]     当前操作的位置信息  
// 返回值:true:执行;false:忽略
bool FileStore::check_stash_info(coll_t& cid,ghobject_t& oid, const SequencerPosition& spos)
{
  dout(20) << "check_stash_info oid = " << oid << dendl;
  ghobject_t oldoid = oid; 
  if (!replaying || backend->can_checkpoint()){
    return true;
  }
  ...
  bIsRollBack = !oid.is_no_gen();
  
  map<ghobject_t, std::list<StashInfo> >::iterator iter;  
    
  iter = stash_map.find(oid);
  if(iter != stash_map.end()){
    std::list<StashInfo> & lstStashInfo = iter->second;
    std::list<StashInfo>::iterator iterStashInfo = lstStashInfo.begin();
    for(;iterStashInfo != lstStashInfo.end();iterStashInfo++){
      if(bIsRollBack){//roll-back操作
        bResult = false;
        if(iterStashInfo->spos == spos){				     
          if(exists(cid, oid)){
            bResult = true;
            dout(5)<< spos << "FileStore::check_stash_info  roll-back  oid ="<< oid << dendl;
          }
          break;
        }	
      }else{//stash操作
        if(iterStashInfo->spos > spos){ //move_rename前面的操作
          if(exists(cid, iterStashInfo->dst_oid)/*iterStashInfo->exist*/){//目标存在,move_rename前面的操作应用到stash到的新对象上
            bResult = true;
            oid = iterStashInfo->dst_oid;				    
            dout(5) << spos<< "FileStore::check_stash_info oldoid ="<< oldoid <<" ,newoid="<<oid<< dendl;
          }else{
            if(iterStashInfo->will_remove){//目标不存在,有remove操作,可以忽略
              bResult = false;
              dout(5)<< spos << "FileStore::check_stash_info, RemoveOp exist, iterStashInfo->spos > spos.oldoid ="<< oldoid <<" ,newoid="<<oid << dendl;
            }else{//目标不存在,无remove操作,rename不成功,继续执行
              bResult = true;
              dout(5)<< spos << "FileStore::check_stash_info, new object doesn't exist, iterStashInfo->spos > spos.oldoid ="<< oldoid <<" ,newoid="<<oid << dendl;
            }
          }
          break;
        }else if(iterStashInfo->spos == spos){//move_rename操作
          if(exists(cid, iterStashInfo->dst_oid)/*iterStashInfo->exist*/){//目标存在,不需要再rename操作,但是必须执行remove操作删除旧对象
            bResult = false;
            dout(5)<< spos << "FileStore::check_stash_info, new object exist, iterStashInfo->spos == spos.oldoid ="<< oldoid <<" ,newoid="<<oid << dendl;
          }else{
            if(iterStashInfo->will_remove){//目标不存在,有后续的remove操作(新对象),但是必须执行remove操作删除旧对象
              bResult = false;
              dout(5)<< spos << "FileStore::check_stash_info, RemoveOp exist, iterStashInfo->spos == spos.oldoid ="<< oldoid <<" ,newoid="<<oid << dendl;
            }else{//目标不存在,无remove操作,需要rename操作
              bResult = true;
              dout(5)<< spos << "FileStore::check_stash_info, new object doesn't exist, iterStashInfo->spos == spos.oldoid ="<< oldoid <<" ,newoid="<<oid << dendl;
            }
          }
  ...
  dout(5)<< spos << "check_stash_info oid = " << oid <<" , stash_map.size= " <<stash_map.size()<<" , replaying="<<replaying<<" ,return "<<bResult<< dendl;
  return bResult;
}

// async-read begin
void FileStore::start_async_reader() {
  async_read_stop = false;
  async_read_thread.create();
}

void FileStore::stop_async_reader() {
  readq_lock.Lock();
  async_read_stop = true;
  readq_cond.Signal();
  readq_lock.Unlock();
...
ostream& operator<<(ostream& os, const FileStore::AsyncReadItem& item) {
  for (ObjectStore::AsyncReadRequest::const_iterator i = item.request->begin(); i != item.request->end(); ++i) {
    os << " [" << i->first.first << "/" << i->first.second;
    for (list<ObjectStore::AsyncReadParam>::const_iterator j = i->second.begin(); j != i->second.end(); ++j) {
...
void FileStore::async_read_entry() {
  dout(2) << "async_read_entry start" << dendl;
  
  list<WriteItem*> pending_writes;
  while (true) {
    readq_lock.Lock();
    if (readq.empty()) {
      if (async_read_stop) {
      ...
      readq_cond.Wait(readq_lock);
      readq_lock.Unlock();
      continue;
    }
    
    QueuedItem* item = readq.front();
    readq.pop_front();
    size_t len = readq.size();
    readq_lock.Unlock();

    switch (item->get_type())
    {
    case QueuedItem::QIT_ASYNC_READ:
      // to submit when meet a read
      if (pending_writes.size())
        submit_pending_writes(pending_writes);
      do_async_read(item);
      dout(2) << "POP READ" << dendl;
      delete item;
      break;

    case QueuedItem::QIT_WRITE:
      pending_writes.push_back(dynamic_cast<WriteItem*>(item));
      // to submit when queue is empty or to the max count
      if (0 == len || pending_writes.size() >= 32)
        submit_pending_writes(pending_writes);
      break;
    
    default:
      derr << "Unknown QueuedItem type " << item->get_type() << dendl;
      assert(false);
    }
  }
  
  dout(2) << "async_read_entry stop" << dendl;
}

void FileStore::do_async_read(QueuedItem* item)
{
  AsyncReadItem* ri = dynamic_cast<AsyncReadItem*>(item);
  assert(ri);
  AsyncReadRequest* req = ri->request;
  assert(req);
  
  for (AsyncReadRequest::iterator i = req->begin(); i != req->end(); ++i) {
    for (list<AsyncReadParam>::iterator j = i->second.begin(); j != i->second.end(); ++j) {
      int retval = read(i->first.first, i->first.second, 
        j->first.first, j->first.second, *(j->second.first));
      if (retval < 0) {
        derr << "read " << i->first.first << "/" << i->first.second
             << " [" << j->first.first << ", " << j->first.second
             << "] failed: " << cpp_strerror(retval) << dendl;
        assert(false);
      } else if (j->second.second) {
        j->second.second->complete(retval);
  ...
  if (ri->on_complete) {
    op_finisher.queue(ri->on_complete);
    ri->on_complete = NULL;
  }
  
  // async_read_ops.put(1);
  // async_read_bytes.put(ri->bytes);
  // dout(20) << "async_read finish " << *ri << dendl;
  delete ri->request;
  ri->request = NULL;
}

void FileStore::submit_pending_writes(list<WriteItem*>& items)
{
  assert(items.size());
  dout(2) << "POP WRITE " << items.size() << dendl;
  
  OptimizedOpList op_list;
  list<WriteItem*>::iterator i;
  for (i = items.begin(); i != items.end(); ++i) {
    assert(*i);
    if ((*i)->op_list) {
      op_list.splice(*((*i)->op_list));
      delete (*i)->op_list;
      (*i)->op_list = NULL;
    }
  }
  ...
  for (i = items.begin(); i != items.end(); ++i) {
    assert(!((*i)->op_list));
    _op_journal_transactions((*i)->op->tls, (*i)->op->op,
      new C_JournaledAhead(this, (*i)->osr, (*i)->op, (*i)->ondisk), (*i)->osd_op);
    delete *i;
    *i = NULL;
  }
  items.clear();
...
void FileStore::async_read(AsyncReadRequest* request, Context* on_complete) {
  assert(request);
  
  AsyncReadItem* item = new AsyncReadItem(request, on_complete);
  if (!item->get_bytes()) {
    delete request;
    if (on_complete)
      op_finisher.queue(on_complete);
    delete item;
    return;
  }
  
  //async_read_ops.take(1);
  //async_read_bytes.take(item->bytes);
  
  if (m_filestore_async_read_in_journal) {
    journal->submit_entry(item);
  } else {
    enqueue(item);
    dout(3) << "PUSH READ" << dendl;
  }
}
// async-read end

// wumq added for dropped-disk checking
void FileStore::check_disk()
{
  int fd = ::openat(basedir_fd, "check_disk", O_RDWR | O_CREAT, 0644);
  if (-1 == fd) {
    int err = errno;
    derr << __func__ << " failed:" << cpp_strerror(err) << dendl;
    assert(err != EIO);
  } else {
    TEMP_FAILURE_RETRY(::close(fd));
  }
