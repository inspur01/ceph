...
public:
  ...
  public:
    enum {
      ...
      OP_OPTIMIZE_RENAME = 101, // journal-tuning, seq, len, cid, oid
      OP_OPTIMIZE_FINISH_APPEND = 102, // journal-tuning, seq, oid
    };
  ...
  public:
    ...
    void optimize_rename(
      uint32_t seq,
      uint64_t len,
      coll_t cid,
      const ghobject_t& oid) {
      __u32 op = OP_OPTIMIZE_RENAME;
      ::encode(op, tbl);
      ::encode(seq, tbl);
      ::encode(len, tbl);
      ::encode(cid, tbl);
    ...
    void optimize_finish_append(
      uint64_t seq,
      const ghobject_t& oid) {
      __u32 op = OP_OPTIMIZE_FINISH_APPEND;
      ::encode(op, tbl);
      ::encode(seq, tbl);
      ::encode(oid, tbl);
      ++ops;
 ...
 public:
   ...
   * async_read
   */
  typedef pair<pair<uint64_t, uint64_t>, pair<bufferlist*, Context*> > AsyncReadParam;
  typedef map<pair<coll_t, ghobject_t>, list<AsyncReadParam> > AsyncReadRequest;
  
  virtual void async_read(AsyncReadRequest* request, Context* on_complete) {
    delete request;
    if (on_complete)
      on_complete->complete(-EOPNOTSUPP);
  }
