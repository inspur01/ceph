...
int HashIndex::_created(const vector<string> &path,
  ...
  dout(0) << "split*merge*16 is " << (uint64_t)(abs(merge_threshold)) * (uint64_t)split_multiplier * 16  << ", the oid is " << oid  << " , the real num is " << info.objs  << " !" << dendl;
  
  if (must_split(info)) {
	dout(0) << " inspur split is " << oid << " !" << dendl;
