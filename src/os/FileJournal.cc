...
int FileJournal::prepare_multi_write(bufferlist& bl, uint64_t& orig_ops, uint64_t& orig_bytes)
{
  ...
  uint64_t extra_bytes(0); // journal-tuning
  while (!writeq_empty()) {
    int r = prepare_single_write(bl, queue_pos, orig_ops, orig_bytes, extra_bytes);
    if (r == -ENOSPC) {
      if (orig_ops)
    ...
    if (bmax) {
      if (bl.length() + extra_bytes >= bmax && orig_ops) {
  ...
  dout(2) << "POP WRITE " << orig_ops << ", "
       << ((bl.length() + extra_bytes) >> 20) << " MBs" << dendl;
  //dout(2) << __func__ << " ops " << orig_ops << ", bytes " << orig_bytes << dendl;
  
  if (extra_bytes) { // journal-tuning
    fs->_optimize_syncfs();
    throttle_bytes.put(extra_bytes);
  }
...
int FileJournal::prepare_single_write(bufferlist& bl, off64_t& queue_pos, 
    uint64_t& orig_ops, uint64_t& orig_bytes, uint64_t& extra_bytes)
{
  // grab next item
  ...
  if (next_write.extra) { // journal-tuning
    OptimizedOpList* ops = static_cast<OptimizedOpList*>(next_write.extra);
    //dout(20) << "find optimized-ops: " << *ops << dendl;
    int retval = ops->apply(fs);
    assert(retval >= 0);
    extra_bytes += ops->get_bytes();
    delete ops;
    next_write.extra = NULL;
  }
...
void FileJournal::write_thread_entry()
{
  ...
  utime_t sleep_time;
  sleep_time.set_from_double(0.01);
...
void FileJournal::do_read()
{
  ...
  while (true) {
    ...
    fs->do_async_read(item);
    delete item;
    
    if (max_read_entries && count >= max_read_entries)
      break;
    
    if (max_read_bytes && bytes >= max_read_bytes)
      break;
  }
  
  if (count)
    dout(2) << "POP READ " << count << ", " << (bytes >> 20) << " MBs" << dendl;
}
...
void FileJournal::submit_entry(uint64_t seq, bufferlist& e, int alignment,
			       Context *oncommit, TrackedOpRef osd_op, void* extra)
{
  // dump on queue
  ...
  if (extra) { // journal-tuning
    OptimizedOpList* ops = static_cast<OptimizedOpList*>(extra);
    throttle_bytes.take(ops->get_bytes());
  }
...

  {
    ...
    writeq.push_back(write_item(seq, e, alignment, osd_op, extra)); // journal-tuning
    writeq_cond.Signal();
  }
}

void FileJournal::submit_entry(QueuedItem* item)
{
...
// kongjun start
void FileJournal::pre_read_journal_entry(uint64_t pre_op_seq)
{

	// store journal read start pos
	off64_t store_read_pos = read_pos;

	while (1) {
		bufferlist bl;
		uint64_t seq = pre_op_seq + 1;
		if (!read_entry(bl, seq)) {
			dout(3) << "pre_read_journal_entry: end of journal, done." << dendl;
			break;
		}

		if (seq <= pre_op_seq) {
			dout(3) << "pre_read_journal_entry: skipping old op seq " << seq << " <= " << pre_op_seq << dendl;
			continue;
		}
		assert(pre_op_seq == seq-1);

		dout(3) << "pre_read_journal_entry: applying op seq " << seq << dendl;
		bufferlist::iterator p = bl.begin();
...
void FileJournal::throttle()
{
  if (throttle_ops.wait(g_conf->journal_queue_max_ops))
    dout(10) << "throttle: waited for ops" << dendl;
  if (throttle_bytes.wait(g_conf->journal_queue_max_bytes))
    dout(10) << "throttle: waited for bytes" << dendl;
}
