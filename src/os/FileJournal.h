...
public:
  ...
  struct write_item {
    ...
    write_item(uint64_t s, bufferlist& b, int al, TrackedOpRef opref, void *ex) :
      seq(s), alignment(al), tracked_op(opref), extra(ex) {
      bl.claim(b);
    }
    write_item() : seq(0), alignment(0), extra(NULL) {}
  };
  ...
  void submit_entry(uint64_t seq, bufferlist& bl, int alignment,
		    ...
		    void *extra = NULL); // journal-tuning
  
  void submit_entry(QueuedItem* item);
...
private:
  ...
  int prepare_multi_write(bufferlist& bl, uint64_t& orig_ops, uint64_t& orig_bytes);
  int prepare_single_write(bufferlist& bl, off64_t& queue_pos, 
        uint64_t& orig_ops, uint64_t& orig_bytes, uint64_t& extra_bytes); // journal-tuning
  ...
  size_t max_read_entries;
  size_t max_read_bytes;
  
  void do_read();

 public:
  FileJournal(FileStore *store, uuid_d fsid, Finisher *fin, Cond *sync_cond, const char *f, bool dio=false, bool ai=true, bool faio=false) :
    ...
    max_read_entries(g_conf->journal_max_read_entries),
    max_read_bytes(g_conf->journal_max_read_bytes)
    { }
  ~FileJournal() {
    delete[] zero_buf;
  }
