...
private:
  ...
  int fsid_fd, op_fd, basedir_fd, current_fd, temp_fd; // journal-tuning
...
public:
  ...
  int _do_transactions(
    ...
    ThreadPool::TPHandle *handle, uint64_t tls_num = 0);
  ...
  void async_read(AsyncReadRequest* request, Context* on_complete);
...
private:
  ...
  void _optimize_queue_transactions(
    OpSequencer* osr,
    list<Transaction*>& tls,
    TrackedOpRef op,
    ThreadPool::TPHandle* handle);

  OptimizedOpList* _optimize_parse_transactions(
    list<Transaction*>& tls,
    list<Transaction*>& new_tls,
    uint64_t& tls_num,
    set<OpTracerRef>& tracers);
  
  int _optimize_temp_open(uint32_t seq, bool create, FDRef* outfd, bool dio = false);
  int _optimize_write_fd(int fd, const bufferlist& bl, uint64_t off, uint64_t len);
  int _optimize_temp_write(TempWrite& op);
  int _optimize_append(OptimizedAppend& op);
  int _optimize_rename(uint32_t seq, coll_t& cid, const ghobject_t& oid);
  void _optimize_clear_temp_objects();
  void _optimize_syncfs();
  // journal-tuning end

  // stash-map begin
  struct StashInfo {
    ghobject_t dst_oid;// 目标对象OID
    SequencerPosition spos;// stash的位置
    bool exist;// 目标对象是否存在
    bool will_remove;// 操作中是否有remove操作
    int  obj_use_count;//对象使用次数，默认值为0，有stash操作时会加一，当非零时后续有remove操作时也不设置will_remove为true
        
    StashInfo(ghobject_t _dst_oid, SequencerPosition _spos, bool _exist, bool _will_remove = false,int _count=0)
      :dst_oid(_dst_oid), spos(_spos), exist(_exist), will_remove(_will_remove),obj_use_count(_count) {}
  };

  struct RemoveInfo {
    SequencerPosition spos;// remove的位置
    bool nextop_exist;// 是否存在后续操作
    bool will_rollback;// 操作中是否有rollback操作

    RemoveInfo(SequencerPosition _spos, bool _nextop_exist = false, bool _will_rollback = false)
      :spos(_spos), nextop_exist(_nextop_exist), will_rollback(_will_rollback) {}
  };

  // 存放ghobject_t 对应的_collection_move_rename操作
  map<ghobject_t, list<StashInfo> > stash_map; 

  // 目标对象的oid作为key，方便check_stash_info进行后续是否有remove操作的判断
  map<ghobject_t, StashInfo& > dst_oid_stashinfo_map; 

  // 目标对象的oid作为key，方便check_stash_info进行remove后续是否有rollback操作的判断
  map<ghobject_t, list<RemoveInfo> > remove_map; 

  // 创建stash_map，解析出transaction，并对每个transaction进行decode
  // tls[in], 传入transaction链表
  // seq[in], 日志开始的位置或者条数
  void update_stash_info(list<Transaction*>& tls, uint64_t seq);

  // 检查stash_map,判断每一个操作是否需要执行
  // oid[in,out] 返回时可能成为stash后的新对象的oid(把之前A上的操作，应用到move_rename后的B上)
  // cid[in]  对象集合id
  // spos[in] 当前操作的位置
  // 返回值：真则执行操作，假则不执行操作
  bool check_stash_info(coll_t& cid,ghobject_t& oid, const SequencerPosition& spos);
  // 检查remove_map,确认操作是否需要执行,以及应用到的对象
  // 参数: oid [in] 对象id,该id有可能会被代替为stash到的新对象的id
  //       spos[in] 当前操作的位置信息  
  // 返回值:true:执行;false:忽略
  bool check_remove_info(ghobject_t& oid, const SequencerPosition& spos);
  // stash-map end

  void check_disk(); // wumq added for dropped-disk checking
  ...
  // journal-tuning begin
  bool m_internal_osd_journal_optimize;
  OpTracerMap op_tracer_map;
  AppendDescMap append_desc_map;
  SharedLRU<uint32_t, FDCache::FD> temp_fd_cache;
  // journal-tuning end

  // async-read begin
  struct AsyncReadItem: public QueuedItem {
    AsyncReadRequest* request;
    Context *on_complete;
    
    AsyncReadItem(AsyncReadRequest* req, Context* oc)
      :QueuedItem(QIT_ASYNC_READ), request(req), on_complete(oc) {
      for (AsyncReadRequest::iterator i = request->begin(); i != request->end(); ++i) {
        for (list<AsyncReadParam>::iterator j = i->second.begin(); j != i->second.end(); ++j) {
          bytes += j->first.second;
        }
      }
    }
  };
  
  struct WriteItem: public QueuedItem {
    struct Op* op;
    OpSequencer* osr;
    Context* ondisk;
    TrackedOpRef osd_op;
    ...
    WriteItem(struct Op* _op, OpSequencer* _osr, Context* _ondisk, TrackedOpRef _osd_op, OptimizedOpList* _op_list)
      :QueuedItem(QIT_WRITE), op(_op), osr(_osr), ondisk(_ondisk), osd_op(_osd_op), op_list(_op_list) 
    {
      bytes = op->bytes;
      if (op_list)
        bytes += op_list->get_bytes();
    }
  };
  
  friend ostream& operator<<(ostream& os, const AsyncReadItem& item);
  
  Mutex readq_lock;
  Cond readq_cond;
  deque<QueuedItem*> readq;
  bool async_read_stop;
  
  Throttle async_read_ops, async_read_bytes;
  
  class AsyncReader: public Thread {
    FileStore *fs;
  public:
    AsyncReader(FileStore *_fs) : fs(_fs) {}
    void *entry() {
      fs->async_read_entry();
      return 0;
    }
  ...
  void stop_async_reader();
  void async_read_entry();
  
  void enqueue(QueuedItem* item) {
    Mutex::Locker l(readq_lock);
    readq.push_back(item);
    if (1 == readq.size())
      readq_cond.Signal();
  }
  
  void do_async_read(QueuedItem* item);
  void submit_pending_writes(list<WriteItem*>& items);

  bool m_temp_write_dio;
  enum {
    TEMP_WRITE_OSYNC = 0, // O_SYNC
    TEMP_WRITE_FSYNC, // fdatasync
    TEMP_WRITE_SYNCFS, // syncfs
  } m_temp_write_sync_by;
  bool m_filestore_async_read_in_journal;
  bool m_filestore_async_write;
  // async-read end
  ...
  friend class OpTracerMap;
  friend class AppendDescMap;
  friend struct TempWrite;
  friend struct OptimizedAppend;
  friend class OptimizedOpList;
  friend class FileJournal;
