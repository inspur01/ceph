...
bool Inode::check_mode(uid_t ruid, UserGroups& groups, unsigned want)
{
    if (uid == ruid)
    {
        // if uid is owner, owner entry determine access
        want = want << 6;
    }
    else if (groups.is_in(gid))
    {
        // if a gid or sgid matches the owning group, group entry determine access
        want = want << 3;
    }
...
Inode* Inode::get_son(string name)
{
  Inode *opi = NULL;
  if(is_dir()) {
    if(dir == NULL)
      return NULL;
    //map<string, Dentry*> &dnmap = dir->dentry_map;
    ceph::unordered_map<string, Dentry*> &dnmap = dir->dentries;
    ceph::unordered_map<string, Dentry*>::iterator it = dnmap.find(string(name));
    if(it != dnmap.end()) {
      if(it->second == NULL)
        return NULL;
      opi = it->second->inode;
