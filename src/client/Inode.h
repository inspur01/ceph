...
struct Inode {
  ...
  bool is_remote_fetched; //  for md readahead
  CephContext *cct;
  ...
  Inode(CephContext *cct_, vinodeno_t vino, ceph_file_layout *newlayout)
    : cct(cct_), ino(vino.ino), snapid(vino.snapid),
      ...
      fcntl_locks(NULL), flock_locks(NULL), bremoved(false),is_remote_fetched(true)
  {
  ...
  bool check_mode(uid_t ruid, UserGroups& groups, unsigned want);
