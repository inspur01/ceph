...
int RGWRados::complete_atomic_overwrite(RGWRadosCtx *rctx, RGWObjState *state, rgw_obj& obj)
{
  ...
  //string tag = state->obj.bucket.bucket_id;
  string tag = state->manifest.get_obj_manifest_prefix();
    ...
    const string& obj_key, string *bucket_obj, int *shard_id)
{
  ...
  RGWRadosCtx obj_ctx(this);

  // Get the bucket info
  RGWBucketInfo binfo;
  ret = get_bucket_instance_info((void*)&obj_ctx, bucket, binfo, NULL, NULL);
  if (ret < 0)
    return ret;
...
int RGWRados::apply_olh_log(RGWRadosCtx& obj_ctx, RGWObjState& state, RGWBucketInfo& bucket_info, rgw_obj& obj,
                            ...
                            uint64_t *plast_ver)
{
  ...
  *plast_ver = last_ver;
  ...
  for (list<cls_rgw_obj_key>::iterator liter = remove_instances.begin();
    ...
    //void *ctx = (void*)&oj_ctx;
    //int ret = delete_obj((void *)&obj_ctx, bucket_info.owner, obj_instance, 0, RGW_BILOG_FLAG_VERSIONED_OP);
    int ret = delete_obj((void *)&obj_ctx, bucket_info.owner, obj_instance);
    if (ret < 0 && ret != -ENOENT) {
 ...
 */
int RGWRados::update_olh(RGWRadosCtx& obj_ctx, RGWObjState *state, RGWBucketInfo& bucket_info, rgw_obj& obj)
{
  map<uint64_t, vector<rgw_bucket_olh_log_entry> > log;
  bool is_truncated;
...
int RGWRados::get_olh_target_state(RGWRadosCtx& obj_ctx, rgw_obj& obj, RGWObjState *olh_state,
...
int RGWRados::stat_system_obj(RGWRadosCtx& obj_ctx,
...
int RGWRados::get_system_obj(RGWRadosCtx& obj_ctx, RGWRados::SystemObject::Read::GetObjState& read_state,
...
int RGWRados::follow_olh(RGWRadosCtx& obj_ctx, RGWObjState *state, rgw_obj& olh_obj, rgw_obj *target)
{
  ...
  if (!pending_entries.empty()) {
     ...
     */
    RGWBucketInfo bucket_info;
    int ret = get_bucket_instance_info((void*)&obj_ctx, olh_obj.bucket, bucket_info, NULL, NULL);
    if (ret < 0) {
