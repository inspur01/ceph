...
static int rgw_check_metadata_all_num(CephContext *cct, map<string, bufferlist>& orig_attrs,
                 map<string, bufferlist>& attrs, int &metadata_num, bool is_object = false)
{
  int metadata_free_num = 0;
  int metadata_add_num = 0;
  
  //if (is_object){
  //  metadata_free_num = cct->_conf->rgw_max_metadata_num - metadata_num + 1;
  //}else{
  metadata_free_num = cct->_conf->rgw_max_metadata_num - metadata_num;
  //}
  
  map<string, bufferlist>::iterator iter;
  //get attrs user metadata 
  ...
  if (metadata_free_num < metadata_add_num)
  { 
     int erase_num = metadata_add_num - metadata_free_num;
     for (iter = attrs.begin(); iter != attrs.end() && erase_num > 0; ++iter) {
       const string& name = iter->first;
       /* check if the attr is user-defined metadata item */
       if (name.compare(0, sizeof(RGW_ATTR_META_PREFIX) - 1, RGW_ATTR_META_PREFIX) == 0 && (orig_attrs.find(name) != orig_attrs.end())) {
          erase_num--;
       } 	 
     }
     bool is_erase = false; 
...
#define C_ACCT_REMOVE_ATTR_PREFIX     "HTTP_X_REMOVE_ACCOUNT_META_"
#define C_ACCT_PUT_ATTR_PREFIX        "HTTP_X_ACCOUNT_META_"
#define C_CONT_REMOVE_ATTR_PREFIX     "HTTP_X_REMOVE_CONTAINER_META_"
#define C_CONT_PUT_ATTR_PREFIX        "HTTP_X_CONTAINER_META_"
#define C_OBJ_PUT_ATTR_PREFIX         "HTTP_X_OBJECT_META_"
#define C_OBJ_REMOVE_ATTR_PREFIX      "HTTP_X_REMOVE_OBJECT_META_"
static int rgw_check_metadata_deal_num(const req_state * const s,
...
{ 
  ...
  if (s->cct->_conf->rgw_max_metadata_num < attrs_num){
    return 0;
  }
  return 1;
}
...
int RGWCreateBucket::verify_permission()
{
  ...
  if (!rgw_check_metadata_deal_num(s, C_CONT_PUT_ATTR_PREFIX, C_CONT_REMOVE_ATTR_PREFIX)){
    return -ERR_TOO_MANY_METADATA;
  }
...
void RGWCreateBucket::execute()
{
  ...
  ret = rgw_check_metadata_all_num(s->cct, orig_attrs, attrs, metadata_num);
  if (ret < 0)
    return; 
...
int RGWPutObj::verify_permission()
{
  if (!verify_bucket_permission(s, RGW_PERM_WRITE))
    return -EACCES;
  if (!rgw_check_metadata_deal_num(s, C_OBJ_PUT_ATTR_PREFIX, C_OBJ_REMOVE_ATTR_PREFIX)){
    return -ERR_TOO_MANY_METADATA;
  }
...
static void prepare_add_del_attrs(const map<string, bufferlist>& orig_attrs,
                                  ...
                                  map<string, bufferlist>& out_attrs,
                                  map<string, bufferlist>& out_rmattrs, int &metadata_num)
{
  map<string, bufferlist>::const_iterator iter;
...
int RGWPutMetadataAccount::verify_permission()
{
  ...
  //   return -EACCES;
  // }
  if (!rgw_check_metadata_deal_num(s, C_ACCT_PUT_ATTR_PREFIX, C_ACCT_REMOVE_ATTR_PREFIX)){
    return -ERR_TOO_MANY_METADATA;
  }
  return 0;
}
...
void RGWPutMetadataAccount::execute()
{
  ...
  prepare_add_del_attrs(orig_attrs, rmattr_names, attrs, rmattrs, metadata_num);
  populate_with_generic_attrs(s, attrs);
  op_ret = rgw_check_metadata_all_num(s->cct, orig_attrs, attrs, metadata_num);
  ...
  /* XXX tenant needed? */
  if (op_ret < 0){  
      rgw_store_user_attrs(store, s->user.user_id, attrs, &rmattrs,
				&acct_op_tracker);
  }else{
      op_ret = rgw_store_user_attrs(store, s->user.user_id, attrs, &rmattrs,
				&acct_op_tracker);
  }
  if (op_ret < 0) {
    return;
...
/*
int RGWPutMetadataBucket::check_metadata_deal_num()
{
  if (!rgw_check_metadata_deal_num(s, CONT_PUT_ATTR_PREFIX, CONT_REMOVE_ATTR_PREFIX)){
    return -ERR_TOO_MANY_METADATA;
  }
  return 1;
}*/
...
void RGWPutMetadataBucket::execute()
{
  ...
  if (op_ret < 0){
     rgw_bucket_set_attrs(store, s->bucket_info, attrs, &rmattrs,
...
void RGWPutMetadata::execute()
{
  ...
  ret = rgw_check_metadata_all_num(s->cct, orig_attrs, attrs, metadata_num, true);
  ...
  if (ret < 0){
    if (s->object) {
      store->set_attrs(s->obj_ctx, obj, attrs, &rmattrs, ptracker);
    } else {
