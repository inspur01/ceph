...
static void rgw_set_keystone_token_auth_info(KeystoneToken& token, struct rgw_swift_auth_info *info, req_state *s = NULL)
{
  info->user = token.token.tenant.id;
  info->display_name = token.token.tenant.name;
  //if (s != NULL)
     //s->cio->print("X-Storage-Url: %s%s\r\n", s->info.script_uri.c_str(), token.token.tenant.name.c_str());
  
  info->status = 200;
}

int RGWSwift::parse_keystone_token_response(const string& token, bufferlist& bl, struct rgw_swift_auth_info *info, KeystoneToken& t, req_state *s)
{
  ...
  rgw_set_keystone_token_auth_info(t, info, s);
...
int RGWSwift::validate_keystone_token(RGWRados *store, const string& token, struct rgw_swift_auth_info *info,
				      RGWUserInfo& rgw_user, req_state *s)
{
  KeystoneToken t;
  ...
  int ret = parse_keystone_token_response(token, bl, info, t, s);
  if (ret < 0)
    return ret;
...
bool RGWSwift::do_verify_swift_token(RGWRados *store, req_state *s)
{
  ...
  if (supports_keystone()) {
    ret = validate_keystone_token(store, s->os_auth_token, &info, s->user, s);
    return (ret >= 0);
  }
