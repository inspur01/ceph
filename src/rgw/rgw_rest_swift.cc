...
#define OBJ_PUT_ATTR_PREFIX         "HTTP_X_OBJECT_META_"
#define OBJ_REMOVE_ATTR_PREFIX      "HTTP_X_REMOVE_OBJECT_META_"
...
int RGWPutMetadata_ObjStore_SWIFT::get_params()
{
  ...
  //get_rmattrs_from_headers(s, OBJ_PUT_ATTR_PREFIX, OBJ_REMOVE_ATTR_PREFIX, rmattr_names);
...
int RGWHandler_ObjStore_SWIFT::init_from_header(struct req_state *s)
{
  ...
  char buf[g_conf->rgw_swift_url_prefix.length() + 16 + tenant_path.length()];
  int blen = sprintf(buf, "/%s/%s%s", g_conf->rgw_swift_url_prefix.c_str(), v1.c_str(), tenant_path.c_str());
