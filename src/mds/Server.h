...
public:
  ...
  void journal_and_reply(MDRequestRef& mdr, CInode *tracei, CDentry *tracedn,
			 LogEvent *le, MDSInternalContextBase *fin, bool early_reply_mds);
  ...
  void handle_client_readahead(MDRequestRef& mdr);
  void handle_client_readahead_dir_complete(MDRequestRef& mdr);
  ...
  void handle_client_quota_get_all(MDRequestRef& mdr);
  void handle_client_quota_get(MDRequestRef& mdr);
  void handle_client_quota_set(MDRequestRef& mdr);
  void handle_client_quota_modify(MDRequestRef& mdr);
  void handle_client_quota_delete(MDRequestRef& mdr);
  void handle_client_quota_clear(MDRequestRef& mdr);
  void handle_client_quota_update(MDRequestRef& mdr);
  void handle_client_quota_update_file(MDRequestRef& mdr);
  void handle_client_quota_file_get_all(MDRequestRef& mdr);
  void handle_client_quota_check(MDRequestRef& mdr);
  bool get_user_quota(unsigned int id, bufferlist &bufferlist);
  bool get_grp_quota(unsigned int id, bufferlist &bufferlist);
  void handle_client_grp_quota_set(MDRequestRef& mdr);
  void handle_client_grp_quota_modify(MDRequestRef& mdr);
  void handle_client_grp_quota_delete(MDRequestRef& mdr);
  void handle_client_grp_quota_clear(MDRequestRef& mdr);
  void handle_client_grp_quota_update(MDRequestRef& mdr);
  void handle_client_grp_file_quota_update(MDRequestRef& mdr);
  bool is_exceed_grp_left_quota(UserQuotaData &qdata,uint64_t old_usr_hardlimit = 0);
  bool is_below_grp_used_quota(uint64_t grp_hardlimit, gid_t gid);
  void broadcast_user_quota(MDRequestRef& mdr, UserQuotaData &uqdata, uint64_t op);
  void broadcast_grp_quota(MDRequestRef& mdr, GrpQuotaData &gqdata, uint64_t op);
  /*dir quota*/
  void handle_client_dir_quota_update(MDRequestRef& mdr);
  void handle_client_dir_quota_get_all(MDRequestRef& mdr);
  void handle_client_dir_quota_get(MDRequestRef& mdr);
  void handle_client_dir_quota_del(MDRequestRef& mdr);
  void handle_client_dir_quota_set(MDRequestRef& mdr);
};
