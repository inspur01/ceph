...
void CInode::remove_client_cap(client_t client)
{
  ...
  // adjust dir cache iterator
  CDir* dir = get_parent_dir();
  if(dir != NULL){
    dir->update_client_complete_pos(client, ino().val);
  }
}
			     ...
			     int getattr_caps)
{
  ...
  /*oi存在client_ranges，则保持原有逻辑，取oi和pi最小者，否则采用pi*/
  if (oi->client_ranges.count(client)){
  // max_size is min of projected, actual
      ...
      e.max_size = (pi->client_ranges.count(client) ? pi->client_ranges[client].range.last : 0);
  }
  dout(10) << "inode = " << e.ino << " oi= " << oi->client_ranges.count(client) << " pi= " << pi->client_ranges.count(client) <<" max_size " << e.max_size << dendl;
