...
MDCache::~MDCache() 
{
  ...
  delete user_quota;
  delete user_file_quota;
  
  delete grp_quota;
  delete grp_file_quota;
  delete grp_used_quota;
  delete dir_quota;
}
...
void MDCache::dump_cache_inoidx(const char *fn)
{
  int r;
  char deffn[200];
  if (!fn) {
    snprintf(deffn, sizeof(deffn), "cachebyinodedump.%d.mds%d", (int)mds->mdsmap->get_epoch(), mds->get_nodeid());
    fn = deffn;
  }

  dout(1) << "dump_cache_inoidx to " << fn << dendl;
  ...
  for (ceph::unordered_map<vinodeno_t,CInode*>::iterator it = inode_map.begin();
    ...
    for (list<CDir*>::iterator p = dfs.begin(); p != dfs.end(); ++p) {
      ...
      dir->dump_dentries_inoidx(fd);
      dir->check_rstats();
    }
  }

  for(xlist<CDir*>::iterator p = complete_readahead_dirs.begin();!p.end();++p){
    CDir* dir = *p; 
    ostringstream ff;
    ff << "readahead complete dir name: " << dir->get_inode()->get_parent_dn()->name << std::endl;
    string t = ff.str();
    r = safe_write(fd, t.c_str(), t.length());
    if (r < 0)
      goto out;
...
void MDCache::del_complete_readahead_dir(client_t client)
{
  dout(10) << "readahead del_complete_readahead_dir enter "  << dendl;
  xlist<CDir*>::iterator next;
  for(xlist<CDir*>::iterator p = complete_readahead_dirs.begin();!p.end();){
    CDir* dir = *p; 
    
    dout(10) << "readahead dir name: " << dir->get_inode()->get_parent_dn()->name
        << "  client: " << client << dendl;
    
    next = ++p;
    dir->uninit_cache_for_client(client);
    p = next;
    
    //if(complete_readahead_dirs.size()==0){
    //  dout(10) << "readahead del_complete_readahead_dir break "  << dendl;
    //  break;
    //}
  }
}

void MDCache::add_complete_readahead_dir(CDir* dir)
{
  if(dir != NULL && !dir->item_complete_readahead.is_on_list()){
    complete_readahead_dirs.push_back(&dir->item_complete_readahead);
  }
...
// Add begin by lvq
class C_MDS_User_QuotaFinished : public MDSInternalContext {
public:
  C_MDS_User_QuotaFinished(MDS *m) : MDSInternalContext(m) {}
  void finish(int r) { 
    //dout(0) << "C_MDS_User_QuotaFinished version : " << version << dendl;
  }
};

void MDCache::fetch_quota()
{
  fetch_user_quota();
  fetch_grp_quota();
  fetch_file_quota();
  fetch_grp_used_quota();
}

void MDCache::fetch_user_quota()
{
  dout(10) << "fetch_quota " << dendl;
  C_MDS_User_QuotaFinished *context = new C_MDS_User_QuotaFinished(mds);
  if (!user_quota) {
    dout(10) << "fetch_quota fetch quota" << dendl;
    user_quota = new UserQuotaMgr(this, mds);
    user_quota->fetch(context);
  }
  else {
    user_quota->fetch(context);
  }
...
class C_MDS_Grp_QuotaFinished : public MDSInternalContext {
public:
  C_MDS_Grp_QuotaFinished(MDS *m) : MDSInternalContext(m) {}
  void finish(int r) { 
    //dout(0) << "C_MDS_User_QuotaFinished version : " << version << dendl;
  }
};

void MDCache::fetch_grp_quota()
{
  dout(10) << "fetch_quota " << dendl;
  C_MDS_Grp_QuotaFinished *context = new C_MDS_Grp_QuotaFinished(mds);
  if (!grp_quota) {
    dout(10) << "fetch_quota fetch quota" << dendl;
    grp_quota = new GrpQuotaMgr(this, mds);
    grp_quota->fetch(context);
  }
  else {
    grp_quota->fetch(context);
  }
...
class C_MDS_User_File_QuotaFinished : public MDSInternalContext {
public:
  C_MDS_User_File_QuotaFinished(MDS *m) : MDSInternalContext(m) {}
  void finish(int r) { 
    //dout(0) << "C_MDS_User_File_QuotaFinished version : " << version << dendl;
  }
};

class C_MDS_Grp_File_QuotaFinished : public MDSInternalContext {
public:
  C_MDS_Grp_File_QuotaFinished(MDS *m) : MDSInternalContext(m) {}
  void finish(int r) { 
    //dout(0) << "C_MDS_User_File_QuotaFinished version : " << version << dendl;
  }
};

void MDCache::fetch_file_quota()
{
  dout(0) << "fetch_file_quota begin " << dendl;
  C_MDS_User_File_QuotaFinished *context = new C_MDS_User_File_QuotaFinished(mds);
  if (!user_file_quota) {
    dout(0) << "fetch_file_quota fetch quota first" << dendl;
    user_file_quota = new UserFileQuotaMgr(this, mds);
  }

  user_file_quota->fetch(context);
  

  C_MDS_Grp_File_QuotaFinished *grp_context = new C_MDS_Grp_File_QuotaFinished(mds);
  if (!grp_file_quota) {
    dout(0) << "grp_file_quota fetch quota first" << dendl;
    grp_file_quota = new GrpFileQuotaMgr(this, mds);
  }

  grp_file_quota->fetch(grp_context);
}

class C_MDS_Grp_Used_QuotaFinished : public MDSInternalContext {
public:
  C_MDS_Grp_Used_QuotaFinished(MDS *m) : MDSInternalContext(m) {}
  void finish(int r) { 
    //dout(0) << "C_MDS_User_File_QuotaFinished version : " << version << dendl;
  }
};

void MDCache::fetch_grp_used_quota()
{
  dout(10) << "fetch_grp_used_quota begin " << dendl;
  C_MDS_Grp_Used_QuotaFinished *context = new C_MDS_Grp_Used_QuotaFinished(mds);

  if (!grp_used_quota) {
    dout(10) << "grp_file_quota fetch quota first" << dendl;
    grp_used_quota = new GrpUsedQuotaMgr(this, mds);
  }
  
  grp_used_quota->fetch(context);
}


void MDCache::store_quota()
{
  if (grp_used_quota) {
    C_MDS_Grp_Used_QuotaFinished *context = new C_MDS_Grp_Used_QuotaFinished(mds);
    grp_used_quota->store(context);
  }

  if (user_quota) {
    C_MDS_User_QuotaFinished *context = new C_MDS_User_QuotaFinished(mds);
    user_quota->store(context);
  }

  if (user_file_quota) {
    C_MDS_User_File_QuotaFinished *context = new C_MDS_User_File_QuotaFinished(mds);
    user_file_quota->store(context);
  }

  if (grp_quota) {
    C_MDS_Grp_QuotaFinished *context = new C_MDS_Grp_QuotaFinished(mds);
    grp_quota->store(context);
  }

  if (grp_file_quota) {
    C_MDS_Grp_File_QuotaFinished *context = new C_MDS_Grp_File_QuotaFinished(mds);
    grp_file_quota->store(context);
  }  
}
// Add end by lvq

class C_MDS_Dir_QuotaFinished : public MDSInternalContext {
public:
  C_MDS_Dir_QuotaFinished(MDS *m) : MDSInternalContext(m) {}
  void finish(int r) { 
    //dout(0) << "C_MDS_Dir_QuotaFinished version : " << version << dendl;
  }
};

void MDCache::fetch_dir_quota()
{
  dout(10) << "fetch_dir_quota " << dendl;
  C_MDS_Dir_QuotaFinished *context = new C_MDS_Dir_QuotaFinished(mds);
  if (!dir_quota) {
    dout(10) << "fetch_quota fetch quota" << dendl;
    dir_quota = new DirQuota(this, mds);
    dir_quota->fetch(context);
  } else {
    dir_quota->fetch(context);
  }
}

void MDCache::store_dir_quota()
{
  if (dir_quota) {
    C_MDS_Dir_QuotaFinished *context = new C_MDS_Dir_QuotaFinished(mds);
    dir_quota->store(context);
  }
}