...
void EUserQuota::encode(bufferlist &bl) const
{
  ENCODE_START(4, 4, bl);
  ::encode(stamp, bl);
  ::encode(cmapv, bl);
  ...
  if (USER_QUOTA_UPDATE == op_type) {
    ::encode(update_quota_map, bl);
  }
  else if (USER_QUOTA_MOD == op_type) {
    ::encode(old_hardlimit, bl);
    ::encode(user_quota_data, bl);
  }
  else {
    ::encode(user_quota_data, bl);
  }
...
void EUserQuota::decode(bufferlist::iterator &bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(4, 4, 4, bl);
  ::decode(stamp, bl);
  ::decode(cmapv, bl);
  ...
  if (USER_QUOTA_UPDATE == op_type) {
    ::decode(update_quota_map, bl);
  }
  else if (USER_QUOTA_MOD == op_type) {
    ::decode(old_hardlimit, bl);
    ::decode(user_quota_data, bl);
  }
  else {
    ::decode(user_quota_data, bl);
  }
...
void EUserQuota::dump(Formatter *f) const
{
  //f->dump_string("type", type);
  //f->dump_int("client map length", client_map.length());
  //f->dump_int("client map version", cmapv);
...
void EUserQuota::generate_test_instances(list<EUserQuota*>& ls)
{
  ls.push_back(new EUserQuota());
}

void EUserQuota::user_quota_replay(MDS *mds)
{
  uint64_t version = mds->mdcache->user_quota->get_version();
  dout(10)  << " EUserQuota::user_quota_replay version: " << version << "  ,cmapv : " << cmapv << dendl;
  if ((version > cmapv) 
      || (0 == version && mds->mdcache->user_quota->is_noused())) {
    dout(10)  << " EUserQuota::user_quota_replay journal is old, don't process version: " << version << "  ,cmapv : " << cmapv << dendl;
    return;
  }

  dout(10)  << " EUserQuota::user_quota_replay ok, op_type " << op_type  << dendl;

  if (USER_QUOTA_SET == op_type) {
    mds->mdcache->user_quota->set_user_quota(user_quota_data.uid, user_quota_data);
  }
  else if (USER_QUOTA_MOD == op_type) {
    mds->mdcache->user_quota->modify_user_quota(user_quota_data.uid, user_quota_data);
  }
  else if (USER_QUOTA_UPDATE == op_type) {
    mds->mdcache->user_quota->update_user_quota(update_quota_map);
  }
  else if (USER_QUOTA_DELETE == op_type) {
    mds->mdcache->user_quota->del_user_quota(user_quota_data.uid);
  }
  else if (USER_QUOTA_CLEAR == op_type) {
    mds->mdcache->user_quota->clear_user_quota();
  }
}   

void EUserQuota::grp_used_quota_replay(MDS *mds)
{
  uint64_t version = mds->mdcache->grp_used_quota->get_version();
  dout(10)  << " EUserQuota::grp_used_quota_replay version: " << version << "  ,cmapv : " << cmapv << dendl;
  if ((version > cmapv) 
      || (0 == version && mds->mdcache->grp_used_quota->is_noused())) {
    dout(10)  << " EUserQuota::grp_used_quota_replay journal is old, don't process version: " << version << "  ,cmapv : " << cmapv << dendl;
    return;
  }

  dout(10)  << " EUserQuota::grp_used_quota_replay ok, op_type " << op_type  << dendl;

  if (USER_QUOTA_SET == op_type) {
    mds->mdcache->grp_used_quota->add(user_quota_data.gid, user_quota_data.hardlimit);
  }
  else if (USER_QUOTA_MOD == op_type) {
    mds->mdcache->grp_used_quota->modify(user_quota_data.gid, user_quota_data.hardlimit, old_hardlimit);
  }
  else if (USER_QUOTA_DELETE == op_type) {
    mds->mdcache->grp_used_quota->sub(user_quota_data.gid, user_quota_data.hardlimit);
  }
  else if (USER_QUOTA_CLEAR == op_type) {
    mds->mdcache->grp_used_quota->clear();
  }
}  


void EUserQuota::replay(MDS *mds)
{
  user_quota_replay(mds);
  grp_used_quota_replay(mds);
}   

// EUserFileQuota

void EUserFileQuota::encode(bufferlist &bl) const
{
  ENCODE_START(4, 4, bl);
  ::encode(stamp, bl);
  ::encode(cmapv, bl);
  ::encode(type, bl);
  ::encode(uid, bl);
  ::encode(update_file_quota_map, bl);
  ENCODE_FINISH(bl);
}
 
void EUserFileQuota::decode(bufferlist::iterator &bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(4, 4, 4, bl);
  ::decode(stamp, bl);
  ::decode(cmapv, bl);
  ::decode(type, bl);
  ::decode(uid, bl);
  ::decode(update_file_quota_map, bl);
  DECODE_FINISH(bl);
}

void EUserFileQuota::dump(Formatter *f) const
{
  //f->dump_string("type", type);
  //f->dump_int("client map length", client_map.length());
  //f->dump_int("client map version", cmapv);
...
void EUserFileQuota::generate_test_instances(list<EUserFileQuota*>& ls)
{
  ls.push_back(new EUserFileQuota());
}

void EUserFileQuota::replay(MDS *mds)
{
  uint64_t version = mds->mdcache->user_file_quota->get_version();
  if ((version > cmapv) 
      || (0 == version && mds->mdcache->user_file_quota->is_noused())) {
    dout(10)  << " EUserFileQuota::replay journal is old, don't process " << dendl;
    return;
  }

  dout(10) << __func__ << " EUserFileQuota::replay cmapv: " << cmapv << " user_file_quota->get_version: " 
  << mds->mdcache->user_file_quota->get_version() << dendl;

  map<uid_t, UserFileQuota>::iterator it;
  for (it = update_file_quota_map.begin();
      it != update_file_quota_map.end();
      ++it) {
    dout(10) << __func__ << " EUserFileQuota::replay userid: " << it->first << dendl;
    mds->mdcache->user_file_quota->update_user_file_quota(it->first, it->second.vary_cnt);        
  }
}   

// EGrpQuota

void EGrpQuota::encode(bufferlist &bl) const
{
  ENCODE_START(4, 4, bl);
  ::encode(stamp, bl);
  ::encode(cmapv, bl);
  ...
  if (USER_QUOTA_UPDATE == op_type) {
    ...
    ::encode(grp_quota_data, bl);
  }
  
  ENCODE_FINISH(bl);
}
 
void EGrpQuota::decode(bufferlist::iterator &bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(4, 4, 4, bl);
  ::decode(stamp, bl);
  ::decode(cmapv, bl);
  ...
  if (USER_QUOTA_UPDATE == op_type) {
    ...
    ::decode(grp_quota_data, bl);
  }
  
  DECODE_FINISH(bl);
}

void EGrpQuota::dump(Formatter *f) const
{
  //f->dump_string("type", type);
  //f->dump_int("client map length", client_map.length());
  //f->dump_int("client map version", cmapv);
...
void EGrpQuota::generate_test_instances(list<EGrpQuota*>& ls)
{
  ls.push_back(new EGrpQuota());
}

void EGrpQuota::replay(MDS *mds)
{
  uint64_t version = mds->mdcache->grp_quota->get_version();
  dout(10)  << " EGrpQuota::replay version: " << version << "  ,cmapv : " << cmapv << dendl;
  if ((version > cmapv) 
      || (0 == version && mds->mdcache->grp_quota->is_noused())) {
    dout(10)  << " EGrpQuota::replay journal is old, don't process version: " << version << "  ,cmapv : " << cmapv << dendl;
    return;
  }

  dout(10)  << " EGrpQuota::replay ok, op_type " << op_type  << dendl;

  if (USER_QUOTA_SET == op_type) {
    mds->mdcache->grp_quota->set_grp_quota(grp_quota_data.gid, grp_quota_data);
  }
  else if (USER_QUOTA_MOD == op_type) {
    mds->mdcache->grp_quota->modify_grp_quota(grp_quota_data.gid, grp_quota_data);
  }
  else if (USER_QUOTA_UPDATE == op_type) {
    mds->mdcache->grp_quota->update_grp_quota(update_quota_map);
  }
  else if (USER_QUOTA_DELETE == op_type) {
    mds->mdcache->grp_quota->del_grp_quota(grp_quota_data.gid);
  }
  else if (USER_QUOTA_CLEAR == op_type) {
    mds->mdcache->grp_quota->clear_grp_quota();
  }
}   

// EGrpFileQuota

void EGrpFileQuota::encode(bufferlist &bl) const
{
  ENCODE_START(4, 4, bl);
  ::encode(stamp, bl);
  ::encode(cmapv, bl);
...
void EGrpFileQuota::decode(bufferlist::iterator &bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(4, 4, 4, bl);
  ::decode(stamp, bl);
  ::decode(cmapv, bl);
...
void EGrpFileQuota::dump(Formatter *f) const
{
  //f->dump_string("type", type);
  //f->dump_int("client map length", client_map.length());
  //f->dump_int("client map version", cmapv);
...
void EGrpFileQuota::generate_test_instances(list<EGrpFileQuota*>& ls)
{
  ls.push_back(new EGrpFileQuota());
}

void EGrpFileQuota::replay(MDS *mds)
{
  uint64_t version = mds->mdcache->grp_file_quota->get_version();
  if ((version > cmapv) 
      || (0 == version && mds->mdcache->grp_file_quota->is_noused())) {
    dout(10)  << " EUserFileQuota::replay journal is old, don't process " << dendl;
    return;
  }

  dout(10) << __func__ << " EUserFileQuota::replay cmapv: " << cmapv << " user_file_quota->get_version: " 
  << mds->mdcache->grp_file_quota->get_version() << dendl;

  map<uid_t, GrpFileQuota>::iterator it;
  for (it = update_file_quota_map.begin();
      it != update_file_quota_map.end();
      ++it) {
    dout(10) << __func__ << " EUserFileQuota::replay userid: " << it->first << dendl;
    mds->mdcache->grp_file_quota->update_grp_file_quota(it->first, it->second.vary_cnt);        
  }
}   

// EDirQuota
void EDirQuota::encode(bufferlist &bl) const
{
  ENCODE_START(4, 4, bl);
  ::encode(stamp, bl);
  ::encode(cmapv, bl);
  ::encode(type, bl);
  ::encode(op_type, bl);
  ::encode(dir_quota_data, bl);
  ::encode(update_quota_map, bl);
  ENCODE_FINISH(bl);
}

void EDirQuota::decode(bufferlist::iterator &bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(4, 4, 4, bl);
  ::decode(stamp, bl);
  ::decode(cmapv, bl);
  ::decode(type, bl);
  ::decode(op_type, bl);
  ::decode(dir_quota_data, bl);
  ::decode(update_quota_map, bl);
  DECODE_FINISH(bl);
}

void EDirQuota::dump(Formatter *f) const
{
  //f->dump_string("type", type);
  //f->dump_int("client map length", client_map.length());
  //f->dump_int("client map version", cmapv);
...
void EDirQuota::generate_test_instances(list<EDirQuota*>& ls)
{
  ls.push_back(new EDirQuota());
}

void EDirQuota::replay(MDS *mds)
{
  uint64_t version = mds->mdcache->dir_quota->get_version();
  dout(10)  << " EDirQuota::replay version: " << version << "  ,cmapv : " << cmapv << dendl;
  if ((version > cmapv) 
      || (0 == version && mds->mdcache->dir_quota->is_noused())) {
    dout(10)  << " EDirQuota::replay journal is old, don't process version: " << version << "  ,cmapv : " << cmapv << dendl;
    return;
  }

  dout(10)  << " EDirQuota::replay ok, op_type " << op_type  << dendl;

  if (DIR_QUOTA_SET == op_type) {
    mds->mdcache->dir_quota->set_dir_quota(dir_quota_data.ino, dir_quota_data);
  } else if (DIR_QUOTA_UPDATE == op_type) {
    mds->mdcache->dir_quota->update_dir_quota(update_quota_map);
  } else if (DIR_QUOTA_DEL == op_type) {
    mds->mdcache->dir_quota->del_dir_quota(dir_quota_data.ino);
  }
}   
