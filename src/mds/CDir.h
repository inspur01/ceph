...
class CDir : public MDSCacheObject {
   ...
   */
public: 
  xlist<CDir*>::item item_complete_readahead;
private:
...
public:
  ...
  //  cache by inoidx
  std::map<_inodeno_t, CDentry*> dentries_inoidx;
  bool add_dentry_inoidx(CDentry* dn);
  bool remove_dentry_inoidx(CDentry* dn);
  int dump_dentries_inoidx(int fd);
  

  // complete readahead
  std::map<client_t, std::map<_inodeno_t, CDentry*>::iterator> client_complete_pos;
  int dump_client_complete_pos(int fd);
  bool is_first_access_for_client(client_t client);
  int init_cache_for_client(client_t client);
  int uninit_cache_for_client(client_t client);
  int update_client_complete_pos(client_t client, _inodeno_t ino);
