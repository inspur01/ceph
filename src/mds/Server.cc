...
#include "events/EUserQuota.h"
#include "events/EUserFileQuota.h"
#include "events/EGrpQuota.h"
#include "events/EGrpFileQuota.h"
#include "events/EDirQuota.h"
			     ...
			     interval_set<inodeno_t>& inos, version_t piv)
{
  dout(10) << "_session_logged " << session->info.inst << " state_seq " << state_seq << " " << (open ? "open":"close")
	   << " " << pv << dendl;
  client_t this_client = session->get_client();
  if (piv) {
  ...
  if(!open){
    mdcache->del_complete_readahead_dir(this_client);
  }
}
...
/*设置配额及时生效handle_set_vattr()调用*/
void Server::journal_and_reply(MDRequestRef& mdr, CInode *in, CDentry *dn, LogEvent *le, MDSInternalContextBase *fin, bool early_reply_mds)
{
  dout(10) << "journal_and_reply tracei " << in << " tracedn " << dn << dendl;
...
void Server::dispatch_client_request(MDRequestRef& mdr)
{
  ...
  // user quota
  case CEPH_MDS_OP_QUOTA_GET_ALL:
    handle_client_quota_get_all(mdr);
    break;
  case CEPH_MDS_OP_QUOTA_SET:
    handle_client_quota_set(mdr);
    break;
  case CEPH_MDS_OP_QUOTA_MODIFY:
    handle_client_quota_modify(mdr);
    break;	
  case CEPH_MDS_OP_QUOTA_GET:
    handle_client_quota_get(mdr);
    break;
  case CEPH_MDS_OP_QUOTA_DEL:
    handle_client_quota_delete(mdr);
    break;
  case CEPH_MDS_OP_QUOTA_CLEAR:
    handle_client_quota_clear(mdr);
    break;
  case CEPH_MDS_OP_QUOTA_USED_UPDATE:
    handle_client_quota_update(mdr);
    break;
  // file quota
  case CEPH_MDS_OP_QUOTA_FILE_UPDATE:
    handle_client_quota_update_file(mdr);
    break;
  case CEPH_MDS_OP_QUOTA_FILE_GET_ALL:
    handle_client_quota_file_get_all(mdr);
    break;
  // grp quota
  case CEPH_MDS_OP_GRP_QUOTA_SET:
    handle_client_grp_quota_set(mdr);
    break;
  case CEPH_MDS_OP_GRP_QUOTA_MODIFY:
    handle_client_grp_quota_modify(mdr);
    break;	
  case CEPH_MDS_OP_GRP_QUOTA_DEL:
    handle_client_grp_quota_delete(mdr);
    break;
  case CEPH_MDS_OP_GRP_QUOTA_CLEAR:
    handle_client_grp_quota_clear(mdr);
    break;
  case CEPH_MDS_OP_GRP_QUOTA_USED_UPDATE:
    handle_client_grp_quota_update(mdr);
    break;
  // file quota
  case CEPH_MDS_OP_GRP_FILE_QUOTA_UPDATE:
    handle_client_grp_file_quota_update(mdr);
    break;    
  case CEPH_MDS_OP_QUOTA_CHECK:
    handle_client_quota_check(mdr);
    break;
  // dir quota
  case CEPH_MDS_OP_DIR_QUOTA_USED_UPDATE:
    handle_client_dir_quota_update(mdr);
    break; 
  case CEPH_MDS_OP_DIR_QUOTA_GET_ALL:
    handle_client_dir_quota_get_all(mdr);
    break;
  case CEPH_MDS_OP_DIR_QUOTA_GET:
    handle_client_dir_quota_get(mdr);
    break;
  case CEPH_MDS_OP_DIR_QUOTA_DEL:
    handle_client_dir_quota_del(mdr);
    break;
  case CEPH_MDS_OP_DIR_QUOTA_SET:
    handle_client_dir_quota_set(mdr);
    break;
	
  case CEPH_MDS_OP_READAHEAD:
    handle_client_readahead(mdr);
    break;
  case CEPH_MDS_OP_READAHEAD_DIR_COMPLETE:
    handle_client_readahead_dir_complete(mdr);	
    break;
...
{
  dout(10) << "parse_quota_vxattr name " << name << " value '" << value << "'" << dendl;
  try {
    if (name == "quota") {
    ...
    } else if (name == "quota.max_Gbytes") {
      quota->max_Gbytes = boost::lexical_cast<unsigned>(value);
				...
				ceph_file_layout *layout)
{
  ...
  try {
    ...
    } else if (name == "layout.losf_write") {
      /* 如果配置项没打开，则不支持设置目录属性fl_unused;
         可设置时，只能设置为0,1,2三个，其他的返回错误*/
      if(g_conf->client_losf_write == CEPH_NO_TICK_NO_FLUSH_INODE){
        dout(10) << "layout.unused is not open,setting is refused"<< dendl;
        return -ENOENT;
      }
      if(boost::lexical_cast<unsigned>(value) != CEPH_NO_TICK_NO_FLUSH_INODE
        && boost::lexical_cast<unsigned>(value) != CEPH_NO_TICK_FLUSH_INODE
        && boost::lexical_cast<unsigned>(value) != CEPH_TICK_FLUSH_INODE){
        dout(3) << "layout.unused's value range is (0,1,2)"<< dendl;
        return -ENOENT;
      }
      layout->fl_unused = boost::lexical_cast<unsigned>(value);
			       ...
			       set<SimpleLock*> xlocks)
{
  ...
  if (name.find("ceph.file.layout") == 0 ||
    ...
    journal_and_reply(mdr, cur, 0, le, new C_MDS_inode_update_finish(mds, mdr, cur), false);
    return;
  }
...
void Server::handle_client_mkdir(MDRequestRef& mdr)
{
  ...
  /*
    客户端写文件，如果每次写打算只刷当前inode的bufferhead，
    则启用目录flie_layout的继承，
    因为是以目录的flie_layout.fl_unused字段为开关的。
  */
  if( g_conf->client_losf_write == CEPH_TICK_FLUSH_INODE
    || g_conf->client_losf_write == CEPH_NO_TICK_FLUSH_INODE )
  {
    ceph_file_layout layout;

    if (dn->get_dir()->inode->get_projected_inode()->has_layout())
      layout = dn->get_dir()->inode->get_projected_inode()->layout;
    else
      layout = mds->mdcache->default_file_layout;

    if (!ceph_file_layout_is_valid(&layout)) {
      dout(10) << " invalid initial dir layout" << dendl;
      reply_request(mdr, -EINVAL);
      return;
    }
    newi->inode.layout = layout;
  }
...
void Server::handle_client_quota_get_all(MDRequestRef& mdr)
{  
  assert(mdcache->user_quota);
  assert(mdcache->grp_quota);
  
  MClientReply *reply = new MClientReply(mdr->client_request);
  
  bufferlist bl;
  ::encode(mdcache->user_quota->user_quota_map, bl);
  ::encode(mdcache->grp_quota->grp_quota_map, bl);
...
bool Server::get_user_quota(unsigned int id, bufferlist &bufferlist)
{
  assert(mdcache->user_quota);
  
  UserQuotaData *user_quota_data = mdcache->user_quota->get_user_quota(id);
  if (!user_quota_data) {
  	dout(0) << " user quota info does not exist, uid: " << id << dendl;
  	return false;
  }

  ::encode(*user_quota_data, bufferlist);
  return true;
}

bool Server::get_grp_quota(unsigned int id, bufferlist &bufferlist)
{
  assert(mdcache->user_quota);
  
  GrpQuotaData *grp_quota_data = mdcache->grp_quota->get_grp_quota(id);
  if (!grp_quota_data) {
  	dout(0) << " grp quota info does not exist, gid: " << id << dendl;
  	return false;
  }

  ::encode(*grp_quota_data, bufferlist);
  return true;
}

void Server::handle_client_quota_get(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  unsigned int id = 0;
  char type = '0';
  ...
  if (QUOTA_TYPE_USER == type) {
    ret = get_user_quota(id, bufferlist);
  }
  else if (QUOTA_TYPE_GRP == type) {
    ret = get_grp_quota(id, bufferlist);
  }

  if (!ret) {
  	reply_request(mdr, -E_QUOTA_NOTEXIST);
  	return;
  }
  
  MClientReply *reply = new MClientReply(mdr->client_request);
  
  reply->set_extra_bl(bufferlist);
  
  reply_request(mdr, reply);
}

void Server::broadcast_user_quota(MDRequestRef& mdr, UserQuotaData &uqdata, uint64_t op) 
{ 
    const ceph::unordered_map<entity_name_t, Session*> session_map = mds->sessionmap.get_sessions();
    for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
          it != session_map.end();
          ++it) {
      // only broadcast to client
      if (it->first.type() != CEPH_ENTITY_TYPE_CLIENT) {
          continue;
      }
    
      MClientUserQuota *msg = new MClientUserQuota(op);
      msg->user_quota_info = uqdata;
      mds->send_message_client_counted(msg, it->second);
    }
...
// Add by lvq
class C_MDS_User_Quota_SetFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  UserQuotaMgr *user_quota;
  UserQuotaData uqdata;
  
  C_MDS_User_Quota_SetFinish(MDS *m, MDRequestRef& r, UserQuotaMgr *u, UserQuotaData &qdata) : 
    MDSInternalContext(m), mdr(r), user_quota(u), uqdata(qdata) {}
    
  void finish(int r) { 
    user_quota->set_user_quota(uqdata.uid, uqdata);
    user_quota->mdcache->grp_used_quota->add(uqdata.gid, uqdata.hardlimit);
    user_quota->inc_version();
    //broadcast to all clients
    user_quota->mds->server->broadcast_user_quota(mdr, uqdata, CEPH_MSG_CLIENT_USER_QUOTA_SET);    
    
  }
};

bool Server::is_exceed_grp_left_quota(UserQuotaData &qdata, uint64_t old_usr_hardlimit/*=0*/)
{
  GrpQuotaMgr *grp_quota   = mdcache->grp_quota;
  
  assert(grp_quota);
  assert(mdcache->grp_used_quota);

  // user's hardlimit is exceed group's left hardlimit
  GrpQuotaData *p_grp_qdata =  grp_quota->get_grp_quota(qdata.gid);
  if (p_grp_qdata) {
    uint64_t grp_used_quota = mdcache->grp_used_quota->get_grp_used_quota(qdata.gid);
    grp_used_quota -= old_usr_hardlimit;
    
    if (qdata.hardlimit > (p_grp_qdata->hardlimit - grp_used_quota))
    {
      return true;
    }
  }
...
void Server::handle_client_quota_set(MDRequestRef& mdr)
{
  MClientRequest *req          = mdr->client_request;
  UserQuotaMgr *user_quota     = mdcache->user_quota;
  UserFileQuotaMgr *ufilequota = mdcache->user_file_quota;
  UserQuotaData qdata;
  ...
  if (user_quota->have_user_quota(qdata.uid)) {
  	reply_request(mdr, -E_QUOTA_EXIST);
  	return;
  }

  if (ufilequota->user_file_quota_map.count(qdata.uid) &&
      ufilequota->user_file_quota_map[qdata.uid].user_file_cnt > 0) {   
    reply_request(mdr, -E_QUOTA_HAVE_FILE);
    return;
  }

  if (is_exceed_grp_left_quota(qdata)) {
    reply_request(mdr, -E_QUOTA_USER_EXCEED_GRP);
    dout(0) << "user hardlimit is exceed grp left hardlimit , uid: " << qdata.uid << dendl;
    return;
  }

  mdcache->grp_used_quota->set_version(user_quota->get_version());
  
  C_MDS_User_Quota_SetFinish *fin = new C_MDS_User_Quota_SetFinish(mds, mdr, user_quota, qdata);
  EUserQuota *le = new EUserQuota(mdlog, "SetUserQuota");
  le->set_version(user_quota->get_version());
  le->user_quota_data = qdata;
  le->op_type = USER_QUOTA_SET;
  
  mdlog->start_entry(le);
  journal_and_reply(mdr, NULL, NULL, le, fin, false);
}

// Add by lvq
class C_MDS_User_Quota_ModFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  UserQuotaMgr *user_quota;
  UserQuotaData uqdata;
  
  C_MDS_User_Quota_ModFinish(MDS *m, MDRequestRef& r, UserQuotaMgr *u, UserQuotaData &qdata) : 
    MDSInternalContext(m), mdr(r), user_quota(u), uqdata(qdata) {}
  void finish(int r) { 
    UserQuotaData *p_old_uqdata = user_quota->get_user_quota(uqdata.uid);
    user_quota->mdcache->grp_used_quota->modify(uqdata.gid, uqdata.hardlimit, p_old_uqdata->hardlimit);
    user_quota->modify_user_quota(uqdata.uid, uqdata);
    user_quota->inc_version();
    //broadcast to all clients
    user_quota->mds->server->broadcast_user_quota(mdr, uqdata, CEPH_MSG_CLIENT_USER_QUOTA_SET);    
...
void Server::handle_client_quota_modify(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  UserQuotaMgr *user_quota = mdcache->user_quota;
  UserQuotaData qdata;
  ...
  UserQuotaData *p_uqdata = user_quota->get_user_quota(qdata.uid);
  if (!p_uqdata) {
  	reply_request(mdr, -E_QUOTA_NOTEXIST);
  	return;
  }  

  if (is_exceed_grp_left_quota(qdata, p_uqdata->hardlimit)) {
    reply_request(mdr, -E_QUOTA_USER_EXCEED_GRP);
    dout(0) << "user hardlimit is exceed grp left hardlimit , uid: " << qdata.uid << dendl;
    return;
  }

  C_MDS_User_Quota_ModFinish *fin = new C_MDS_User_Quota_ModFinish(mds, mdr, user_quota, qdata);
  
  EUserQuota *le = new EUserQuota(mdlog, "ModUserQuota");
  le->set_version(user_quota->get_version());
  le->user_quota_data = qdata;
  le->old_hardlimit = p_uqdata->hardlimit;
  le->op_type = USER_QUOTA_MOD;
...
// Add by lvq
class C_MDS_User_Quota_DelFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  UserQuotaMgr *user_quota;
  UserQuotaData uqdata;
  
  C_MDS_User_Quota_DelFinish(MDS *m, MDRequestRef& r, UserQuotaMgr *u, UserQuotaData &qdata) : 
    MDSInternalContext(m), mdr(r), user_quota(u), uqdata(qdata) {}
  void finish(int r) { 
    user_quota->del_user_quota(uqdata.uid);
    user_quota->mdcache->grp_used_quota->sub(uqdata.gid, uqdata.hardlimit);
    user_quota->inc_version();
    // broadcast to all clients
    user_quota->mds->server->broadcast_user_quota(mdr, uqdata, CEPH_MSG_CLIENT_USER_QUOTA_DEL);    

  }
};

void Server::handle_client_quota_delete(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  UserQuotaMgr *user_quota = mdcache->user_quota;
  UserQuotaData qdata;
  ...
  dout(0) << "Server::handle_client_quota_delete " << dendl;
  
  UserQuotaData *puser_quota_data = user_quota->get_user_quota(qdata.uid);
  if (!puser_quota_data) {
  	dout(10) << "quota info does not exist, uid: " << qdata.uid << dendl;
  	reply_request(mdr, -E_QUOTA_NOTEXIST);
  	return;
  }
  qdata = *puser_quota_data;

  mdcache->grp_used_quota->set_version(user_quota->get_version());
  
  C_MDS_User_Quota_DelFinish *fin = new C_MDS_User_Quota_DelFinish(mds, mdr, user_quota, qdata);
  
  EUserQuota *le = new EUserQuota(mdlog, "DelUserQuota");
  le->user_quota_data = *puser_quota_data;
  le->op_type = USER_QUOTA_DELETE;
  le->set_version(user_quota->get_version());
...
// Add by lvq
class C_MDS_User_Quota_ClearFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  UserQuotaMgr *user_quota;
  
  C_MDS_User_Quota_ClearFinish(MDS *m, MDRequestRef& r, UserQuotaMgr *u) : MDSInternalContext(m), mdr(r), user_quota(u) {}
  void finish(int r) { 
    user_quota->clear_user_quota();
    user_quota->mdcache->grp_used_quota->clear();
    user_quota->inc_version();
    //broadcast to all clients
    const ceph::unordered_map<entity_name_t, Session*> session_map = mds->sessionmap.get_sessions();
    for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
      ...
      MClientUserQuota *msg = new MClientUserQuota(CEPH_MSG_CLIENT_USER_QUOTA_CLEAR);
      mds->send_message_client_counted(msg, it->second);
    }
...
void Server::handle_client_quota_clear(MDRequestRef& mdr)
{
  UserQuotaMgr *user_quota = mdcache->user_quota;
  
  if (user_quota->get_user_quota_size() <= 0) {
    reply_request(mdr, 0);
    return;
  }
  ...
  // callback context
  C_MDS_User_Quota_ClearFinish *fin = new C_MDS_User_Quota_ClearFinish(mds, mdr, user_quota);

  // user quota clear journal event
  EUserQuota *le = new EUserQuota(mdlog, "ClearUserQuota");
  le->set_version(user_quota->get_version());
  le->op_type = USER_QUOTA_CLEAR;
  mdlog->start_entry(le);
  journal_and_reply(mdr, NULL, NULL, le, fin, false);	
}

// Add by lvq
class C_MDS_User_Quota_UpdateFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  map<uid_t, UserQuotaData> update_quota_map;
  UserQuotaMgr *user_quota;
  
  C_MDS_User_Quota_UpdateFinish(MDS *m, MDRequestRef& r, UserQuotaMgr *u) : MDSInternalContext(m), mdr(r), user_quota(u) {}
  void finish(int r) { 
    for (map<uid_t, UserQuotaData>::iterator it = update_quota_map.begin();
        it != update_quota_map.end();
        ++it) {
      if (user_quota->user_quota_map.count(it->first)) {
        user_quota->update_user_quota(it->first,it->second);
        it->second.used_bytes = user_quota->user_quota_map[it->first].used_bytes;
      } 
      else {
        dout(0) << "quota info does not exist, uid: " << it->first << dendl;
        update_quota_map.erase(it->first);
      }
    }
    user_quota->inc_version();
    const ceph::unordered_map<entity_name_t, Session*> session_map = mds->sessionmap.get_sessions();
    for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
      ...
      MClientUserQuota *msg = new MClientUserQuota(CEPH_MSG_CLIENT_USER_QUOTA_UPDATE);
      msg->user_quota_map = update_quota_map;
      mds->send_message_client_counted(msg, it->second);
    }
...
void Server::handle_client_quota_update(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  
  assert(mdcache->user_quota);

  C_MDS_User_Quota_UpdateFinish *fin = new C_MDS_User_Quota_UpdateFinish(mds, mdr, mdcache->user_quota);

  
  bufferlist bl = req->get_data();
  bufferlist::iterator it = bl.begin();
  ::decode(fin->update_quota_map, it);
  
  EUserQuota *le = new EUserQuota(mdlog, "UpdateUserQuota");
  le->op_type = USER_QUOTA_UPDATE;
  le->set_version(mdcache->user_quota->get_version());
  dout(10) << "Server::handle_client_quota_update, version: " << le->cmapv << ", mdcache->user_quota->version:" 
    << mdcache->user_quota->version << dendl;

  for (map<uid_t, UserQuotaData>::iterator it = fin->update_quota_map.begin();
      it != fin->update_quota_map.end();
      ++it) {
    if (mdcache->user_quota->user_quota_map.count(it->first)) {
      le->update_quota_map[it->first] = it->second;
    } 
    else {
      dout(0) << "quota info does not exist, uid: " << it->first << dendl;
      fin->update_quota_map.erase(it->first);
    }
  }

  mdlog->start_entry(le);
  journal_and_reply(mdr, NULL, NULL, le, fin);
}

void Server::handle_client_quota_file_get_all(MDRequestRef& mdr)
{
  UserFileQuotaMgr *ufquota = mdcache->user_file_quota;
  GrpFileQuotaMgr *gfquota = mdcache->grp_file_quota;
  ...
  bufferlist bl;
  ::encode(ufquota->user_file_quota_map, bl);
  ::encode(gfquota->grp_file_quota_map, bl);
...
// Add by lvq
class C_MDS_User_File_Quota_UpdateFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  map<uid_t, UserFileQuota> update_quota_map;
  UserFileQuotaMgr *user_file_quota;
  
  C_MDS_User_File_Quota_UpdateFinish(MDS *m, MDRequestRef& r, UserFileQuotaMgr *u) : MDSInternalContext(m), mdr(r), user_file_quota(u) {}
  void finish(int r) { 
    // update MDCache quota map
    for (map<uid_t, UserFileQuota>::iterator it = update_quota_map.begin();
          it != update_quota_map.end();
          ++it) {
      user_file_quota->update_user_file_quota(it->first, it->second.vary_cnt);    
      update_quota_map[it->first].user_file_cnt = user_file_quota->user_file_quota_map[it->first].user_file_cnt;
    }
    ...
    for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
      ...
      MClientUserFileQuota *msg = new MClientUserFileQuota();
      msg->user_quota_map = update_quota_map;
      mds->send_message_client_counted(msg, it->second);
    }
...
void Server::handle_client_quota_update_file(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  
  assert(mdcache->user_file_quota);

  C_MDS_User_File_Quota_UpdateFinish *fin = new C_MDS_User_File_Quota_UpdateFinish(mds, mdr, mdcache->user_file_quota);
  fin->user_file_quota = mdcache->user_file_quota;
  
  bufferlist bl = req->get_data();
  bufferlist::iterator it = bl.begin();
  ::decode(fin->update_quota_map, it);

  EUserFileQuota *le = new EUserFileQuota(mdlog, "UpdateFileQuota");
  le->set_version(fin->user_file_quota->get_version());
 
  for (map<uid_t, UserFileQuota>::iterator it = fin->update_quota_map.begin();
        it != fin->update_quota_map.end();
        ++it) {	
    le->update_file_quota_map[it->first] = it->second;
  }  
...
void Server::handle_client_quota_check(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  map<uid_t, uint64_t> uquota_data_map;
  map<uid_t, uint32_t> uquota_file_map;
  map<gid_t, uint64_t> gquota_data_map;
  map<gid_t, uint32_t> gquota_file_map;	

  assert(mdcache->user_file_quota && mdcache->user_quota &&
  	mdcache->grp_file_quota && mdcache->grp_quota);

  bufferlist bl = req->get_data();

  bufferlist::iterator it = bl.begin();
  ::decode(uquota_data_map, it);
  ::decode(uquota_file_map, it);
  ::decode(gquota_data_map, it);
  ::decode(gquota_file_map, it);

  // update user used bytes
  for (map<uid_t, UserQuotaData>::iterator it = mdcache->user_quota->user_quota_map.begin();
      it != mdcache->user_quota->user_quota_map.end();
      ++it) {
    if (uquota_data_map.count(it->first)) {
    	mdcache->user_quota->user_quota_map[it->first].used_bytes = uquota_data_map[it->first];
    } else {
      mdcache->user_quota->user_quota_map[it->first].used_bytes = 0;
    }
  }

  // update user file num
  for (map<uid_t, UserFileQuota>::iterator it = mdcache->user_file_quota->user_file_quota_map.begin();
      it != mdcache->user_file_quota->user_file_quota_map.end();
      ++it) {
    if (uquota_file_map.count(it->first)) {
    	mdcache->user_file_quota->user_file_quota_map[it->first].user_file_cnt = uquota_file_map[it->first];
    } else {
      mdcache->user_file_quota->user_file_quota_map[it->first].user_file_cnt = 0;
    }
  }
  
  // update group used bytes
  for (map<gid_t, GrpQuotaData>::iterator it = mdcache->grp_quota->grp_quota_map.begin();
      it != mdcache->grp_quota->grp_quota_map.end();
      ++it) {
    if (gquota_data_map.count(it->first)) {
    	mdcache->grp_quota->grp_quota_map[it->first].used_bytes = gquota_data_map[it->first];
    } else {
      mdcache->grp_quota->grp_quota_map[it->first].used_bytes = 0;
    }
  }

  // update group file num
  for (map<gid_t, GrpFileQuota>::iterator it = mdcache->grp_file_quota->grp_file_quota_map.begin();
      it != mdcache->grp_file_quota->grp_file_quota_map.end();
      ++it) {
    if (gquota_file_map.count(it->first)) {
    	mdcache->grp_file_quota->grp_file_quota_map[it->first].grp_file_cnt = gquota_file_map[it->first];
    } else {
      mdcache->grp_file_quota->grp_file_quota_map[it->first].grp_file_cnt = 0;
    }
  }

  mdcache->user_quota->set_change();
  mdcache->user_file_quota->set_change();
  mdcache->grp_quota->set_change();
  mdcache->grp_file_quota->set_change();
  mdcache->store_quota();
  ...
  for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
    ...
    // user data
    MClientUserQuota *msg_1 = new MClientUserQuota(CEPH_MSG_CLIENT_USER_QUOTA_UPDATE);
    msg_1->user_quota_map = mdcache->user_quota->user_quota_map;
    mds->send_message_client_counted(msg_1, it->second);

    // user file num
    MClientUserFileQuota *msg_2 = new MClientUserFileQuota();
    msg_2->user_quota_map = mdcache->user_file_quota->user_file_quota_map;
    mds->send_message_client_counted(msg_2, it->second);
  
    // group data
    MClientGrpQuota *msg_3 = new MClientGrpQuota(CEPH_MSG_CLIENT_GRP_QUOTA_UPDATE);
    msg_3->group_quota_map = mdcache->grp_quota->grp_quota_map;
    mds->send_message_client_counted(msg_3, it->second);

	// group file num
    MClientGrpFileQuota *msg_4 = new MClientGrpFileQuota();
    msg_4->group_file_map = mdcache->grp_file_quota->grp_file_quota_map;
    mds->send_message_client_counted(msg_4, it->second);
  }

  reply_request(mdr, 0);
}

void Server::broadcast_grp_quota(MDRequestRef& mdr, GrpQuotaData &gqdata, uint64_t op) 
{ 
  //broadcast to all clients
  const ceph::unordered_map<entity_name_t, Session*> session_map = mds->sessionmap.get_sessions();
  for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
    ...
    MClientGrpQuota *msg = new MClientGrpQuota(op);
    msg->group_quota_info = gqdata;
    mds->send_message_client_counted(msg, it->second);
  }
...
// Add by lvq
class C_MDS_Grp_Quota_SetFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  GrpQuotaMgr *grp_quota;
  GrpQuotaData gqdata;
  
  C_MDS_Grp_Quota_SetFinish(MDS *m, MDRequestRef& r, GrpQuotaMgr *u, GrpQuotaData &qdata) : 
    MDSInternalContext(m), mdr(r), grp_quota(u), gqdata(qdata) {}
    
  void finish(int r) { 
    grp_quota->set_grp_quota(gqdata.gid, gqdata);
    grp_quota->inc_version();
    //broadcast to all clients
    grp_quota->mds->server->broadcast_grp_quota(mdr, gqdata, CEPH_MSG_CLIENT_GRP_QUOTA_SET);    
   
  }
};

bool Server::is_below_grp_used_quota(uint64_t grp_hardlimit, gid_t gid)
{  
  assert(mdcache->grp_used_quota);

  // group's hardlimit is exceed alluser's used hardlimit
  uint64_t grp_used_quota = mdcache->grp_used_quota->get_grp_used_quota(gid);
  if (grp_hardlimit < grp_used_quota) {
    return true;
  }
...
//create a new quota info
void Server::handle_client_grp_quota_set(MDRequestRef& mdr)
{
  MClientRequest *req          = mdr->client_request;
  GrpQuotaMgr *grp_quota      = mdcache->grp_quota;
  GrpFileQuotaMgr *gfilequota = mdcache->grp_file_quota;
  GrpQuotaData qdata;
  ...
  assert(grp_quota);
  assert(gfilequota);
  if (grp_quota->have_grp_quota(qdata.gid)) {
  	reply_request(mdr, -E_QUOTA_EXIST);
  	dout(0) << "have grp quota, can't set quota gid: " << qdata.gid << dendl;
  	return;
  }

  if (gfilequota->grp_file_quota_map.count(qdata.gid) &&
      gfilequota->grp_file_quota_map[qdata.gid].grp_file_cnt > 0) {   
    reply_request(mdr, -E_QUOTA_HAVE_FILE);
    dout(0) << "grp has files, can't set quota gid: " << qdata.gid << dendl;
    return;
  }

  if (is_below_grp_used_quota(qdata.hardlimit, qdata.gid)) {
    reply_request(mdr, -E_QUOTA_GRP_BELOW_USERS);
    dout(0) << "grp's hardlimit can't below it's alluser's hardlimit, gid: " << qdata.gid << dendl;
    return;
  }

  C_MDS_Grp_Quota_SetFinish *fin = new C_MDS_Grp_Quota_SetFinish(mds, mdr, grp_quota, qdata);
  EGrpQuota *le = new EGrpQuota(mdlog, "SetGrpQuota");
  le->set_version(grp_quota->get_version());
  le->grp_quota_data = qdata;
  le->op_type = USER_QUOTA_SET;
...
// Add by lvq
class C_MDS_Grp_Quota_ModFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  GrpQuotaMgr *grp_quota;
  GrpQuotaData gqdata;
  
  C_MDS_Grp_Quota_ModFinish(MDS *m, MDRequestRef& r, GrpQuotaMgr *u, GrpQuotaData &qdata) : 
    MDSInternalContext(m), mdr(r), grp_quota(u), gqdata(qdata) {}
  void finish(int r) { 
    grp_quota->modify_grp_quota(gqdata.gid, gqdata);
    grp_quota->inc_version();
    //broadcast to all clients
    grp_quota->mds->server->broadcast_grp_quota(mdr, gqdata, CEPH_MSG_CLIENT_GRP_QUOTA_SET);    
...
void Server::handle_client_grp_quota_modify(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  GrpQuotaMgr *grp_quota = mdcache->grp_quota;
  GrpQuotaData qdata;
  ...
  C_MDS_Grp_Quota_ModFinish *fin = new C_MDS_Grp_Quota_ModFinish(mds, mdr, grp_quota, qdata);
  
  EGrpQuota *le = new EGrpQuota(mdlog, "ModGrpQuota");
  le->set_version(grp_quota->get_version());
  le->grp_quota_data = qdata;
  le->op_type = USER_QUOTA_MOD;
...
// Add by lvq
class C_MDS_Grp_Quota_DelFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  GrpQuotaMgr *grp_quota;
  GrpQuotaData gqdata;
  
  C_MDS_Grp_Quota_DelFinish(MDS *m, MDRequestRef& r, GrpQuotaMgr *u, GrpQuotaData &qdata) : 
    MDSInternalContext(m), mdr(r), grp_quota(u), gqdata(qdata) {}
  void finish(int r) { 
    grp_quota->del_grp_quota(gqdata.gid);
    grp_quota->inc_version();
    // broadcast to all clients
    grp_quota->mds->server->broadcast_grp_quota(mdr, gqdata, CEPH_MSG_CLIENT_GRP_QUOTA_DEL);
   
  }
};

void Server::handle_client_grp_quota_delete(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  GrpQuotaMgr *grp_quota = mdcache->grp_quota;
  GrpQuotaData qdata;
  ...
  GrpQuotaData *pgrp_quota_data = grp_quota->get_grp_quota(qdata.gid);
  if (!pgrp_quota_data) {
  	dout(10) << "quota info does not exist, gid: " << qdata.gid << dendl;
  	reply_request(mdr, -E_QUOTA_NOTEXIST);
  	return;
  }

  C_MDS_Grp_Quota_DelFinish *fin = new C_MDS_Grp_Quota_DelFinish(mds, mdr, grp_quota, qdata);
  
  EGrpQuota *le = new EGrpQuota(mdlog, "DelGrpQuota");
  le->grp_quota_data = *pgrp_quota_data;
  le->op_type = USER_QUOTA_DELETE;
  le->set_version(grp_quota->get_version());
...
// Add by lvq
class C_MDS_Grp_Quota_ClearFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  GrpQuotaMgr *grp_quota;
  
  C_MDS_Grp_Quota_ClearFinish(MDS *m, MDRequestRef& r, GrpQuotaMgr *u) : MDSInternalContext(m), mdr(r), grp_quota(u) {}
  void finish(int r) { 
    grp_quota->clear_grp_quota();
    grp_quota->inc_version();
    //broadcast to all clients
    const ceph::unordered_map<entity_name_t, Session*> session_map = mds->sessionmap.get_sessions();
    for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
      ...
      MClientGrpQuota *msg = new MClientGrpQuota(CEPH_MSG_CLIENT_GRP_QUOTA_CLEAR);
      mds->send_message_client_counted(msg, it->second);
    }
...
void Server::handle_client_grp_quota_clear(MDRequestRef& mdr)
{
  GrpQuotaMgr *grp_quota = mdcache->grp_quota;
  
  if (grp_quota->get_grp_quota_size() <= 0) {
    reply_request(mdr, 0);
    return;
  }
  ...
  // callback context
  C_MDS_Grp_Quota_ClearFinish *fin = new C_MDS_Grp_Quota_ClearFinish(mds, mdr, grp_quota);

  // user quota clear journal event
  EGrpQuota *le = new EGrpQuota(mdlog, "ClearGrpQuota");
  le->set_version(grp_quota->get_version());
  le->op_type = USER_QUOTA_CLEAR;
  mdlog->start_entry(le);
  journal_and_reply(mdr, NULL, NULL, le, fin, false);	
...
// Add by lvq
class C_MDS_Grp_Quota_UpdateFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  map<gid_t, GrpQuotaData> update_quota_map;
  GrpQuotaMgr *grp_quota;
  
  C_MDS_Grp_Quota_UpdateFinish(MDS *m, MDRequestRef& r, GrpQuotaMgr *u) : MDSInternalContext(m), mdr(r), grp_quota(u) {}
  void finish(int r) { 
    for (map<gid_t, GrpQuotaData>::iterator it = update_quota_map.begin();
        it != update_quota_map.end();
        ++it) {
      if (grp_quota->grp_quota_map.count(it->first)) {
        grp_quota->update_grp_quota(it->first, it->second);
        it->second.used_bytes = grp_quota->grp_quota_map[it->first].used_bytes;
      } 
      else {
        dout(0) << "quota info does not exist, gid: " << it->first << dendl;
        update_quota_map.erase(it->first);
      }
    }
    grp_quota->inc_version();
    ...
    for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
      ...
      MClientGrpQuota *msg = new MClientGrpQuota(CEPH_MSG_CLIENT_GRP_QUOTA_UPDATE);
      msg->group_quota_map = update_quota_map;
      mds->send_message_client_counted(msg, it->second);
    }
...
void Server::handle_client_grp_quota_update(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  
  assert(mdcache->grp_quota);

  C_MDS_Grp_Quota_UpdateFinish *fin = new C_MDS_Grp_Quota_UpdateFinish(mds, mdr, mdcache->grp_quota);
  ...
  EGrpQuota *le = new EGrpQuota(mdlog, "UpdateGrpQuota");
  le->op_type = USER_QUOTA_UPDATE;
  le->set_version(mdcache->grp_quota->get_version());
  dout(10) << "Server::handle_client_quota_update, version: " << le->cmapv << ", mdcache->grp_quota->version:" 
    << mdcache->grp_quota->version << dendl;

  for (map<gid_t, GrpQuotaData>::iterator it = fin->update_quota_map.begin();
      it != fin->update_quota_map.end();
      ++it) {
    if (mdcache->grp_quota->grp_quota_map.count(it->first)) {
      le->update_quota_map[it->first] = it->second;
    } 
    else {
      dout(0) << "quota info does not exist, gid: " << it->first << dendl;
...
// Add by lvq
class C_MDS_Grp_File_Quota_UpdateFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  map<gid_t, GrpFileQuota> update_quota_map;
  GrpFileQuotaMgr *grp_file_quota;
  
  C_MDS_Grp_File_Quota_UpdateFinish(MDS *m, MDRequestRef& r, GrpFileQuotaMgr *u) : MDSInternalContext(m), mdr(r), grp_file_quota(u) {}
  void finish(int r) { 
    // update MDCache quota map
    for (map<gid_t, GrpFileQuota>::iterator it = update_quota_map.begin();
          it != update_quota_map.end();
          ++it) {
      grp_file_quota->update_grp_file_quota(it->first, it->second.vary_cnt);    
      update_quota_map[it->first].grp_file_cnt = grp_file_quota->grp_file_quota_map[it->first].grp_file_cnt;
    }
    ...
    for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
      ...
      MClientGrpFileQuota *msg = new MClientGrpFileQuota();
      msg->group_file_map = update_quota_map;
      mds->send_message_client_counted(msg, it->second);
    }
...
void Server::handle_client_grp_file_quota_update(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  
  assert(mdcache->grp_file_quota);

  C_MDS_Grp_File_Quota_UpdateFinish *fin = new C_MDS_Grp_File_Quota_UpdateFinish(mds, mdr, mdcache->grp_file_quota);
  fin->grp_file_quota = mdcache->grp_file_quota;
  
  bufferlist bl = req->get_data();
  bufferlist::iterator it = bl.begin();
  ::decode(fin->update_quota_map, it);

  EGrpFileQuota *le = new EGrpFileQuota(mdlog, "UpdateGrpFileQuota");
  le->set_version(fin->grp_file_quota->get_version());
  
  for (map<gid_t, GrpFileQuota>::iterator it = fin->update_quota_map.begin();
        it != fin->update_quota_map.end();
        ++it) {	
    le->update_file_quota_map[it->first] = it->second;
  }  
...
class C_MDS_Dir_Quota_UpdateFinish : public MDSInternalContext {
public:
  MDRequestRef mdr;
  map<inodeno_t, DirQuotaData> update_quota_map;
  DirQuota *dir_quota;
  
  C_MDS_Dir_Quota_UpdateFinish(MDS *m, MDRequestRef& r, DirQuota *u) : MDSInternalContext(m), mdr(r), dir_quota(u) {}
  void finish(int r) { 
    for (map<inodeno_t, DirQuotaData>::iterator it = update_quota_map.begin();
        it != update_quota_map.end();
        ++it) {
      if (dir_quota->dir_quota_map.count(it->first)) {
        dir_quota->update_dir_quota(it->first,it->second);
        it->second.used_bytes = dir_quota->dir_quota_map[it->first].used_bytes;
      }
      else {
        dout(0) << "quota info does not exist, uid: " << it->first << dendl;
        update_quota_map.erase(it->first);
    ...
    for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
        ...
        ++it) {
      // only broadcast to client
      if (!it->first.is_client())
        continue;

      if (it->second == mdr->session)
        continue;

      MClientDirQuota *msg = new MClientDirQuota(CEPH_MSG_CLIENT_DIR_QUOTA_UPDATE);
      msg->dir_quota_map = update_quota_map;
      mds->send_message_client_counted(msg, it->second);
    }
...
void Server::handle_client_dir_quota_update(MDRequestRef& mdr)
{
  MClientRequest *req = mdr->client_request;
  
  assert(mdcache->dir_quota);

  C_MDS_Dir_Quota_UpdateFinish *fin = new C_MDS_Dir_Quota_UpdateFinish(mds, mdr, mdcache->dir_quota);

  bufferlist bl = req->get_data();
  bufferlist::iterator it = bl.begin();
  ::decode(fin->update_quota_map, it);
  
  EDirQuota *le = new EDirQuota(mdlog, "UpDirQuota");
  le->op_type = DIR_QUOTA_UPDATE;
  le->set_version(mdcache->dir_quota->get_version());
  dout(0) << "Server::handle_client_dir_quota_update, version: " << le->cmapv 
    << ", mdcache->user_quota->version:" 
    << mdcache->dir_quota->version << dendl;

  for (map<inodeno_t, DirQuotaData>::iterator it = fin->update_quota_map.begin();
      it != fin->update_quota_map.end();
      ++it) {
    if (mdcache->dir_quota->dir_quota_map.count(it->first)) {
      le->update_quota_map[it->first] = it->second;
    } else {
      dout(0) << "quota info does not exist, uid: " << it->first << dendl;
      fin->update_quota_map.erase(it->first);
...
void Server::handle_client_dir_quota_get_all(MDRequestRef& mdr)
{  
  assert(mdcache->dir_quota);
  ...
  bufferlist bl;
  ::encode(mdcache->dir_quota->dir_quota_map, bl);
...
void Server::handle_client_dir_quota_get(MDRequestRef& mdr)
{  
  MClientRequest *req = mdr->client_request;
  inodeno_t ino = 0;
  DirQuotaData *dqdata = NULL;
  ...
  dqdata = mdcache->dir_quota->get_used_bytes(ino);
  if (!dqdata) {
    reply_request(mdr, -E_QUOTA_NOTEXIST);
    return;
  }
...
void Server::handle_client_dir_quota_del(MDRequestRef& mdr)
{  
  MClientRequest *req = mdr->client_request;
  DirQuotaData dqdata;
  ...
  if (!mdcache->dir_quota->have_dir_quota(dqdata.ino)) {
    dout(10) << "quota info does not exist, ino: " << dqdata.ino << dendl;
    reply_request(mdr, -E_QUOTA_NOTEXIST);
    return;
  }

  mdcache->dir_quota->del_dir_quota(dqdata.ino);
  mdcache->store_dir_quota();
  ...
  for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
    ...
    MClientDirQuota *msg = new MClientDirQuota(CEPH_MSG_CLIENT_DIR_QUOTA_DEL);
    msg->dir_quota_info = dqdata;
    mds->send_message_client_counted(msg, it->second);
  }
...
void Server::handle_client_dir_quota_set(MDRequestRef& mdr)
{  
  MClientRequest *req = mdr->client_request;
  DirQuotaData dqdata;
  ...
  if (mdcache->dir_quota->have_dir_quota(dqdata.ino)) {
    dout(10) << "quota info exist, ino: " << dqdata.ino << dendl;
    reply_request(mdr, -E_QUOTA_EXIST);
    return;
  }

  mdcache->dir_quota->set_dir_quota(dqdata.ino, dqdata);
  mdcache->store_dir_quota();
  ...
  for (ceph::unordered_map<entity_name_t,Session*>::const_iterator it = session_map.begin();
    ...
    MClientDirQuota *msg = new MClientDirQuota(CEPH_MSG_CLIENT_DIR_QUOTA_SET);
    msg->dir_quota_info = dqdata;
    mds->send_message_client_counted(msg, it->second);
  }
...
void Server::handle_client_readahead(MDRequestRef& mdr)
{
  MClientRequest *req = NULL;
  req =  mdr->client_request;
  ...
  // ok!
  dout(10) << "handle_client_readahead on " << *dir << dendl;
  assert(dir->is_auth());
  ...
  if (!max)
    max = dir->get_num_any();  // whatever, something big.
  unsigned max_bytes = 512 << 10;

  // start final blob
  bufferlist dirbl;
  dir->encode_dirstat(dirbl, mds->get_nodeid());

  // if this client first access
  //if(dir->is_first_access_for_client(client)){
  //  dir->init_cache_for_client(client);
  //}
  ...
  bufferlist dnbl;
  __u32 numfiles = 0;
  __u8 end = (dir->dentries_inoidx.begin() == dir->dentries_inoidx.end());
  
  CDentry* tmpDn = dir->lookup(offset_str);
  if(tmpDn == NULL){
    reply_request(mdr, -ENOENT);
    return;
  }
  
  dout(10) << "readahead start item: " << tmpDn->name << "  cnt:" << max  << dendl;
  if(tmpDn != NULL){
    
    std::map<_inodeno_t, CDentry*>::iterator start_item = tmpDn->items_index_ino;
    //start_item++;
    if(dir->dentries_inoidx.size()>0  && start_item != dir->dentries_inoidx.end()){
      //dout(10) << "readahead start forloop: " << start_item->second->name<< dendl;
      for (std::map<_inodeno_t, CDentry*>::iterator it = start_item;
        !end && numfiles < max;
        end = (it == dir->dentries_inoidx.end())) {
        CDentry* dn = it->second;
        ++it;
        if (dn->state_test(CDentry::STATE_PURGING))
          continue;
        ...
        //client is cache
        if (in->client_caps.find(client) != in->client_caps.end()){
          continue;
        }

        if ((int)(dnbl.length() + dn->name.length() + sizeof(__u32) + sizeof(LeaseStat)) > bytes_left) {
          dout(10) << "readahead ran out of room, stopping at " << dnbl.length() << " < " << bytes_left << dendl;
          break;
        }
        ...
        if (realm->get_last_destroyed() > dir->fnode.snap_purged_thru) {
          snaps = &realm->get_snaps();
          dout(10) << "readahead last_destroyed " << realm->get_last_destroyed() << " > " << dir->fnode.snap_purged_thru
                << ", doing snap purge with " << *snaps << dendl;
          dir->fnode.snap_purged_thru = realm->get_last_destroyed();
          assert(snapid == CEPH_NOSNAP || snaps->count(snapid));  // just checkin'! 
        }
        ...
        // dentry
        dout(10) << "readahead including dn : " << dn->name << dendl;
        ::encode(dn->name, dnbl);
        mds->locker->issue_client_lease(dn, client, dnbl, now, mdr->session);
        // inode
        dout(10) << "readahead including inode " << *in << dendl;
        int r = in->encode_inodestat(dnbl, mdr->session, realm, snapid, bytes_left - (int)dnbl.length());
        if (r < 0) {
          // chop off dn->name, lease
          dout(10) << "readahead ran out of room, stopping at " << start_len << " < " << bytes_left << dendl;
          bufferlist keep;
          keep.substr_of(dnbl, 0, start_len);
          dnbl.swap(keep);
          break;
        ...
        assert(r >= 0);
        numfiles++;
        dout(10) <<  "readahead numfiles: " << numfiles << dendl;
        // touch dn
        mdcache->lru.lru_touch(dn);
      }
    }
  ...
  // yay, reply
  dout(10) << "readahead reply to " << *req << " readdir num=" << numfiles
    << " bytes=" << dirbl.length()
    << " end=" << (int)end
    << " complete=" << (int)complete
    << dendl;
...
void Server::handle_client_readahead_dir_complete(MDRequestRef& mdr)
{
  MClientRequest *req = NULL;
  req =  mdr->client_request;
	
  client_t client = req->get_source().num();
  dout(10) << "readahead_dir_complete client id = " << client << dendl;
  set<SimpleLock*> rdlocks, wrlocks, xlocks;
  CInode *diri = rdlock_path_pin_ref(mdr, 0, rdlocks, false, true);
  if (!diri) return;
  ...
  // ok!
  dout(10) << "handle_client_readahead_dir_complete on " << *dir << dendl;
  assert(dir->is_auth());
  ...
  unsigned max = req->head.args.readdir.max_entries;
  dout(10) << "readahead cnt value : " << max << dendl;
  unsigned max_bytes = 512 << 10;
  ...
  while(g_conf->mds_md_readahead
        && dir->client_complete_pos[client] != dir->dentries_inoidx.end() 
        && numfiles < max){
    std::map<_inodeno_t, CDentry*>::iterator p = dir->client_complete_pos[client];
    CDentry *dn = p->second;
    dir->client_complete_pos[client]++;
    if (dn->state_test(CDentry::STATE_PURGING))
      continue;
  ...
  //__u8 complete = (end && offset_str.empty());  // FIXME: what purpose does this serve
  __u8 end = (__u8)(dir->client_complete_pos[client] == dir->dentries_inoidx.end());
  __u8 complete = end ;
  // finish final blob
  ::encode(numfiles, dirbl);
  ::encode(end, dirbl);
  ...
  dout(10) << "readahead reply to  start" << dendl;
  ...
  dout(10) << "readahead reply to end" << dendl;
