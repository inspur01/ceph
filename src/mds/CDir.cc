...
CDir::CDir(CInode *in, frag_t fg, MDCache *mdcache, bool auth) :
  ...
  bloom(NULL),item_complete_readahead(this)
{
  g_num_dir++;
  g_num_dira++;
...
void CDir::link_remote_inode(CDentry *dn, inodeno_t ino, unsigned char d_type)
{
  ...
  // add item by inode
  CInode* pnode = cache->get_inode(ino);
  if(pnode != NULL){
    add_dentry_inoidx(dn);
  }
...
void CDir::unlink_inode_work( CDentry *dn )
{
  ...
  // remove item in  dentries_inoidx
  remove_dentry_inoidx(dn);
}
...
bool CDir::add_dentry_inoidx(CDentry* dn)
{
  if(g_conf->mds_md_readahead == false){
    return false;
  }

  if(dn != NULL){
    if(dn->get_linkage()->get_inode() == NULL){
      return false;
    }
    
    _inodeno_t ino = dn->get_linkage()->get_inode()->ino().val;
    std::pair<std::map<_inodeno_t, CDentry*>::iterator, bool> ret;
    ret = dentries_inoidx.insert(std::make_pair(ino, dn));
    if(ret.second == true){
      dn->items_index_ino = ret.first;
      dout(10) << "add item by inode: " <<  dn->name << " ino:" << ino << "  addr: " << dn << dendl; 
    }

    // adjust the pos for each client
    if(ret.second == true){
      std::map<client_t, std::map<_inodeno_t, CDentry*>::iterator>::iterator it;
      for (it = client_complete_pos.begin();it!=client_complete_pos.end();++it) {
        
        if(ino < it->second->first){
          dout(10)  << "add item by inode client  " << it->first <<  "  ino: " << it->second->first  
                << "  --->  " << ret.first->first << " name :" << it->second->second->name
                << "  --->  "<< ret.first->second->name  << dendl;
          it->second = ret.first;
        }
      }
    }
...
bool CDir::remove_dentry_inoidx(CDentry* dn)
{
  if(g_conf->mds_md_readahead == false){
    return false;
  }
  ...
  if(dn != NULL){
    ...
    if(dn->get_linkage()->get_inode() != NULL){
      ...
      for (it = client_complete_pos.begin();it != client_complete_pos.end();++it) {
        if(ino == it->second->first){
          std::map<_inodeno_t, CDentry*>::iterator tmpiter = it->second;
          it->second++;
          dout(10)  << "readahead client:  " << it->first <<  "  ino: " << tmpiter->first  
                << "  --->  " << it->second->first << " name :" << tmpiter->second->name
                << "  --->  "<< it->second->second->name  << dendl;
        }
      }
      
      //dout(10) << "readahead remove_dentry_inoidx: " <<  dn->name << " ino: " <<  dn->items_index_ino->first << " addr: " << dn << dendl; 
      if(dn->items_index_ino != dentries_inoidx.end()){
        dentries_inoidx.erase(dn->items_index_ino);
      }
    }
...
int CDir::dump_dentries_inoidx(int fd)
{ 
  int r;
  if(fd < 0 ){
    return -1;
  }
  for (std::map<_inodeno_t, CDentry*>::iterator it = dentries_inoidx.begin();
      it != dentries_inoidx.end();++it) {
    ostringstream ss;
    ss << "ino: " << it->first << "   name: " << it->second->name << std::endl;
    std::string s = ss.str();
    r = safe_write(fd, s.c_str(), s.length());
    if (r < 0)
      return -1;
  }
  dump_client_complete_pos(fd);
  return 0;
}


int CDir::dump_client_complete_pos(int fd)
{ 
  int r;
  if(fd < 0 ){
    return -1;
  ...
  ostringstream head;
  head << " *******client_complete_pos******** " << std::endl;
  ...
  for (it = client_complete_pos.begin();it!=client_complete_pos.end();++it) {
    ...
    ss << "client:  " << it->first <<  "  ino: " << it->second->first  
    	<< "  name: " << it->second->second->name  << std::endl;
    std::string s = ss.str();
    r = safe_write(fd, s.c_str(), s.length());
    if (r < 0)
      return -1;
...
int CDir::init_cache_for_client(client_t client)
{
  if(g_conf->mds_md_readahead == false){
    return 0;
  }
  
  if(!item_complete_readahead.is_on_list()){
    cache->add_complete_readahead_dir(this);
  }
  client_complete_pos.insert(std::make_pair(client, dentries_inoidx.begin()));
  return 0;
}

int CDir::uninit_cache_for_client(client_t client)
{
  client_complete_pos.erase(client);
  if(client_complete_pos.size() == 0){
    item_complete_readahead.remove_myself();
  }
  return 0;
}	

bool CDir::is_first_access_for_client(client_t client)
{
  if(client_complete_pos.count(client)){
    return false;
  }
  return true;
}
...
int CDir::update_client_complete_pos(client_t client, _inodeno_t ino)
{
  if(client_complete_pos.count(client)){
    _inodeno_t nowino = client_complete_pos[client]->first;
    if(ino <= nowino){
      inodeno_t inot(ino);
      CInode* inode = cache->get_inode(inot);
      if(inode != NULL){
        CDentry* dn = inode->get_parent_dn();
        if(dn != NULL){
          client_complete_pos[client] = dn->items_index_ino;
        }
      }
    }
  }
