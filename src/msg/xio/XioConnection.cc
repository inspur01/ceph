...
void print_xio_log(CephContext *cct, const char *tag, const char *com)
{
    ldout(cct,0) << tag << " NullWarning : " << com << dendl;
}
...
int XioConnection::send_message(Message *m)
{
  ...
  iret = ms->_send_message(m, this);
  if(iret == -3)
  {
	  ldout(msgr->cct,0) << __func__ << " " << "warning: need to sleep 5ms, msg["  << m << "]" << dendl;
	  usleep(5*1000);
	  iret = 0;
  }
...
void XioConnection::flush_disorder_msg(XioMessenger *msgr)
{
  unsigned next_sn = exp_sn;
  map<unsigned, Message *>::iterator iter;

  while (msg_map.size()) {
    iter = msg_map.find(next_sn);
    if (iter != msg_map.end()) {
      ldout(msgr->cct,2) << "[error] conn:" << this << __func__ <<
		" send sn:" << ((Message *)iter->second)->get_seq() << dendl;
      msgr->ds_dispatch((Message *)iter->second);
      msg_map.erase(iter);
      next_sn++;
    } else {
      exp_sn = next_sn;
...
int XioConnection::on_msg_req2(XioDispatchHook *m_hook)
{
  struct xio_msg *treq = NULL;
  XioMessenger *msgr = static_cast<XioMessenger*>(get_messenger());
  ...
  Message *m =
    ...
    decode_message(msgr->cct, header, footer, payload, middle,
		   data);
  ...
  if (m) {
    ...
    if (peer_type != (int) hdr.peer_type) { /* XXX isn't peer_type -1? */
      ldout(msgr->cct,2) << "valid peer type, peer_addr " << peer_addr << " hdr.addr " << hdr.addr << dendl;
      peer_type = hdr.peer_type;
      if (!hdr.addr.is_blank_ip())
        peer_addr = hdr.addr;
      peer.addr = peer_addr;
    ...
    if (magic & (MSG_MAGIC_TRACE_XCON)) {
      ...
      ldout(msgr->cct,2) << "[myinfo] conn:" << this << " exp sn:" <<
		exp_sn << ", recv sn:" << m->get_seq() << dendl;
    if (exp_sn != m->get_seq()) {
      ldout(msgr->cct,2) << "[error] conn:" << this << " exp sn:" <<
		exp_sn << ", recv sn:" << m->get_seq() << dendl;
      assert(msg_map.size() < XIO_DISORDER_MSG_DEPTH);
      msg_map.insert(pair<unsigned, Message *>(m->get_seq(), m));
    } else {
      /* dispatch it */
      msgr->ds_dispatch(m);
      exp_sn++;
			      ...
			      void *cb_user_context)
{
  ...
  XioMessenger *msgr = static_cast<XioMessenger*>(get_messenger());
  XioDispatchHook *m_hook = pool_alloc_xio_dispatch_hook(this, NULL /* msg */, in_seq);
  XioInSeq& msg_seq = m_hook->msg_seq;
  in_seq.clear();
					   ...
					   void *conn_user_context)
{
  ...
  isend_ctr = __sync_fetch_and_sub(&send_ctr, 1);
  __sync_fetch_and_sub(&this->portal->imsgcount, 1);
  //ldout(msgr->cct,2) << __FUNCTION__ << " => isend_ctr: " << isend_ctr << ", send_ctr: " << send_ctr << dendl;
  /* unblock flow-controlled connections, avoid oscillation */
  if (unlikely(cstate.session_state.read() == XioConnection::FLOW_CONTROLLED)) {
    if ((isend_ctr <= uint32_t(xio_qdepth_low_mark())) &&(1 /* XXX memory <= memory low-water mark */))
    {
  ...
  //this->flush_input_queue(CState::OP_FLAG_LOCKED);
  xmsg->put();
...
int XioConnection::flush_input_queue(uint32_t flags) {
  ...
  int ix = 0;
  int q_size = outgoing.mqueue.size();
