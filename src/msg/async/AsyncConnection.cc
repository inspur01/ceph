...
int AsyncConnection::_try_send(bufferlist &send_bl, bool send)
{
  ...
  while (left_pbrs) {
    ...
    int r = do_sendmsg(msg, msglen, left_pbrs);
    if (r < 0)
      return r;
      ...
      case STATE_OPEN_MESSAGE_THROTTLE_MESSAGE:
        {
          if (policy.throttler_messages) {
            if (!policy.throttler_messages->get_or_fail()) {
              if (!waiting_for_throttle) {
                ...
                ldout(async_msgr->cct, 5) << __func__ << " wants 1 message from policy throttle "
                     << policy.throttler_messages->get_current() << "/"
                     << policy.throttler_messages->get_max() << " failed, just wait." << dendl;
              }
              // following thread pool deal with th full message queue isn't a short time, so we can wait 10 ms.
              if (register_time_events.empty())
                register_time_events.insert(center->create_time_event(10000, wakeup_handler));
              break;
            }
          }
          ...
          if (waiting_for_throttle) {
            ...
            ldout(async_msgr->cct, 5) << __func__ << " has gotten 1 message" << dendl;
          }
      ...
      case STATE_OPEN_MESSAGE_THROTTLE_BYTES:
        {
          ...
          if (message_size) {
            if (policy.throttler_bytes) {
              if (!policy.throttler_bytes->get_or_fail(message_size)) {
                if (!waiting_for_throttle) {
                  waiting_for_throttle = true;
                  ldout(async_msgr->cct, 5) << __func__ << " wants " << message_size << " bytes from policy throttler "
          ...
          if (waiting_for_throttle) {
            waiting_for_throttle = false;
            ldout(async_msgr->cct, 5) << __func__ << " has gotten " << message_size << " bytes" << dendl;
          }
      ...
      case STATE_OPEN_MESSAGE_READ_FOOTER_AND_DISPATCH:
        {
          ...
          Message *message = decode_message(async_msgr->cct,/* async_msgr->crcflags, // c.o. by wumq*/ current_header, footer, front, middle, data);
          if (!message) {
                                        ...
                                        bufferlist &authorizer_reply)
{
  ...
  if (existing == this)
    existing = NULL;
  if (existing) {
    // There is no possible that existing connection will acquire this connection's lock
    ...
    if (existing->replacing || existing->state == STATE_CLOSED) {
      ldout(async_msgr->cct, 1) << __func__ << " existing racing replace or mark_down happened while replacing."
                                << " existing state=" << get_state_name(existing->state) << dendl;
    ...
    ldout(async_msgr->cct, 0) << __func__ << " accept connect_seq " << connect.connect_seq
                              << " vs existing csq=" << existing->connect_seq << " existing state="
...
void AsyncConnection::fault()
{
  ...
  if (!once_ready && !is_queued() &&
    ...
    ldout(async_msgr->cct, 0) << __func__ << " with nothing to send and in the half accept state just closed" << dendl;
    center->dispatch_event_external(reset_handler);
    _stop();
    return ;
  }
...
int AsyncConnection::_send(Message *m)
{
  ...
  m->encode(features, !async_msgr->cct->_conf->ms_nocrc/*msgr->crcflags*/); // m. by wumq
