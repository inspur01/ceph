...
bool is_format_validate(std::string input){

    boost::regex expression("[~@%^\\-=:\\?\\.,/{}\\[\\]]");
    boost::smatch what;

    if ( boost::regex_search(input, what, expression) )
    {
        if(what.size()>0)
                return false;
    }
...
struct MyProgressContext : public librbd::ProgressContext {
  ...
  void fail2() {
    if (progress) {
      cerr << "\rFail: no snaps exist" << std::endl;
    }
  }
};
  ...
  if (r < 0)
{
  ...
  for (size_t i = 0; i < snaps.size(); ++i) {
	...
	//continue; purge snaps after protecting snap
  }
...
if (poolname &&(!is_format_validate(poolname))){
          cerr<<poolname<<" format wrong"<<std::endl;
          return EXIT_FAILURE;
  }

else if (imgname &&(!is_format_validate(imgname))){
          cerr<<imgname<<" format wrong"<<std::endl;
          return EXIT_FAILURE;
  }
  
else if (snapname &&(!is_format_validate(snapname))){
          cerr<<snapname<<" format wrong"<<std::endl;
          return EXIT_FAILURE;
  }
...
if (dest_poolname &&(!is_format_validate(dest_poolname))){
          cerr<<dest_poolname<<" format wrong"<<std::endl;
          return EXIT_FAILURE;
	}
else if (destname &&(!is_format_validate(destname))){
          cerr<<destname<<" format wrong"<<std::endl;
          return EXIT_FAILURE;
	}
else if (dest_snapname &&(!is_format_validate(dest_snapname))){
          cerr<<dest_snapname<<" format wrong"<<std::endl;
          return EXIT_FAILURE;
	}
  ...
  if ((opt_cmd == OPT_CLONE) && dest_snapname) {
    cerr << "dest image name format wrong" << std::endl;
    return EXIT_FAILURE;
  }
