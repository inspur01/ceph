...
void Objecter::shutdown()
{
  ...
  while(!homeless_session->todo_ops.empty()) {//flc added for quickdelete
    std::list<Op*>::iterator i = homeless_session->todo_ops.begin();
...
void Objecter::handle_osd_map(MOSDMap *m)
{
  ...
  if (mds_max_optimized_ops_per_osd > 0) { 
    map<int, list<Op*> > need_move;
    for (map<int, OSDSession*>::iterator siter = osd_sessions.begin(); siter != osd_sessions.end(); ++siter) {
      _calc_todo_ops_target(siter->second, need_move);
    }
    _calc_todo_ops_target(homeless_session, need_move);

    for (map<int, list<Op*> >::iterator mi = need_move.begin(); mi != need_move.end(); ++mi) {
      OSDSession *s = NULL;
      int r = _get_session(mi->first, &s, lc);
      assert(r == 0);
      assert(s);

      s->lock.get_write();
      for (list<Op*>::iterator itr = mi->second.begin(); itr != mi->second.end(); ++itr) {
        Op *op = *itr;
...
void Objecter::_calc_todo_ops_target(OSDSession *s, map<int, list<Op*> >& need_move)
{
  ...
  for (list<Op*>::iterator itor = s->todo_ops.begin(); itor != s->todo_ops.end();) {
    Op *op =  *itor;
...
ceph_tid_t Objecter::_op_submit(Op *op, RWLock::Context& lc)
{
  ...
  if(mds_max_optimized_ops_per_osd == 0 || op->session == NULL){
    ...
    check_for_latest_map = _calc_target(&op->target) == RECALC_OP_TARGET_POOL_DNE;
  ...
  if(op->is_mds_optimized)
  {
    s->lock.get_write();
    ldout(cct, 10) << "_op_submit osd_id="<<s->osd << " optimized_ops_progressing " << s->optimized_ops_progressing
	      << ", " << mds_max_optimized_ops_per_osd
	      << " todo_ops.size()=" << s->todo_ops.size() << dendl;          
...
void Objecter::handle_osd_op_reply(MOSDOpReply *m)
{
  ...
  if (!op->onack && !op->oncommit) {
    ...
    {
      ldout(cct, 10) << "handle_osd_op_reply osd_id="<<s->osd<<" optimized_ops_progressing "<< s->optimized_ops_progressing
  	<< ", " << mds_max_optimized_ops_per_osd<< " s->todo_ops.size()="<<s->todo_ops.size() 
  	<< dendl;
      ptodo_op = s->todo_ops.front();
      s->todo_ops.pop_front();
    }else if(_is_optimized_op && s->optimized_ops_progressing == 0 && s->todo_ops.size() == 0){
      ldout(cct, 10) << "handle_osd_op_reply osd_id="<<s->osd<<" optimized_ops_progressing "<< s->optimized_ops_progressing
  	<< ", " << mds_max_optimized_ops_per_osd<< " s->todo_ops.size()="<<s->todo_ops.size() 
  	<< dendl;
    }
