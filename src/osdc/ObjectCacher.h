...
template<typename T,class K = int>
class newObjectManager {
  public:
    int objcount;
  public:
    newObjectManager(){
    ...
    T* allocobj(){
      T* tmpret = NULL;
      if(list_freeobj != NULL) {
        tmpret = list_freeobj;
        list_freeobj = list_freeobj->prev;
        return tmpret;
      }
      T* tmphead = new T[objcount];
      for(int index = 1;index < objcount;++index){
        tmphead[index].prev = list_freeobj;
        list_freeobj = &(tmphead[index]);
      }
      tmpret = &(tmphead[0]);
      return tmpret;
    };
    ...
    T* allocobj(K *lpobj){
      ...
      T* tmphead = (T *)operator new(sizeof(T)*objcount);
      new(&tmphead[0])T(lpobj);
      for(int index = 1;index < objcount;++index){
        new(&tmphead[index])T(lpobj);
        tmphead[index].prev = list_freeobj;
        list_freeobj = &(tmphead[index]);
      }
      tmpret = &(tmphead[0]);
...
class ObjectCacher {
 ...
 public:
  ...
  struct OSDRead {
    ...
    void InitObj(snapid_t s,bufferlist *b,int f) {
      snap = s;
      bl = b;
      flags = f;
    }
  ...
  struct OSDWrite {
    ...
    void InitObj(const SnapContext& sc, bufferlist& b, utime_t mt, int f) {
      snapc = sc;
      bl = b;
      mtime = mt;
      flags = f;
  ...
  public:
    class BufferHead *prev;
    class BufferHead *next;
    // states
  ...
  public:
    ...
    void reg_dirty(){
      list<BufferHead*>::iterator  it = find(ob->oset->dirty_bh_map.begin(),ob->oset->dirty_bh_map.end(),this);
      if(it == ob->oset->dirty_bh_map.end())
        ob->oset->dirty_bh_map.push_back(this);
    }
    void unreg_dirty(){
      list<BufferHead*>::iterator  it = find(ob->oset->dirty_bh_map.begin(),ob->oset->dirty_bh_map.end(),this);
      if(it != ob->oset->dirty_bh_map.end())
        ob->oset->dirty_bh_map.erase(it);
    }
    void InitObj(Object *o) {
      ...
      ref = 0;
      ob = o;
      last_write_tid = 0;last_read_tid = 0;
      error = 0;
      ex.start = ex.length = 0;
      prev = 0;
      next = 0;
  ...
  public:
    ...
    Object(ObjectCacher *_oc, sobject_t o, ObjectSet *os, object_locator_t& l,
        uint64_t ts, uint64_t tq,uint64_t file_off) :
      ref(0),
      oc(_oc),
      oid(o), oset(os), set_item(this), oloc(l),
      truncate_size(ts), truncate_seq(tq),foff(file_off),
      complete(false), exists(true),
      last_write_tid(0), last_commit_tid(0),
      dirty_or_tx(0), xlist_mutex((const char*)"xlist") {
        // add to set
        os->objects.push_back(&set_item); 
      }    
    Object():
      set_item(this), xlist_mutex((const char*)"xlist") {
    }
    void InitObj(ObjectCacher *_oc,sobject_t o,ObjectSet *os, object_locator_t& l,
        uint64_t ts,uint64_t tq,uint64_t file_off,uint64_t tmpblockno) {
      lru_init();
      ref = 0;
      oc = _oc;
      oid = o;
      ...
      blockno = tmpblockno;
      os->objects.push_back(&set_item);
      //os->hash_map_objects.insert(make_pair(oid,this));
    }
    void Release() {
      //add by wym
      xlist_mutex.Lock();
    ...
    bool can_close() {
      //print_trace(oc->ofs,oset->parent);
  ...
  struct FileAccessTime:public LRUObject {
    ObjectSet*oset;
    utime_t time;
    FileAccessTime(ObjectSet *_oset):oset(_oset){
      
    };
  };
  struct ObjectSet {
  private:
  	char dirty_lock_nm[128];
  public:
    BufferHead *head;
    BufferHead *tail;
    void *parent;
    ...
    /*losf D????tD¡ä¨®??¡¥¡ê??¨²¨¢??¡é¨º¡Ào¨°¡ê????¡é¡À?inode?¨²¦Ì?bufferhead;
    dirty_bh¦Ì?¡ä|¨¤¨ª1y3¨¬o¨ª?¦Ì¨ª3?-¨¤¡ä?¡§¨°?¦Ì?dirty_or_tx_bh¦Ì?¡ä|¨¤¨ª1y3¨¬¨°??¨´¡ê?
    ?¨²bh_addo¨ªbh_set_state?D¨¬¨ª?¨®¡ê??¨²bh_removeo¨ªbh_set_state?D¨¦?3y?¡ê
    */
    ...
    /////////FileCacher//////
    inline const char* inode2str(inodeno_t ino){
    	memset(dirty_lock_nm,0,sizeof(dirty_lock_nm));
    	sprintf(dirty_lock_nm,"ObjectCacher::ObjectSet_%lu",ino.val);
    	return (const char*) dirty_lock_nm;
    }
    list<BufferHead*> dirty_bh_map ; //add by li.jiebj
    list<ObjectSet*>  *olstptr;
    void reg(list<struct ObjectSet*> *p=NULL){
      if(!olstptr && p)
 				olstptr = p;
      assert(olstptr!=0);
      list<ObjectSet*>::iterator it = find(olstptr->begin(),olstptr->end(),this);
      if(it == olstptr->end())
        olstptr->push_back(this); 
    }
    void unreg(){
     if(olstptr!=0){
	      list<ObjectSet*>::iterator it = find(olstptr->begin(),olstptr->end(),this);
    ...
    void rmLRU()
    {
    	filereadaccess.remove_self();
    	filewriteaccess.remove_self();
   	}
    int64_t dirty_size;
    int64_t tx_size;
    int64_t rx_size;
    map<loff_t,uint64_t> fc_section;
    ceph::unordered_map<sobject_t,Object*> hash_map_objects;
    ceph::unordered_map<uint64_t,Object*> hash_map_blockno_objects;
    LRU ob_lru;
    LRU bh_lru_readahead;
    Mutex dirty_wait_mutex;
    Cond dirty_wait_cond;
    //add by wym
    bool is_dirty_max; // sign up if the current dirty data is beyond dirty_size
    FileAccessTime filereadaccess;
    FileAccessTime filewriteaccess;
    ////////////////////////
    ObjectSet(void *p, int64_t _poolid, inodeno_t i)
      : head(0),tail(0),parent(p),last_pos(0), ino(i), truncate_seq(0),
		truncate_size(0), poolid(_poolid), dirty_or_tx(0),
		return_enoent(false),olstptr(0),dirty_size(0),
	  tx_size(0),rx_size(0),dirty_wait_mutex(inode2str(ino)),is_dirty_max(false),filereadaccess(this),filewriteaccess(this)
	  {
	  	/*char dt_lock_nm[256]={0};
	  	sprintf(dt_lock_nm,"ObjectCacher::ObjectSet_%lu",ino.val);
	  	char * p = (char *)malloc(strlen(dt_lock_nm));
	  	strcpy(p,dt_lock_nm);
	  	dirty_wait_mutex(Mutex((const char*)p));*/
	  }
		//add by wym
		~ObjectSet()
		{
			//add by li.jiebj
	    for(list<BufferHead*>::iterator it = dirty_bh_map.begin();it!=dirty_bh_map.end();it++){
	       if((*it)->is_dirty())
					(*it)->ob->oc->bh_write(*it);
	    }
	    unreg();
	    //add by wym
	    rmLRU();
 ...
 protected:
  ...
  virtual Object *get_object(sobject_t oid, ObjectSet *oset,
					       object_locator_t &l,
					       uint64_t truncate_size,
					       uint64_t fileoff,
					       uint64_t truncate_seq);
  virtual void close_object(Object *ob);
  ...
  // bh states
  virtual void bh_set_state(BufferHead *bh, int s);
 ...
 public:
  ...
  public:
    ...
    C_ReadFinish(ObjectCacher *c, Object *ob, ceph_tid_t t, loff_t s, uint64_t l) :
      oc(c), poolid(ob->oloc.pool), oid(ob->get_soid()),pOb(ob), start(s), length(l),
      set_item(this), trust_enoent(true),
      tid(t) {
      //add by wym
      ob->xlist_mutex.Lock();
      ob->reads.push_back(&set_item);
      ob->xlist_mutex.Unlock();
    }

    void finish(int r) {
      oc->bh_read_finish(poolid, pOb, tid, start, length, bl, r, trust_enoent);

      // object destructor clears the list
      //add by wym
      pOb->xlist_mutex.Lock();
      if (set_item.is_on_list())
	      set_item.remove_myself();
      pOb->xlist_mutex.Unlock();
    }
  ...
  public:
    ceph_tid_t tid;
    C_WriteCommit(ObjectCacher *c, int64_t _poolid, Object *ob, loff_t s, uint64_t l) :
      oc(c), poolid(_poolid), oid(ob->get_soid()),pOb(ob), start(s), length(l), tid(0) {}
    void finish(int r) {
      oc->bh_write_commit(poolid, pOb, start, length, tid, r);
  ...
  virtual int writex(OSDWrite *wr, ObjectSet *oset, Mutex& wait_on_lock,
	     Context *onfreespace);
...
public:
  ...
  virtual bool flush_set(ObjectSet *oset, Context *onfinish=0);

  /* ?¨²¨¢??¡é¦Ì?¨º¡Ào¨°¡ê?flush_set_new???¡é¡À?inode?¨²¦Ì?bufferhead*/
  virtual bool flush_set_new(ObjectSet *oset, Context *onfinish=0);

  virtual bool flush_set(ObjectSet *oset, vector<ObjectExtent>& ex, Context *onfinish=0);
  ...
  virtual void discard_set(ObjectSet *oset, vector<ObjectExtent>& ex);
  ...
  virtual bool fcache_in_scop(uint64_t rx_size){return true;}
  virtual BufferHead *allocbh(Object *ob) {
    return new BufferHead(ob);
  }
  virtual void releasebh(BufferHead *bh) {
    delete bh;
  }
};
