...
public:
  ...
  uint32_t mds_max_optimized_ops_per_osd;//flc added for quickdelete
private:
  atomic64_t last_tid;
...
public:
 ...
 public:
  ...
  struct OSDSession : public RefCountedObject {
    ...
    list<Op*> todo_ops;
    uint32_t optimized_ops_progressing;
 ...
 public:
  Objecter(CephContext *cct_, Messenger *m, MonClient *mc,
    ...
    osdmap(new OSDMap),
    cct(cct_),
    initialized(0), mds_max_optimized_ops_per_osd(0),
 ...
 public:
  ...
  void _calc_todo_ops_target(OSDSession *s, map<int, list<Op*> >& need_move); // flc modified for quickdelete
...
public:
  ...
  // writes
  ceph_tid_t _optimized_modify(const object_t& oid, const object_locator_t& oloc,
		vector<OSDOp>& ops, utime_t mtime,
		const SnapContext& snapc, int flags,
	        Context *onack, Context *oncommit) {
    Op *o = new Op(oid, oloc, ops, flags | global_op_flags.read() | CEPH_OSD_FLAG_WRITE, onack, oncommit, NULL);    
    o->is_mds_optimized = true;//flc added for quickdelete
    o->priority = cct->_conf->mds_optimized_op_priority;//flc added for quickdelete
    o->mtime = mtime;
    o->snapc = snapc;
    return op_submit(o);
  }
