...
ObjectCacher::BufferHead *ObjectCacher::Object::split(BufferHead *left, loff_t off)
{
  ...
  // split off right
  ObjectCacher::BufferHead *right = oc->allocbh(this);//new BufferHead(this);
...
void ObjectCacher::Object::try_merge_bh(BufferHead *bh)
{
  ...
  if (p != data.begin()) {
    ...
    if (p->second->end() == bh->start() &&
        p->second->get_state() == bh->get_state() &&  p->second->have_relation(bh)) {
  ...
  for (vector<ObjectExtent>::iterator ex_it = rd->extents.begin();
    ...
    while (left > 0) {
      ...
      if (p == data.end()) {
        // rest is a miss.
        WCEPHLOG(3531);
        BufferHead *n = oc->allocbh(this);//new BufferHead(this);
        WCEPHLOG(3532);
        n->set_start(cur);
        n->set_length(left);
        oc->bh_add(this, n);
...
ObjectCacher::BufferHead *ObjectCacher::Object::map_write(OSDWrite *wr)
{
  ...
  for (vector<ObjectExtent>::iterator ex_it = wr->extents.begin();
    ...
    while (left > 0) {
      ...
      if (p == data.end()) {
        if (final == NULL) {
          final = oc->allocbh(this);//new BufferHead(this);
          final->set_start( cur );
...
{
  ofs.open("/tmp/oc.log",ios::out|ios::trunc);
...
int ObjectCacher::file_read(ObjectSet *oset, ceph_file_layout *layout, snapid_t snapid,
					       ...
					       uint64_t truncate_seq)
{
  ...
  // create it.
  Object *o = new Object(this, oid, oset, l, truncate_size, truncate_seq,fileoff);
...
void ObjectCacher::bh_read_finish(int64_t poolid, Object *pOb, ceph_tid_t tid,
...
void ObjectCacher::bh_write_commit(int64_t poolid,Object *pOb, loff_t start,
...
void ObjectCacher::trim(ObjectSet *oset)
{
  assert(lock.is_locked());
...
int ObjectCacher::file_write(ObjectSet *oset, ceph_file_layout *layout, const SnapContext& snapc,
    loff_t offset, uint64_t len, 
...
/* ?¨²¨¢??¡é¨º¡Ào¨°¡ê????¡é¡À?inode?¨²¦Ì?bufferhead */
bool ObjectCacher::flush_set_new(ObjectSet *oset, Context *onfinish)
{
  assert(lock.is_locked());
  ...
  ldout(cct, 10) << "flush_set_new dirty_bh size " << oset->dirty_bh.size() << dendl;
  ...
  next = it = oset->dirty_bh.begin();
  while (it != oset->dirty_bh.end()) {
    ++next;
    BufferHead *bh = *it;
...
/*
int ObjectCacher::file_readahead(Fh *f ,uint64_t off, uint64_t len,Context *onfinish)
{
	int ret = -1;
  ...
  if (readahead &&
    ...
    if (l > (loff_t)len) {
      if (objectcacher->file_is_cached(&in->oset, &in->layout, in->snapid, off, min))
        ...
        ret = objectcacher->file_read(&in->oset, &in->layout, in->snapid,
                                        off, l,
                                        NULL, 0, onfinish);
      }
    }
