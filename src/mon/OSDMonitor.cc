...
//modify by wangshuguang for capacity balance
bool OSDMonitor::thrash()
{
  ...
  if (osdmap.is_up(o)) {
    ...
    pending_inc.new_weight[o | 0xFFFFFFFF00000000] = CEPH_OSD_IN;
  ...
  if (osdmap.exists(o)) {
    ...
    pending_inc.new_weight[o | 0xFFFFFFFF00000000] = CEPH_OSD_OUT;
  }
...
//add by wangshuguang for capacity balance
/* Assign a lower weight to overloaded OSDs by pg
 *
 ...
 */
int OSDMonitor::reweight_by_pg_utilization(int oload, std::string& out_str,
				const vector<__u32> *pools)
{
	...
	int64_t pool_osd = 0;
	bool changed = false;
	for (j = 0 ; j < pools->size(); j++)
	{
	    pgs_by_osd.clear();
		pgs_by_osd.resize(osdmap.get_max_osd());
		osds.clear();
		num_pg_copies = 0;
		average_util = 0.0;
		weight_sum = 0.0; 
	    ...
	    for (ceph::unordered_map<pg_t,pg_stat_t>::const_iterator p =
	         pgm.pg_stat.begin(); p != pgm.pg_stat.end(); ++p) 
	 	{
	 		if (p->first.pool() != pool_id)
	 			continue;
	 		for (vector<int>::const_iterator q = p->second.acting.begin();
	   	         q != p->second.acting.end(); ++q) 
	 		{
		...
		char buf4[128];
		snprintf(buf4, sizeof(buf4), " weight_sum %04f, num_osds %d, num_pg_copies %u, osds_size %u. ",
		   weight_sum, num_osds, num_pg_copies, osds.size());
		oss << buf4;
		...
		double overload_util = average_util * (double)101 / 100.0;

		// but aggressively adjust weights up whenever possible.
		double underload_util = average_util * (double)97 / 100.0;

		snprintf(buf, sizeof(buf), " average %04f, overload %04f, underload %04f. ",
		   average_util, overload_util, underload_util);
		oss << buf;
		oss << "reweighted: ";
		for (__u32 i = 0; i < osds.size(); i++)
		{
			float util;
			pool_osd = 0;
			pool_osd += pool_id;
			pool_osd = pool_osd << 32;
			pool_osd += osds[i];
			util = pgs_by_osd[osds[i]] / osdmap.crush->get_item_weightf(osds[i]);
                        char buf1[128];
			snprintf(buf1, sizeof(buf1), "osd.%d pg_by_osd:%d, weightf:%04f, util:%04f", osds[i], pgs_by_osd[osds[i]], osdmap.crush->get_item_weightf(osds[i]), util);
			oss << buf1 << ",";
			if (util >= overload_util) 
			{
				...
				unsigned weight = osdmap.get_pool_osd_weight(pool_id, osds[i]);
				unsigned new_weight = (unsigned)((average_util / util) * (float)weight);
				pending_inc.new_weight[pool_osd] = new_weight;
				char buf2[128];
				snprintf(buf2, sizeof(buf2), "osd.%d [%04f -> %04f]", osds[i],
			...
			if (util <= underload_util) 
			{
				...
				if (new_weight > weight) 
				{
					...
					pending_inc.new_weight[pool_osd] = new_weight;
					char buf3[128];
					snprintf(buf3, sizeof(buf3), "osd.%d [%04f -> %04f]", osds[i],
						 (float)weight / (float)0x10000,
						 (float)new_weight / (float)0x10000);
					oss << buf3 << sep;
					changed = true;
	...
	out_str = oss.str();
	dout(10) << "reweight_by_pg_utilization: finished with " << out_str << dendl;
	return changed;
}
...
void OSDMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
  ...
  //modify by wangshuguang for capacity balance
  for (map<int64_t,uint32_t>::iterator i = pending_inc.new_weight.begin();
       i != pending_inc.new_weight.end();
       ++i) {
    if (i->second == CEPH_OSD_OUT) {
      dout(2) << " pool:" << (i->first >> 32)  << " osd." << (i->first & 0x00000000FFFFFFFF) << " OUT" << dendl;
    } else if (i->second == CEPH_OSD_IN) {
      dout(2) << " pool:" << (i->first >> 32)  << " osd." << (i->first & 0x00000000FFFFFFFF) << " IN" << dendl;
    } else {
      dout(2) << " pool:" << (i->first >> 32)  << " osd." << (i->first & 0x00000000FFFFFFFF) << " WEIGHT " << hex << i->second << dec << dendl;
    }
  }
...
bool OSDMonitor::prepare_boot(MOSDBoot *m)
{
  ...
  if (osdmap.is_up(from)) {
	...
	(g_conf->mon_osd_auto_mark_in)) {
      if (can_mark_in(from)) {
	pool_osd = 0xFFFFFFFF00000000;
	pool_osd += from;
	if (osdmap.osd_xinfo[from].old_weight > 0)
	  pending_inc.new_weight[pool_osd] = osdmap.osd_xinfo[from].old_weight;
	else
	  pending_inc.new_weight[pool_osd] = CEPH_OSD_IN;
      } else {
...
void OSDMonitor::tick()
{
  ...
  if (can_mark_out(-1)) {
	...
	if (g_conf->mon_osd_down_out_interval > 0 &&
	  ...
	  pool_osd = 0xFFFFFFFF00000000;
	  pool_osd += o;
	  pending_inc.new_weight[pool_osd] = CEPH_OSD_OUT;
...
bool OSDMonitor::preprocess_command(MMonCommand *m)
{
  ...
  //add by yueyuanfang for capacity balance
  else if (prefix == "osd dump" ||
	   prefix == "osd tree" ||
	   prefix == "osd pool tree" ||
	   prefix == "osd ls" ||
	   prefix == "osd getmap" ||
...
/*add by yueyuanfang for capacity balance*/
	...
	if(pool_id < 0){
      ...
      goto reply;
	   ...
	   if (f) {
	 f->open_object_section("tree");
	 p->print_osd_pool_tree(NULL, f.get(),pool_id);
	 f->close_section();
	 f->flush(ds);
	   } else {
	 p->print_osd_pool_tree(&ds, NULL,pool_id);
	   } 
	   rdata.append(ds);
    } 
/*yueyuanfang*/
				 ...
				 stringstream &ss)
{
  ...
  pi->tier_min_object_size = g_conf->osd_pool_default_tier_min_object_size;
                                         ...
                                         stringstream& ss)
{
  ...
  } else if (var == "tier_min_object_size") {
    if (interr.length()) {
      ss << "error parsing int '" << val << "': " << interr;
      return -EINVAL;
    }
    p.tier_min_object_size = n;
  } else {
				      ...
				      map<string,cmd_vartype> &cmdmap)
{
  ...
  if (prefix == "osd setcrushmap" ||
    ...
    if (key == "pause")
      ...
      return prepare_set_flag(m, CEPH_OSDMAP_NOFLUSH);
    else {
    ...
    if (key == "pause")
      ...
      return prepare_unset_flag(m, CEPH_OSDMAP_NOFLUSH);
    else {
      ss << "unrecognized flag '" << key << "'";
      err = -EINVAL;
    }
	...
	//modify by wangshuguang for capacity balance
    if (osdmap.exists(id)) {
	  int64_t pool_osd = 0xFFFFFFFF00000000;
	  pool_osd += id;
      pending_inc.new_weight[pool_osd] = ww;
  ...
  //modify by wangshuguang for capacity balance
   /*else if (prefix == "osd reweight-by-utilization") {
    ...
    for (unsigned j = 0; j < poolnamevec.size(); j++) {
      int pool = osdmap.lookup_pg_pool_name(poolnamevec[j]);
      if (pool < 0) {
    ...
    string out_str;
    err = reweight_by_pg_utilization(oload, out_str, &pools);
    if (err < 0) {
  ...
  /*add by yueyuanfang for capacity balance*/
  else if (prefix == "osd pool reweight") 
  {
	const PGMap &pgm = mon->pgmon()->pg_map;
    ...
    if (osdmap.exists(id)) 
	{
	         ...
	         pgm.pg_stat.begin(); p != pgm.pg_stat.end(); ++p) 
		{
			...
			if(!changed)
			{
				ss << "pool "<< poolstr << " has no osd."<< id << "in pool";
				err = -EINVAL;
      			goto reply;
			}
			else
