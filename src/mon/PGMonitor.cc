...
void PGMonitor::check_osd_map(epoch_t epoch)
{
  ...
  for (epoch_t e = pg_map.last_osdmap_epoch+1;
    ...
    for (map<int64_t,uint32_t>::iterator p = inc.new_weight.begin();
	 p != inc.new_weight.end();
	 ++p)
      if (p->second == CEPH_OSD_OUT) {
	dout(10) << "check_osd_map  osd." << (p->first & 0x00000000FFFFFFFF) << " went OUT" << dendl;
	pending_inc.stat_osd_out(p->first & 0x00000000FFFFFFFF);
      }
			   ...
			   list<pair<health_status_t,string> > *detail) const
{
  ...
  if (g_conf->mon_osd_max_op_age > 0 &&
    ...
    if (sum > 0) {
      ...
      summary.push_back(make_pair(HEALTH_OK, ss.str()));
