...
COMMAND("grant show ", "show grant liscense", "mon", "r", "cli,rest")
...
COMMAND("osd set " \
	"name=key,type=CephChoices,strings=pause|noup|nodown|noout|noin|nobackfill|norecover|noscrub|nodeep-scrub|noflush", \
...
COMMAND("osd pool reweight " \
	...
	"name=id,type=CephInt,range=0 " \
    "type=CephFloat,name=weight,range=0.0|1.0", \
	"reweight pool osd weight to 0.0 < <weight> < 1.0", "osd", "rw", "cli,rest")
COMMAND("osd lost " \
	"name=id,type=CephInt,range=0 " \
	"name=sure,type=CephChoices,strings=--yes-i-really-mean-it", \
