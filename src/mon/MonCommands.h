...
/*
 * Monitor commands (GrantMonitor.cc)
 */
COMMAND("grant import "\
		"name=grantkey,type=CephString ", \
		"grant import: import grantkey from string", "mon", "rw", "cli,rest")
COMMAND("grant show ", "show grant license", "mon", "r", "cli,rest")
...
COMMAND("osd erasure-code-profile ls", \
	...
	"osd", "r", "cli,rest")
COMMAND("osd set " \
	"name=key,type=CephChoices,strings=pause|noup|nodown|noout|noin|nobackfill|norecover|noscrub|nodeep-scrub|notieragent|noflush", \
...
//add by yueyuanfang for capacity balance
COMMAND("osd pool reweight " \
	"name=pool,type=CephPoolname " \
	"name=id,type=CephInt,range=0 " \
    "type=CephFloat,name=weight,range=0.0|1.0", \
	"reweight pool osd to 0.0 < <weight> < 1.0", "osd", "rw", "cli,rest")
COMMAND("osd pg-temp " \
...
COMMAND("osd pool stats " \
        ...
        "osd", "r", "cli,rest")
//delete by wangshuguang for capacity balance
