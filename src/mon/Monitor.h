...
#include "messages/MClientGetUid.h"//zzg
#include "messages/MClientUidReply.h" //zzg
...
class MAuthRotating;
class GrantKeyServer;  //add by bjq
struct MRoute;
struct MForward;
struct MTimeCheck;
struct MMonHealth;
...
public:
  ...
  GrantKeyServer *grant_key_server ; //add by bjq
...
private:
  void new_tick();
  void new_check_tick();         //add  by zhanghao
public:
void one_day_tick() ;         //add by bijingqiangbj@inspur.com
...
public:
  ...
  static const string SKEY_L;   //add by zhanghao
 ...
 */
  class CephGrantData {
	...
	string end_t;
	string limit_nodes;
	bool is_trial_expire;
	//the left
   	 int64_t left_days;
	string s_left_days;// get by local time and end_t
	string s_left_nodes;
	string s_left_cap;

  	//string cluster_time;
	//string t_left_days;//true left days, get by cluster_time and end_t
...
public:
  ...
  string current_date ;
public:
	bool is_exceed_max_host();
	bool is_exceed_max_cap();
	string get_ip_from_addr(const sockaddr_storage &ss);
	bool get_grant_data();
	bool valid_grant();
	void force_shutdown_by_grant(bool is_grant);
	void show_grant_data(stringstream &ss); 
	CephGrantData(Monitor* mon_i) : mon_gr(mon_i), lic_type(GRANT_TRIAL), is_trial_expire(false) { }
  };
  
public:
  CephGrantData grant_data;// add by zhanghao
private:
...
private:
  ...
  void handle_client_uid(MClientGetUid *m);//zzg
...
public:
  ...
  class GrantMonitor *grantmon() {
    return (class GrantMonitor *)paxos_service[PAXOS_GRANT];
  }
...
public:
 ...
 public:
  ...
  void shutdown();
  void tick();
  void check_tick();               //add by zhanghao

  void handle_signal(int sig);

  // static string  encrypt_l(const string& sKey, const string& plainText); // add by zhanghao
  // static string  decrypt_l(const string& sKey, const string& cipherText); //add by zhanghao 

  static string  encrypt_m(const string& sKey, const string& plainText); // add by zhanghao
  static string  decrypt_m(const string& sKey, const string& cipherText); //add by zhanghao 
...
//add by zhanghao
struct Date_for_grant {
  	string date;
	int64_t year, month, day;
	int64_t total_days;
	Date_for_grant(string date_i): date(date_i), year(0), month(0), day(0), total_days(0) {}
	bool is_leap() {
		return ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) ;
	}
	void string_to_date() {
		year = atol((date.substr(0,4)).c_str());
		month = atol((date.substr(4,2)).c_str());
		day = atol((date.substr(6,2)).c_str());
	}
	void date_to_string() {
		char buf[32];
  		sprintf(buf, "%ld%02ld%02ld", year, month, day);
  		date = buf;
	}
	int64_t day_in_year() {
		int64_t DAY[12] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
		total_days = 0;
		if (is_leap())
			DAY[1] = 29;
	...
	//day + days_i
	void day_to_date(int64_t days_i) {
		day_in_year();
		int64_t DAY[12] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
		int64_t all_days_year = 0;
		if (is_leap()) {
			DAY[1] = 29;
			all_days_year = 366;
		}
		...
		if (total_days > all_days_year) {
			...
			if (is_leap()) {
				...
				all_days_year = 365;
			total_days -= all_days_year;
		}
		int64_t index = 0;
		while (total_days > 0)
			total_days -= DAY[index++];
		total_days = total_days + DAY[--index];
		month = index+1;
		day = total_days;
	}	
			
	//only calculate the year diff is one year
	int64_t days_between_date(Date_for_grant* d_grant) {
		int64_t days_diff = 0;
		d_grant->string_to_date();
		if (year == d_grant->year) 
			days_diff = abs(day_in_year() - d_grant->day_in_year());
		else if (year > d_grant->year) {
			if (is_leap())
				days_diff = day_in_year()+366-d_grant->day_in_year();
			else 
				days_diff = day_in_year()+365-d_grant->day_in_year();
		}
		else {
			if (is_leap())
				days_diff = -day_in_year()+366+d_grant->day_in_year();
