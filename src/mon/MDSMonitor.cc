...
void MDSMonitor::fail_mds_gid(uint64_t gid)
{
  ...
  dout(0) <<" [ MDSMonitor::fail_mds_gid ]: blacklist !  " << gid << " " << info.addr << " mds." << info.rank << "." << info.inc
          << " " << ceph_mds_state_name(info.state) <<  "      name = "<< info.name << dendl;
    ...
    std::stringstream *ss) const
{
  ...
  */
//Modify by yutao at 20150326 for bug 0003316: ceph new 文件系统是无法指定纠删池
/*
  if (pool->is_tier()) {
    *ss << " pool '" << pool_name << "' (id '" << pool_id
...
//2015-09-11日 flc 限定纠删池不能作为元数据池//begin////////////////////////
    ...
    if (metadata_pool->is_erasure()) {
      ss << "pool(id '" << metadata << "')"
         << " is an erasure-code pool, Cannot be used as metadata pool.";
      return -EINVAL;    
    }
//2015-09-11日 flc 限定纠删池不能作为元数据池//end////////////////////////
...
//2015-09-10日 flc 限定纠删池不能作为元数据池//begin////////////////////////
    if (metadata_pool->is_erasure()) {
      ss << "pool '" << metadata_name << "' (id '" << metadata << "')"
         << " is an erasure-code pool, Cannot be used as metadata pool.";
      return -EINVAL;    
    }
//2015-09-10日 flc 限定纠删池不能作为元数据池//end////////////////////////
...
void MDSMonitor::tick()
{
  ...
  if (mon->osdmon()->is_writeable()) {
	...
	dout(0) << " replacing " << gid << " " << info.addr << " mds." << info.rank << "." << info.inc
		 << " " << ceph_mds_state_name(info.state)
	...
	if (si.state > 0 ||
	  ...
	  utime_t until = now;
	  until += g_conf->mds_blacklist_interval;
	  dout(0) <<" [ MDSMonitor::tick ]: blacklist ! no beacon from " << gid << " " << info.addr << " mds." << info.rank << "." << info.inc
	          << " " << ceph_mds_state_name(info.state) << " since " << since << "      cutoff = "<< cutoff << dendl;
	  pending_mdsmap.last_failure_osd_epoch = mon->osdmon()->blacklist(info.addr, until);
	  propose_osdmap = true;
	}
	pending_mdsmap.mds_info.erase(gid);
