...
#include "ClientUid.h"//zzg

#include "GrantMonitor.h"    //add by zhanghao

#include <cstdlib>          //add by zhanghao
#include <uuid/uuid.h>      //add by zhanghao
#include <stdio.h>          //add by zhanghao 
#include <sys/socket.h>    //add by zhanghao
#include <netdb.h>         //add by zhanghao
#include <cctype>          //add by zhanghao
...
const string Monitor::SKEY_L = "3Qw65kYgInVfkK7hEdR0dDMsKZDtLFlc";       //add by zhanghao 
...
Monitor::Monitor(CephContext* cct_, string nm, MonitorDBStore *s,
  ...
  quorum_features(0),
  scrub_version(0),
  grant_data(this),  //add by zhanghao
  ...
  routed_request_tid(0)
{
  ...
  paxos_service[PAXOS_GRANT] = new GrantMonitor(this, paxos, "grant");
  ...
  grant_key_server = new GrantKeyServer();
}
...
PaxosService *Monitor::get_paxos_service_by_name(const string& name)
{
  ...
  if (name == "grant")
    return paxos_service[PAXOS_GRANT];
...
int Monitor::init()
{
  ...
  one_day_tick();   //add by bijingqiangbi@inspur.com
  check_tick();  //add  by zhanghao
  lock.Unlock();
  return 0;
}
...
/* add by zhanghao
 * display the authorization data
 */
    grant_data.show_grant_data(ss);
...
void Monitor::dispatch(MonSession *s, Message *m, const bool src_is_mon)
{
  ...
  switch (m->get_type()) {
    ...
    case CEPH_MSG_GET_CLIENT_UID:
      handle_client_uid(static_cast<MClientGetUid*>(m));  //zzg
      break;
...
//zzg
void Monitor::handle_client_uid(MClientGetUid *m)
{
  dout(10) << "Get user:" << m->username << " passwd " << m->passwd << dendl;
  int wuuid = -1,wugid = -1;
  int authed = 0;
  
  entity_inst_t inst = m->get_source_inst();
  
  ClientUid *cu = new ClientUid();
  if (m->bauth)
    authed = cu->user_authenticate(m->username.c_str(),m->passwd.c_str());
  if (authed == 0){  //user auth
    authed = cu->user_info(m->username.c_str(), &wuuid, &wugid);   //get user info
  }
  MClientUidReply *reply = new MClientUidReply(authed, wuuid, wugid);
...
/*
 * add by zhanghao
 * detect whether the authorization expires
 * C_Check_Tick
 * new_check_tick
 * check_tick
 */
class C_Check_Tick : public Context {
  Monitor *mon;
public:
  C_Check_Tick(Monitor *m) : mon(m) {}
  void finish(int r) {  	
    mon->check_tick();
 }
};

void Monitor::new_check_tick()
{
  C_Check_Tick *ctx = new C_Check_Tick(this);
  int interval = (g_conf->mon_tick_check_interval > 3600)?3600:(g_conf->mon_tick_check_interval);  
  timer.add_event_after(interval, ctx);
}

//add by bijingqiangbj@inspur.com 
//one day tick 
class C_OneDay_Tick : public Context {
  Monitor *mon;
public:
  C_OneDay_Tick(Monitor *m) : mon(m) {}
  void finish(int r) {    
    mon->one_day_tick();
 }
};

void Monitor::one_day_tick()
{
  	dout(10) << "into func: one_day_tick" << dendl;
    ...
    // now update current date
  	if(grant_data.last_date.empty()){
    	grant_data.last_date = curr_date; 
    } else {
      	Date_for_grant dfg(grant_data.last_date) ;
      	grant_data.last_date = dfg.next();
    }     
    grant_data.current_date = curr_date; //get current_date 

    C_OneDay_Tick *ctx = new C_OneDay_Tick(this);
    int seconds_per_day = 86400 ;
    timer.add_event_after(seconds_per_day, ctx);//one day
    dout(10) <<"last_date: "<<grant_data.last_date<<dendl ;
    dout(10) <<"current_date: "<<grant_data.current_date<<dendl ;
}

void Monitor::check_tick()
{
	dout(10) << "into func: check_tick" << dendl;
	if(!is_leader()) {
		new_check_tick();
		return;
	}
	if (!grant_data.valid_grant()) {
		dout(0) << "start force_shutdown_by_grant" << dendl;
		grant_data.force_shutdown_by_grant(true);
	}
	new_check_tick();
}
...
#if 0
/* add by zhanghao
 * encrypt or decrypt for license or others
 * encrypt_l, decrypt_l
 */
 string  Monitor::encrypt_l(const string& sKey,const string& plainText)
{
	
	bufferptr keybuf(sKey.c_str(), sKey.size());

	CryptoKey key(CEPH_CRYPTO_AES, ceph_clock_now(g_ceph_context), keybuf);

	bufferptr ptr(plainText.c_str(), plainText.size());
	...
	//dout(0) << "encoded len is " << enc_out.length() << dendl;
	//dout(10) << "encoded msg is " << enc_out.c_str() << dendl;
  string out_s(enc_out.c_str());
  dout(0)<<"encrypt_l: "<<plainText<<" to "<<out_s << dendl ;
	return out_s;

}

string  Monitor::decrypt_l(const string& sKey,const string& cipherText)
{
	...
	bufferptr ptr(cipherText.c_str(), cipherText.size());
	...
	string out_s(dec_out.c_str());
  dout(0)<<"decrypt_l: "<<cipherText<<" to "<<out_s << dendl;
	return out_s;
}

#endif

std::string Monitor::encrypt_m(const std::string & sKey,const string &plainText1)
{
   	const char *plainText = plainText1.c_str() ;
    std::string error ;
    const unsigned char *key = (const unsigned char *)sKey.c_str();
    ...
    CryptoPP::StreamTransformationFilter cbcEncryptor(cbcEncryption, new CryptoPP::HexEncoder(sink));
...
std::string Monitor::decrypt_m(const std::string& sKey,const string& cipherText1)
{
    std::string error ;
	for (string::size_type index = 0; index != cipherText1.size(); ++index) {
		if (isalpha(cipherText1[index]))
			continue;
		else if (isdigit(cipherText1[index]))
			continue;
		else {
			error = "error";
			break;
   	...
   	const char *cipherText = cipherText1.c_str() ;
 	const unsigned char *key = (const unsigned char *)sKey.c_str();
    ...
    CryptoPP::HexDecoder decryptor(new CryptoPP::StreamTransformationFilter(cbcDecryption, new CryptoPP::StringSink(decryptedtext)));
 ...
 * judge the ip number
 */
 bool Monitor::CephGrantData::is_exceed_max_host() {
/*
 * get all ip in the cluster
 */
    set<string> all_ip;
    all_ip.clear();
	for (int k=0; k!=(mon_gr->osdmon()->osdmap).get_max_osd();  k++ ) {
		 if ((mon_gr->osdmon()->osdmap).exists(k)) {
				all_ip.insert(get_ip_from_addr((mon_gr->osdmon()->osdmap).get_addr(k).addr));
		 }
	}
	int64_t left_nodes = atol(limit_nodes.c_str()) - all_ip.size();
	char buf[16];
	sprintf(buf, "%ld", left_nodes);
	s_left_nodes = buf;
	if (all_ip.size() <= static_cast<unsigned long>(atol(limit_nodes.c_str()))) {
		return false;
	} 
	return true; 
}
 ...
 * convert addr to ip
 */
string Monitor::CephGrantData::get_ip_from_addr(const sockaddr_storage &ss) {
...
/*
 * judge the max capacity
 */
bool Monitor::CephGrantData::is_exceed_max_cap() 
{
	int64_t total_cap = (*(mon_gr->pgmon())).pg_map.osd_sum.kb/1024/1024/1024;
	int64_t left_cap = atol(limit_cap.c_str()) - total_cap; 
	char buf[16];
	sprintf(buf, "%ld", left_cap);
	s_left_cap = buf;
	if (total_cap > atol(limit_cap.c_str())) {
		return true;
	}
	return false;
}
 ...
 * date init
 */
 bool Monitor::CephGrantData::get_grant_data()
 {
  string str_auth_code = mon_gr->grant_key_server->data.grant_data;// not sure the name,and did not add the grantmon function
  if (str_auth_code.empty())
    return false ;
  //decrypt
  string d_auth_code = decrypt_m(Monitor::SKEY_L, str_auth_code);
  //dout(0)<<"str_auth_code is: "<<str_auth_code<<dendl ;
  //dout(0)<<"d_auth_code is: "<<d_auth_code<<dendl ;
  string s_lic_type;
  char* p = NULL;
  char* p_left = NULL;
  p = strtok_r(const_cast<char*>(d_auth_code.c_str()), "/", &p_left);
  s_lic_type = p;
  p = strtok_r(p_left, "/", &p_left);
  end_t = p;
  p = strtok_r(p_left, "/", &p_left);
  limit_nodes = p;
  p = strtok_r(p_left, "/", &p_left);
  ...
  p = NULL;
  p_left = NULL;
  p = strtok_r(const_cast<char*>(limit_cap.c_str()), "T", &p_left);
  limit_cap = p;

  if (s_lic_type == "trial") {
    dout(10)<<"str_auth_code is a trial "<<dendl ;
    lic_type = GRANT_TRIAL;
    utime_t curr_time = ceph_clock_now(g_ceph_context);
    time_t clu_time = curr_time.tv.tv_sec;
    struct tm *ptm =localtime(&clu_time);     
    ...
    Date_for_grant end_date(end_t);
    end_date.string_to_date();

    Date_for_grant clu_date(date_clu);

    left_days = end_date.days_between_date(&clu_date); 
    
    char buf_l[16];
    sprintf(buf_l, "%ld", left_days);
    s_left_days = buf_l;      
  } else if (s_lic_type == "official"){
      dout(10)<<"str_auth_code is a official "<<dendl ;
      lic_type = GRANT_OFFICIAL;
      this->limit_nodes = limit_nodes ;
      this->limit_cap = limit_cap ;
      is_exceed_max_cap() ;
      is_exceed_max_host() ;
  } else {
    lic_type = GRANT_NONE;  
...
/*
 * check the grant state
 */			
bool Monitor::CephGrantData::valid_grant()
{
	if (!get_grant_data())
		return true ;//prevent from sending kill osd signal
	if (lic_type == GRANT_TRIAL) {
    	dout(10) << "in func:valid_grant " << dendl;

		// int64_t days_shift = 0 ;// default value: no shift days 
    	bool is_date_equal = true ;
    	bool is_ajust_backward = false ;
		if(!this->last_date.empty() && !this->current_date.empty())	{			
			Date_for_grant last_date_grant(this->last_date);
			Date_for_grant current_date_grant(this->current_date);
	    	// days_shift = last_date_grant.days_between_date(&current_date_grant);  
        	if(this->last_date != this->current_date) {
        		is_date_equal = false ;
           		if(this->last_date > this->current_date) {
            		is_ajust_backward = true ;
           		}
        	}
        	dout(10) << "last_date: " <<this->last_date<< dendl;
        	dout(10) << "current_date: " <<this->current_date<< dendl;      
    	}
		// if ( (abs(days_shift)>2) || (left_days == 0)) {
     	if ( (!is_date_equal) || (left_days == 0)) {
	   		if(left_days == 0) {
				dout(0) << "the trial period is expired, shutdown all osds " << dendl;
			} else {
     			if(is_ajust_backward) {
					dout(0) << "we found that you have ajusted the local time backward , shutdown all osds " << dendl;
				} else {
        			dout(0) << "we found that you have ajusted the local time forward, shutdown all osds " << dendl;
    			}
    		}
			lic_type =  GRANT_EXPIRE;
			char buf[16];
			left_days = -1;
			is_trial_expire = true;
			sprintf(buf, "%ld", left_days);
			s_left_days = buf; 
			return false;
		}	
	} else if (lic_type == GRANT_OFFICIAL) {
			if (is_exceed_max_cap()) {
				dout(0) << " the total_cap > limit_cap , so shutdown all osd " << dendl;
				lic_type = GRANT_EXPIRE;
				//s_left_cap=-1,s_left_nodes=-1
				int64_t left_cap = -1;
				int64_t left_node = -1;
				char buf_c[16], buf_n[16];
				sprintf(buf_c, "%ld", left_cap);
				s_left_cap= buf_c; 
				sprintf(buf_n, "%ld", left_node);
				s_left_nodes= buf_n;
				is_trial_expire = false;
				return false;
			}
/*
 * judge the node number
 */
			if (is_exceed_max_host()) {
				dout(0) << "the nodes in the cluster is exceed the limit nodes, so shutdown all osd " << dendl;
				lic_type = GRANT_EXPIRE;
				//s_left_cap=-1,s_left_nodes=-1
				int64_t left_cap = -1;
				int64_t left_node = -1;
 ...
 * show grant data
 */
void Monitor::CephGrantData::show_grant_data(stringstream &ss) 
{
  	valid_grant();
	if (lic_type == GRANT_TRIAL) {
		ss << "    grant_status:trial, left_days:" << s_left_days << "\n";
	}
	else if (lic_type == GRANT_OFFICIAL) {
		ss << "    grant_status:official, left_hosts:" << s_left_nodes << ",left_capacity:" << s_left_cap << "\n";
	}
	else if (lic_type == GRANT_EXPIRE) {
		if(is_trial_expire)
			ss << "    grant_status:expired, need new license, " << "left_days:" << s_left_days << "\n"; 
		else
			ss << "    grant_status:exceeded, need new license, " << "left_hosts:" << s_left_nodes << ",left_capacity:" << s_left_cap << "\n";
	}
}	
/*
 * if unauth or expired
 * then shutdown all osd
 */
void Monitor::CephGrantData::force_shutdown_by_grant(bool is_shutdown_all)
{
	if (!is_shutdown_all)
		return;
	dout(0) << "force_shutdown_by_grant (shutdown all osd)" << dendl;
	if ((lic_type == GRANT_NONE) || (lic_type == GRANT_EXPIRE)) {
		for (int i = 0; i < (mon_gr->osdmon()->osdmap).get_max_osd(); i++) {
			if ((mon_gr->osdmon()->osdmap).is_up(i)) {
				entity_inst_t inst = (mon_gr->osdmon()->osdmap).get_inst(i);
				MShutDownAllOsd *m = new MShutDownAllOsd(is_shutdown_all);
				dout(0) << __func__ << " send " << *m << " to " << inst << dendl;
				(mon_gr->messenger)->send_message(m, inst);
			}
		}
	}
}
