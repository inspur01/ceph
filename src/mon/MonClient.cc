...
bool MonClient::ms_dispatch(Message *m)
{
  ...
  case MSG_LOGACK:
  case CEPH_MSG_CLIENT_UID_REPLY: //zzg
    break;
  default:
    return false;
  }
...
//zzg
  case CEPH_MSG_CLIENT_UID_REPLY:
    handle_clientuid_reply(static_cast<MClientUidReply*>(m));
    break; 
//zzg
  }
  return true;
...
//zzg
int MonClient::get_clientuid(string username, string passwd, int bauth)
{
  ldout(cct, 10) << "Windows uid: " << client_uid << " gid: " << client_gid << dendl;
  Mutex::Locker lock(monc_lock);
  
  MClientGetUid *m = new MClientGetUid;
  m->username = username;
  m->passwd = passwd;
  m->bauth = bauth;
  _send_mon_message(m);
  
  while (get_client_uid)
    clientuid_cond.Wait(monc_lock);
  ldout(cct, 10) << "Getting result " << clientuid_err << " uid: " << client_uid << " gid: " << client_gid << dendl;
  
  return clientuid_err;
}

void MonClient::handle_clientuid_reply(MClientUidReply* m)
{
  ldout(cct, 10)<< "authed?" << m->authenticated << " get user uid "<< m->uid << " gid " << m->gid << dendl;
  
  client_uid = m->uid;
  client_gid = m->gid;
  clientuid_err = m->authenticated;
  clientuid_cond.Signal();
  get_client_uid = false;
  m->put();
}
//zzg
