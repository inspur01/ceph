			     ...
			     std::string format, bufferlist *out)
{
  ...
  else if (command == "ms dump") {
    _ms_status_collection->dump_formatted(f);
  }
  else {
    ...
    _crypto_aes(NULL)
{
  ...
  _ms_status_collection = MsStatusCollection::create_collection(this);
  ...
  _admin_socket->register_command("ms dump", "ms dump", _admin_hook, "dump ms status");
...
CephContext::~CephContext()
{
  ...
  _admin_socket->unregister_command("ms dump");
...
MsStatusCollection *CephContext::get_msstatus_collection()
{
  return _ms_status_collection;
}
