...
OPTION(clog_to_syslog_level, OPT_STR, "warn") // this level and above //modify by yutao for bug3272 at 20150508 'info'��'warn'
OPTION(clog_to_syslog_facility, OPT_STR, "default=daemon audit=local0")

OPTION(mon_cluster_log_to_syslog, OPT_STR, "default=false")
OPTION(mon_cluster_log_to_syslog_level, OPT_STR, "warn")   // this level and above //modify by yutao for bug3272 at 20150508 'info'��'warn'
...
OPTION(mon_cluster_log_file_level, OPT_STR, "warn") //modify by yutao for bug3272 at 20150508 'info'��'warn'
...
OPTION(xio_max_send_inline, OPT_INT, 512)
...
SUBSYS(client, 0, 5)
SUBSYS(fuse_ll, 0, 5)     //add by li.jiebj
SUBSYS(acl, 0, 5)         //add by li.jiebj
SUBSYS(osd, 0, 5)
...
SUBSYS(throttle, 1, 1)
SUBSYS(refs, 0, 0)
SUBSYS(directec, 0, 5) // wumq added for directec
SUBSYS(xio, 1, 5)
...
OPTION(ms_status_collection, OPT_BOOL, false) // ms status collection
...
OPTION(mon_tick_check_interval, OPT_INT, 86400)    //add by zhanghao check the authorization
...
OPTION(auth_cluster_required, OPT_STR, "none")   // required of mon, mds, osd daemons //modify by yutao for bug3272 at 20150508
OPTION(auth_service_required, OPT_STR, "none")   // required by daemons of clients //modify by yutao for bug3272 at 20150508
OPTION(auth_client_required, OPT_STR, "none")     // what clients require of daemons //modify by yutao for bug3272 at 20150508
...
OPTION(client_readahead_max_periods, OPT_LONGLONG, 1)  // as multiple of file layout period (object size * num stripes)
OPTION(client_read_total_bytes_per, OPT_LONGLONG, 960*1024*1024)
OPTION(client_readahead_cacher,OPT_STR,"objectcacher") // filecacher / objectcacher
OPTION(client_readahead_window_ratio,OPT_INT,5)	// compute window size
OPTION(client_readahead_frequency_ratio,OPT_INT,1)	// readahead frequency
OPTION(client_readahead_obj_mult, OPT_FLOAT, 0.5)
OPTION(client_dirty_flush_mode, OPT_INT, 3) // flusher mode add by li.jiebj 0:get dirty bh from bh_lru_dirty 1:sort                                                                         // all the oset dirty list get the largest bh every time 3: loop all the oset and get the largest one 
OPTION(client_file_dirty_threshold_level, OPT_FLOAT, 0.75) // File dirty data threshold

OPTION(mds_md_readahead, OPT_BOOL, false) // 
OPTION(client_md_readahead, OPT_INT, 0) // 
OPTION(client_losf_file_size, OPT_INT, 128)  // losf 
OPTION(client_losf_delay_close, OPT_INT, 1) // losf
OPTION(client_oc_max_release, OPT_INT, 1)   // losf
OPTION(client_losf_write, OPT_INT, 0)   // value range(0,1,2) losf write 0:original process 1:flush_set_new 2:tick+flush_set_new
OPTION(client_file_readahead_num, OPT_INT, 0)// file readahead num add by szh
OPTION(client_file_readahead_mode, OPT_INT, 0)// file readahead num add by szh,0:fs default(recommand),1:by file time(atime or mtime or ctime)
/* add by zhd */
OPTION(client_disable_lookup_in_create, OPT_BOOL, true)
/* end add */
...
OPTION(client_dir_quota, OPT_BOOL, false)
OPTION(client_fs_provisioning, OPT_BOOL, false)
OPTION(client_quota_report, OPT_INT, 100)
OPTION(client_quota_tree_scale, OPT_INT, 1024)
OPTION(client_oc, OPT_BOOL, true)
OPTION(client_oc_size, OPT_U64, 1024*1024*1024* 2)    // GB * n
OPTION(client_oc_max_dirty, OPT_U64, 1024*1024*1024* 2)    // GB * n  (dirty OR tx.. bigish)
OPTION(client_oc_target_dirty, OPT_U64, 1024*1024* 128) // target dirty (keep this smallish)
OPTION(client_oc_max_dirty_age, OPT_DOUBLE, 5.0)      // max age in cache before writeback
OPTION(client_oc_max_flush_under_lock, OPT_INT, 20)	// flush n aged bhs each time 
...
OPTION(client_permissions, OPT_BOOL, false)

OPTION(client_samba_imempool_block, OPT_INT, 1024)
OPTION(client_samba_aio_thread_max, OPT_INT, 32)
OPTION(client_smb_aio_asys_incom_mode, OPT_INT, 1)
OPTION(client_samba_aio_open_bind_cpu, OPT_INT, 0)
...
OPTION(fuse_readdir_permission, OPT_BOOL, false)
OPTION(fuse_check_acl_permissions, OPT_BOOL, false)
...
OPTION(objecter_inflight_op_bytes, OPT_U64, 1024*1024*1024*2) // max in-flight data (both directions)
...
OPTION(osd_pool_default_size, OPT_INT, 2) //modify by yutao for bug3272 at 20150508 '3'��'2'
OPTION(osd_pool_default_min_size, OPT_INT, 1)  // 0 means no specific default; ceph will use size-size/2 //modify by yutao for bug3272 at 20150508 '0'��'1'
...
OPTION(osd_pool_default_tier_min_object_size, OPT_INT, 0)  // 0MB (1MB 1*1024*1024L)
...
OPTION(osd_flush_load_threshold, OPT_FLOAT, 2)
...
OPTION(filestore_commit_timeout, OPT_FLOAT, 3600)
OPTION(filestore_fiemap_threshold, OPT_INT, 4096)
OPTION(filestore_merge_threshold, OPT_INT, 50)
OPTION(filestore_split_multiple, OPT_INT, 10)
...
OPTION(journal_max_write_bytes, OPT_INT, 128 << 20)
OPTION(journal_max_write_entries, OPT_INT, 100)
OPTION(journal_queue_max_ops, OPT_INT, 300)
OPTION(journal_queue_max_bytes, OPT_INT, 256 << 20)
...
OPTION(rgw_max_metadata_num, OPT_INT, 20)
...
OPTION(fuse_posixacl, OPT_BOOL, true) //add by lvq
OPTION(internal_osd_journal_optimize, OPT_BOOL, true)
OPTION(temp_write_min_length, OPT_U32, 1 << 17) // 128 KB
OPTION(osd_pool_direct_erasure_code, OPT_BOOL, true) // wumq added for directec
OPTION(ectransaction_dump_file, OPT_STR, "") // wumq addred for directec
OPTION(optimized_append_min_length, OPT_U32, 0) // off
OPTION(journal_optimize_log_interval, OPT_U32, 1000)

OPTION(osd_heartbeat_use_min_delay_socket, OPT_BOOL, false)  // set SO_PRIORITY of the sockets as 6(high) if true

OPTION(filestore_async_read, OPT_BOOL, false)
OPTION(filestore_async_read_in_journal, OPT_BOOL, true)
OPTION(filestore_async_read_ops, OPT_U32, 300)
OPTION(filestore_async_read_bytes, OPT_U32, 256 << 20) // 256MB

OPTION(temp_write_dio, OPT_BOOL, false)
OPTION(temp_write_sync_by, OPT_STR, "syncfs") // O_SYNC, fsync, syncfs

OPTION(journal_max_read_entries, OPT_U32, 100)
OPTION(journal_max_read_bytes, OPT_U32, 128 << 20) // 128MB

OPTION(mds_max_optimized_ops_per_osd, OPT_U32, 32)  //flc added for quickdelete
OPTION(mds_optimized_op_priority, OPT_U32, 63)//flc added for quickdelete
OPTION(mds_max_optimized_ops, OPT_U64, 524288) // wumq added for quickdelete
OPTION(client_user_quota, OPT_BOOL, false)
OPTION(client_file_quota, OPT_BOOL, true)
OPTION(client_quota_update, OPT_INT, 5) //used for userQuota && dirQuota
OPTION(client_file_update, OPT_INT, 10)
OPTION(client_user_quota_amount, OPT_U32, 2048)
OPTION(client_group_quota_amount, OPT_U32,256)

OPTION(client_extra_root_users, OPT_BOOL, false)
OPTION(client_root_users_hadoop, OPT_STR, "hdfs") // separator ',' between multi-users
