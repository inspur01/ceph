...
static const struct sockaddr *find_ip_in_subnet_list(CephContext *cct,
						     ...
						     const std::string &networks,int whoami)
{
  std::list<string> nets;
  get_str_list(networks, nets);
  int inetworkcount = nets.size();
  if(inetworkcount <= 0)
    return NULL;
  int inetindex = 0;
  if(whoami >= 0)
    inetindex = whoami % inetworkcount;
  int iindex = 0;
...
static void fill_in_one_address(CephContext *cct,
				...
				const char *conf_var,int whoami)
{
  const struct sockaddr *found = find_ip_in_subnet_list(cct, ifa, networks,whoami);
  if (!found) {
...
void pick_addresses(CephContext *cct, int needs,int whoami)
{
  struct ifaddrs *ifa;
  ...
  if ((needs & CEPH_PICK_ADDRESS_PUBLIC)
    ...
    fill_in_one_address(cct, ifa, cct->_conf->public_network, "public_addr",whoami);
  }

  if ((needs & CEPH_PICK_ADDRESS_CLUSTER)
      && cct->_conf->cluster_addr.is_blank_ip()
      && !cct->_conf->cluster_network.empty()) {
    fill_in_one_address(cct, ifa, cct->_conf->cluster_network, "cluster_addr",whoami);
  }
