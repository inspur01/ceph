...
int main(int argc, const char **argv) 
{
  ...
  if (mkfs) {
	...
	monmap.set_epoch(0);
	//modify begin by yutao at 20150331 for bug 3223 ceph mon dump 以及ceph quorum_status 查看集群mon状态，修改时间和创建时间不对
        if(monmap.created.is_zero())
        {
            monmap.created = ceph_clock_now(g_ceph_context);
            monmap.last_changed = monmap.created;
        }
	//modify end by yutao at 20150331 for bug 3223
      }
